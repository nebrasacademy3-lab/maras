...
const mobilePaths = ["/api/mobile/", "/api/public/", "/api/catalog/", "/api/assistant", "/api/support", "/api/course-requests", "/api/favorites", "/api/cart", "/api/profile", "/api/progress", "/api/reviews", "/api/video/"];
...
function acceptedOrigins(request: NextRequest) {
  ...
  for (const configured of [process.env.APP_URL, process.env.NEXT_PUBLIC_SITE_URL, process.env.MOBILE_APP_URL, process.env.EXPO_WEB_ORIGIN]) {
    ...
    try { values.add(new URL(configured).origin); } catch { /* Invalid optional values are ignored. */ }
  }
...
function corsHeaders(origin: string) {
  return {
    ...
    "access-control-allow-headers": "authorization,content-type,x-meras-client,x-meras-course,x-meras-lesson,x-meras-device-id,x-meras-device-label,x-meras-platform",
