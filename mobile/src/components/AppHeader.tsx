...
export function AppHeader({ title, subtitle, back = false, home = false, unread = 0, auth = false, onBack }: { title?: string; subtitle?: string; back?: boolean; home?: boolean; unread?: number; auth?: boolean; onBack?: () => void }) {
  ...
  return <View style={[styles.header, { direction, flexDirection: rowDirection }]}>
    ...
    <View style={[styles.copy, { alignItems: startAlignment }]}><Text style={[styles.title, { color: colors.text }]}>{title || "مراس العلم"}</Text>{subtitle && <Text style={[styles.subtitle, { color: colors.textSoft }]}>{subtitle}</Text>}</View>
...
const styles = StyleSheet.create({
  header: { minHeight: 62, flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 14 }, authHeader: { minHeight: 62, flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: 10, marginBottom: 14 }, authCopy: { flex: 1, minWidth: 130, alignItems: "flex-start" }, authTitle: { fontSize: 16, fontWeight: "900", writingDirection: "rtl" }, authActions: { flexDirection: "row", gap: 7 }, copy: { flex: 1, minWidth: 160, alignItems: "flex-start" }, commerceActions: { flexDirection: "row", gap: 6 }, title: { fontSize: 18, fontWeight: "900", writingDirection: "rtl" }, subtitle: { fontSize: 10, marginTop: 2, writingDirection: "rtl" }, iconButton: { width: 42, height: 42, borderRadius: 15, borderWidth: 1, alignItems: "center", justifyContent: "center" }, badge: { position: "absolute", top: -4, left: -4, minWidth: 18, height: 18, paddingHorizontal: 3, borderRadius: 9, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "white" }, badgeText: { color: "#FFF", fontSize: 8, fontWeight: "900" },
});
