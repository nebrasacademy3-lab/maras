...
export function AppHeader({ title, subtitle, back = false, home = false, unread = 0, auth = false, onBack }: { title?: string; subtitle?: string; back?: boolean; home?: boolean; unread?: number; auth?: boolean; onBack?: () => void }) {
  ...
  const badge = (value: number) => value > 0 ? <View style={[styles.badge, { backgroundColor: colors.danger, borderColor: colors.surface }]}><Text style={styles.badgeText}>{value > 99 ? "99+" : value}</Text></View> : null;
  ...
  return <View style={[styles.header, { direction }]}>
    ...
    <View testID="app-header-title" style={styles.copy}><Text accessibilityRole="header" style={[styles.title, { color: colors.text }]}>{title || "مراس العلم"}</Text>{subtitle && <Text style={[styles.subtitle, { color: colors.textSoft }]}>{subtitle}</Text>}</View>
  </View>;
}

const styles = StyleSheet.create({
  header: { gap: 14, paddingTop: 6, marginBottom: 18 },
  ...
  brandName: { flex: 1, minWidth: 0, fontSize: 13, lineHeight: 20, fontWeight: "900" },
  actions: { flexShrink: 0, alignItems: "center", gap: 6, flexWrap: "nowrap" },
  copy: { gap: 4, width: "100%" }, title: { fontSize: 25, lineHeight: 35, fontWeight: "900" }, subtitle: { fontSize: 13, lineHeight: 21 },
  ...
  iconButton: { width: 44, height: 44, flexShrink: 0, borderRadius: 16, borderWidth: 1, alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(11,31,70,.06)" },
  badge: { position: "absolute", top: -4, end: -4, minWidth: 18, height: 18, paddingHorizontal: 3, borderRadius: 9, alignItems: "center", justifyContent: "center", borderWidth: 2 }, badgeText: { color: "#FFF", fontSize: 8, fontWeight: "900", textAlign: "center" },
});
