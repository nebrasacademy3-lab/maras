...
import { api, STORE_MODE } from "@/src/lib/api";
...
export function AppHeader({ title, subtitle, back = false, home = false, unread = 0, auth = false }: { title?: string; subtitle?: string; back?: boolean; home?: boolean; unread?: number; auth?: boolean }) {
  ...
  const { user } = useAuth();
  const cart = useQuery({ queryKey: ["cart", user?.id], queryFn: () => api<{ count?: number; courseSlugs?: string[]; items?: unknown[] }>("/api/cart"), enabled: Boolean(user) && !auth && STORE_MODE !== "reader" });
  ...
  return <View style={styles.header}>
    ...
    {user && STORE_MODE !== "reader" ? <View style={styles.commerceActions}><Pressable accessibilityRole="button" accessibilityLabel={`السلة${cartCount ? `، ${cartCount} مواد` : ""}`} onPress={() => router.push("/cart")} style={[styles.iconButton, { backgroundColor: colors.surface, borderColor: colors.border }]}><Ionicons name="bag-handle-outline" size={20} color={colors.text} />{badge(cartCount)}</Pressable></View> : null}
