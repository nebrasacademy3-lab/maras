...
export function AdminStudentProfile({ email, onClose, onStepUpRequired, onOpenCourse, onOpenSection }: { email: string; onClose: () => void; onStepUpRequired?: StepUp; onOpenCourse?: (slug: string) => void; onOpenSection?: (section: "finance" | "support" | "requests" | "ai" | "referrals", search: string) => void }) {
  ...
  return <>
    ...
    <Feedback text={feedback} />
    <View style={styles.metrics}><AdminCapability all={["subscriptions.manage"]}><Metric label="اشتراكات نشطة" value={data.summary.activeSubscriptions} icon="shield-checkmark-outline" /></AdminCapability><Metric label="دروس مكتملة" value={data.summary.completedLessons} icon="checkmark-done-outline" /><AdminCapability all={["finance.view"]}><Metric label="قيمة المدفوعات" value={money(data.summary.paidValue)} icon="cash-outline" /></AdminCapability><AdminCapability all={["referrals.manage"]}><Metric label="إحالات مؤهلة" value={data.summary.qualifiedReferrals || 0} icon="gift-outline" /></AdminCapability><AdminCapability all={["ai.manage"]}><Metric label="أدوات مراس" value={data.summary.aiActive ? "اشتراك نشط" : "خطة مجانية"} icon="sparkles-outline" /></AdminCapability><AdminCapability all={["support.manage"]}><Metric label="دعم مفتوح" value={data.summary.openTickets} icon="headset-outline" /></AdminCapability></View>
...
{capabilities.can(["finance.view"]) && <>
    <SectionTitle title="الطلبات" subtitle={`${data.pagination?.orders?.total ?? data.orders.length} طلب`} />{pager("orders")}{onOpenSection && <AppButton title="إدارة مدفوعات الطالب" variant="ghost" onPress={() => onOpenSection("finance", email)} />}
...
{capabilities.can(["notifications.manage"]) && <>
    ...
    {capabilities.can(["ai.manage"])&&pager("ai")}{capabilities.can(["catalog.view"])&&<>{pager("waitlist")}{pager("tracks")}</>}{capabilities.can(["referrals.manage"])&&<>{pager("referrals")}{pager("rewards")}</>}
  </>;
}
