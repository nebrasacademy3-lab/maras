...
export function AdminStudentProfile({ email, onClose, onStepUpRequired, onOpenCourse, onOpenSection }: { email: string; onClose: () => void; onStepUpRequired?: StepUp; onOpenCourse?: (slug: string) => void; onOpenSection?: (section: "finance" | "support" | "requests" | "ai" | "referrals", search: string) => void }) {
  ...
  const profile = useQuery({ queryKey: ["admin-student", email, pages], queryFn: ({ signal }) => api<StudentProfile>(`/api/admin/students/${encodeURIComponent(email)}?${new URLSearchParams(Object.entries(pages).map(([key, page]) => [`${key}Page`, String(page)]))}`, { signal }) });
  const { busy, feedback, run } = useAdminMutation(onStepUpRequired);
  ...
  const saveProfile = async (payload: Record<string, unknown>) => {
    ...
    if (ok) { await client.invalidateQueries({ queryKey: ["admin-student", email] }); await client.invalidateQueries({ queryKey: ["admin-console"] }); }
    return ok;
  };
  ...
  return <>
    ...
    {pager("ai")}{pager("waitlist")}{pager("tracks")}{pager("referrals")}{pager("rewards")}
  </>;
}
