import { GEMINI_VERIFICATION_METRICS, OPERATIONS_PANELS, OPERATIONS_TASKS, type GeminiVerificationSummary } from "@/src/lib/operations-contract";
import {AdminCapability} from "@/src/components/AdminCapability";
import {useAdminCapabilities} from "@/src/lib/admin-capabilities";
import { MerasAlert as Alert, promptNative } from "@/src/lib/interaction-events";
import { useAuth } from "@/src/providers/AuthProvider";
import { AdminStudentActions } from "@/src/components/admin-student-actions";
...
import { ADMIN_STEP_UP_MESSAGE, api, getApiToken, ApiError, isAdminStepUpError, jsonBody } from "@/src/lib/api";
...
function useAdminMutation(onStepUpRequired?: StepUp) {
  ...
  const {user}=useAuth(); const actor=useRef(getApiToken()); const mounted=useRef(true),pending=useRef(false);
  useEffect(()=>{mounted.current=true;actor.current=getApiToken();return()=>{mounted.current=false;};},[user?.id]);
  const [feedback, setFeedback] = useState("");
  const run = async (key: string, action: () => Promise<unknown>, success: string) => {
    if(!mounted.current||pending.current||actor.current!==getApiToken())return false;
    pending.current=true;setBusy(key); setFeedback("");
    try { await action(); if(!mounted.current||actor.current!==getApiToken())return false;setFeedback(success); return true; }
    catch (reason) {
...
function Metric({ label, value, icon }: { label: string; value: string | number; icon: React.ComponentProps<typeof Ionicons>["name"] }) {
  ...
  return <Card style={styles.metric}><Ionicons name={icon} size={20} color={colors.primary} /><Text selectable style={[styles.metricValue, { color: colors.text, fontVariant: ["tabular-nums"] }]}>{typeof value === "number" ? value.toLocaleString("ar-SA") : value}</Text><Text style={[styles.metricLabel, { color: colors.textSoft }]}>{label}</Text></Card>;
}
...
export function AdminFinance({ onStepUpRequired, initialSearch = "" }: { onStepUpRequired?: StepUp; initialSearch?: string }) {
  const {user}=useAuth(); const capabilities=useAdminCapabilities();
  const { colors } = useTheme();
  const client = useQueryClient();
  const [search, setSearch] = useState(initialSearch);
  const finance = useQuery({ queryKey: ["admin-finance", user?.id, capabilities.permissions.join(","), search], queryFn: ({signal}) => api<FinanceData>(`/api/admin/finance${search ? `?search=${encodeURIComponent(search)}` : ""}`,{signal}) });
  const refunds = useQuery({ queryKey: ["admin-finance", user?.id, capabilities.permissions.join(","), "refunds"], queryFn: ({signal}) => api<{ requests: RefundRow[] }>("/api/admin/refunds",{signal}) });
  ...
  const approveReview = async (item: FinanceQueueItem) => {
    const reason = await promptNative("سبب اعتماد الدفعة", "سيُفعّل المحتوى وتصدر الفاتورة ويُشعر الطالب.", "تم التحقق من التحصيل في لوحة Tap");
    if (!reason?.trim()) return;
    if (await run(`approve-${item.orderNumber}`, () => api("/api/admin/finance", { method: "POST", body: jsonBody({ action: "resolvePaymentReview", orderNumber: item.orderNumber, decision: "approve", reason: reason.trim() }) }), "تم اعتماد الدفعة وتفعيل المواد.")) refreshAll();
  };
  const rejectRefund = async (row: RefundRow) => {
    const note = await promptNative("سبب الرفض", "لن يتغير الطلب عند إلغاء هذه النافذة.");
    if (!note?.trim()) return;
    if (await run(`refund-reject-${row.id}`, () => api("/api/admin/refunds", { method: "POST", body: jsonBody({ action: "reject", id: row.id, note: note.trim() }) }), "تم رفض طلب الاسترداد.")) refreshAll();
  };
...
export function AdminOperations({ onStepUpRequired }: { onStepUpRequired?: StepUp }) {
  const {user}=useAuth(); const capabilities=useAdminCapabilities();
  const scope = `${capabilities.owner}:${[...capabilities.permissions].sort().join(",")}`;
  const actionActor = getApiToken();
  const liveAccess=useRef(capabilities);
  useEffect(()=>{liveAccess.current=capabilities;},[capabilities]);
  const { colors } = useTheme();
  const client = useQueryClient();
  const summary = useQuery({ queryKey: ["admin-operations", user?.id, scope, "summary"], queryFn: ({signal}) => api<OperationsData>(OPERATIONS_PANELS.automation.endpoint,{signal}), enabled: capabilities.can(OPERATIONS_PANELS.automation.permissions) });
  const support = useQuery({ queryKey: ["admin-operations", user?.id, scope, "support"], queryFn: ({signal}) => api<SupportMetrics>(OPERATIONS_PANELS.support.endpoint,{signal}), enabled: capabilities.can(OPERATIONS_PANELS.automation.permissions)&&capabilities.can(OPERATIONS_PANELS.support.permissions) });
  const analytics = useQuery({ queryKey: ["admin-operations", user?.id, scope, "analytics"], queryFn: ({signal}) => api<AnalyticsData>(OPERATIONS_PANELS.analytics.endpoint+"?days=30",{signal}), enabled: capabilities.can(OPERATIONS_PANELS.automation.permissions)&&capabilities.can(OPERATIONS_PANELS.analytics.permissions) });
  const { busy, feedback, run } = useAdminMutation(onStepUpRequired);
  const runTask = (key: keyof typeof OPERATIONS_TASKS, label: string) => { const task=OPERATIONS_TASKS[key]; if(actionActor!==getApiToken()||!liveAccess.current.can(OPERATIONS_PANELS.automation.permissions)||!liveAccess.current.can(task.permissions))return; void run(key, () => api(task.endpoint, { method: "POST" }), label).then((ok) => { if (ok) void client.invalidateQueries({ queryKey: ["admin-operations", user?.id, scope] }); }); };
  if (!capabilities.can(OPERATIONS_PANELS.automation.permissions)) return <EmptyState icon="lock-closed-outline" title="التشغيل غير متاح" text="لا تملك صلاحية عرض هذا القسم." />;
  ...
  const verification = summary.data.geminiVerification;
  return <>
    {verification ? <><SectionTitle title="إثبات مشاريع Gemini" subtitle="انتهاء الإثبات يمنع التوليد؛ تجديده لا يفعّل الاحتياط المدفوع."/><View style={styles.metrics}>{GEMINI_VERIFICATION_METRICS.map(item=><Metric key={item.key} label={item.label} value={verification[item.key]} icon="shield-checkmark-outline"/>)}</View></> : null}
    <View style={styles.metrics}><Metric label="ملفات تنتظر التنظيف" value={queues.storageCleanupPending || 0} icon="trash-outline"/><Metric label="تنظيف يحتاج مراجعة" value={queues.storageCleanupBlocked || 0} icon="alert-circle-outline"/><Metric label="تنظيف متأخر" value={queues.storageCleanupOverdue || 0} icon="time-outline"/></View>
...
export function AdminLearningTracks({ onStepUpRequired }: { onStepUpRequired?: StepUp }) {
  ...
  return <>
    ...
    <Card>
      ...
      <Field label="المعرّف (اختياري عند الإضافة)" placeholder="يُنشأ تلقائيًا عند تركه فارغًا" value={draft.slug} autoCapitalize="none" inputDirection="ltr" onChangeText={(slug) => setDraft({ ...draft, slug: slug.toLowerCase().replace(/[^a-z0-9._-]/g, "-") })} />
      ...
      <View style={styles.actions}><AppButton full={false} title={editingId ? "حفظ التعديلات" : "إنشاء المسار"} icon={editingId ? "save-outline" : "add-circle-outline"} loading={busy === "save"} disabled={!draft.title.trim() || Boolean(editingId && !draft.slug.trim())} onPress={() => void save()} />{editingId ? <AppButton full={false} title="إلغاء" variant="ghost" onPress={() => { setEditingId(null); setDraft(emptyTrack); }} /> : null}</View>
    </Card>
...
type StudentProfile = {
  ...
  notifications: { id: number; title: string; body: string; readAt: string | null; pushStatus: string | null }[];
  progress: { id: number; courseSlug: string; lessonId: string; completed: boolean; watchedSeconds: number }[];
  student: { mfaEnabled?: boolean; updatedAt: string; id: number; email: string; phone: string | null; fullName: string; status: string; universitySlug: string | null; specialty: string | null; academicLevel: string | null; lastLoginAt: string | null; createdAt: string };
  ...
  catalog: { courses: { slug: string; title: string }[]; institutions: { slug: string; name: string }[] };
...
export function AdminStudentProfile({ email, onClose, onStepUpRequired, onOpenCourse, onOpenSection }: { email: string; onClose: () => void; onStepUpRequired?: StepUp; onOpenCourse?: (slug: string) => void; onOpenSection?: (section: "finance" | "support" | "requests" | "ai" | "referrals", search: string) => void }) {
  ...
  const profile = useQuery({ queryKey: ["admin-student",user?.id,capabilities.permissions.join(","), email, pages], queryFn: ({ signal }) => api<StudentProfile>(`/api/admin/students/${encodeURIComponent(email)}?${new URLSearchParams(Object.entries(pages).map(([key, page]) => [`${key}Page`, String(page)]))}`, { signal }), enabled: capabilities.can(["students.view"]) });
  const { busy, feedback, run } = useAdminMutation(onStepUpRequired);
  const act = (key: string, payload: Record<string, unknown>, success: string) => void run(key, () => api("/api/admin/console", { method: "POST", body: jsonBody(payload) }), success).then((ok) => { if (ok) { void client.invalidateQueries({ queryKey: ["admin-student",user?.id,capabilities.permissions.join(","), email] }); void client.invalidateQueries({ queryKey: ["admin-console"] }); } });
  ...
  const saveProfile = async (payload: Record<string, unknown>) => {
    const ok = await run("student-action", () => api("/api/admin/console", { method: "POST", body: jsonBody(payload) }), "تم حفظ الإجراء وتحديث ملف الطالب");
    if (ok) { await client.invalidateQueries({ queryKey: ["admin-student",user?.id,capabilities.permissions.join(","), email] }); await client.invalidateQueries({ queryKey: ["admin-console"] }); }
    return ok;
  };
  if (!capabilities.can(["students.view"])) return <EmptyState title="الملف غير متاح ضمن صلاحياتك" text="لم نعرض بيانات الطالب." action={<AppButton title="رجوع" onPress={onClose}/>}/>;
  ...
  const pager = (key: string) => {
    const item = data.pagination?.[key]; if (!item || item.total <= item.pageSize) return null;
    return <View style={styles.actions}><Text style={{ color: colors.text }}>صفحة {item.page} / {Math.ceil(item.total / item.pageSize)} · {item.total} سجل</Text><AppButton full={false} title="السابق" disabled={item.page <= 1 || profile.isFetching} onPress={() => setPages(current => ({ ...current, [key]: item.page - 1 }))} /><AppButton full={false} title="التالي" disabled={item.page * item.pageSize >= item.total || profile.isFetching} onPress={() => setPages(current => ({ ...current, [key]: item.page + 1 }))} /></View>;
  };
  ...
  return <>
    <View style={styles.actions}><AppButton full={false} title="قائمة الطلاب" icon="arrow-back-outline" variant="ghost" onPress={onClose} /><AdminCapability all={["students.manage"]}><AppButton full={false} title={data.student.status === "active" ? "إيقاف الحساب" : "تفعيل الحساب"} variant={data.student.status === "active" ? "danger" : "soft"} loading={busy === "status"} onPress={() => act("status", { action: "updateUser", id: data.student.id, role: "student", status: data.student.status === "active" ? "suspended" : "active" }, data.student.status === "active" ? "تم إيقاف الحساب." : "تم تفعيل الحساب.")} /></AdminCapability></View>
    ...
    <Card><Text style={[styles.rowTitle, { color: colors.text }]}>حماية الحساب</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>MFA: {data.student.mfaEnabled ? "مفعّلة" : "غير مفعّلة"}</Text>{user?.isPlatformOwner && data.student.mfaEnabled && <><Field label="سبب إعادة ضبط MFA بعد التحقق من هوية الطالب" value={mfaReason} onChangeText={setMfaReason} multiline maxLength={600}/><AppButton title="إعادة ضبط MFA وإلغاء الجلسات" variant="danger" disabled={mfaReason.trim().length < 4 || Boolean(busy)} onPress={() => Alert.alert("إعادة ضبط حماية الطالب؟", "ستلغى جلسات الحساب ورموز الاستعادة مع تسجيل السبب.", [{ text: "إلغاء", style: "cancel" }, { text: "إعادة الضبط", style: "destructive", onPress: () => void run("reset-mfa", () => api("/api/admin/staff", { method: "POST", body: jsonBody({ action: "resetMfa", id: data.student.id, reason: mfaReason }) }), "أعيد ضبط MFA وألغيت جلسات الطالب").then(ok => { if (ok) { setMfaReason(""); void profile.refetch(); } }) }])}/></>}</Card>
    <Feedback text={feedback} />
    <View style={styles.metrics}><AdminCapability all={["subscriptions.manage"]}><Metric label="اشتراكات نشطة" value={data.summary.activeSubscriptions} icon="shield-checkmark-outline" /></AdminCapability><Metric label="دروس مكتملة" value={data.summary.completedLessons} icon="checkmark-done-outline" /><AdminCapability all={["finance.view"]}><Metric label="قيمة المدفوعات" value={money(data.summary.paidValue)} icon="cash-outline" /></AdminCapability><AdminCapability all={["referrals.manage", "data.all"]}><Metric label="إحالات مؤهلة" value={data.summary.qualifiedReferrals || 0} icon="gift-outline" /></AdminCapability><AdminCapability all={["ai.manage", "data.all"]}><Metric label="أدوات مراس" value={data.summary.aiActive ? "اشتراك نشط" : "خطة مجانية"} icon="sparkles-outline" /></AdminCapability><AdminCapability all={["support.manage"]}><Metric label="دعم مفتوح" value={data.summary.openTickets} icon="headset-outline" /></AdminCapability></View>
    <AdminStudentActions student={data.student} catalog={data.catalog} run={saveProfile} busy={Boolean(busy)} />
{capabilities.can(["subscriptions.manage"]) && <>
    <SectionTitle title="الاشتراكات والوصول" subtitle={`${data.pagination?.subscriptions?.total ?? data.subscriptions.length} سجل`} />{pager("subscriptions")}
    <View style={styles.list}>{data.subscriptions.map((row) => { const current = state(row); return <Card key={row.id} style={styles.rowCard}><View style={styles.rowHead}><View style={styles.rowCopy}><Text style={[styles.rowTitle, { color: colors.text }]}>{courseName.get(row.courseSlug) || row.courseSlug}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{current} · من {dateLabel(row.startsAt)} إلى {row.expiresAt ? dateLabel(row.expiresAt) : "دون انتهاء"} · {row.source === "tap" ? "شراء" : "منحة"}</Text></View></View>{!row.revokedAt ? <View style={styles.actions}>{current === "نشط" ? <AppButton full={false} title="إيقاف مؤقت" variant="soft" loading={busy === `pause-${row.id}`} onPress={() => act(`pause-${row.id}`, { action: "updateAccess", id: row.id, operation: "pause", reason: "إيقاف من التطبيق", operationKey: `${row.id}-${Date.now()}` }, "تم إيقاف الاشتراك مؤقتًا.")} /> : null}{current === "موقوف" ? <AppButton full={false} title="استئناف" variant="soft" loading={busy === `resume-${row.id}`} onPress={() => act(`resume-${row.id}`, { action: "updateAccess", id: row.id, operation: "resume", reason: "", operationKey: `${row.id}-${Date.now()}` }, "تم استئناف الاشتراك.")} /> : null}<AppButton full={false} disabled={!row.expiresAt} title={row.expiresAt ? "+ 30 يومًا" : "وصول دائم"} variant="ghost" loading={busy === `extend-${row.id}`} onPress={() => act(`extend-${row.id}`, { action: "updateAccess", id: row.id, operation: "extend", reason: "", days: 30, operationKey: `${row.id}-${Date.now()}` }, "تم تمديد الاشتراك 30 يومًا.")} /></View> : null}{onOpenCourse && capabilities.can(["catalog.view","students.view"]) && <AppButton title="ملف المادة والمشتركون" variant="ghost" onPress={() => onOpenCourse(row.courseSlug)} />}</Card>; })}{!data.subscriptions.length ? <EmptyState icon="school-outline" title="لا توجد اشتراكات" text="يمكن منح مادة من تبويب المستخدمين." /> : null}</View>
</>}
{capabilities.can(["finance.view"]) && <>
    <SectionTitle title="الطلبات" subtitle={`${data.pagination?.orders?.total ?? data.orders.length} طلب`} />{pager("orders")}{onOpenSection && capabilities.can(["data.all"]) && <AppButton title="إدارة مدفوعات الطالب" variant="ghost" onPress={() => onOpenSection("finance", email)} />}
    <View style={styles.list}>{data.orders.map((row) => <Card key={row.orderNumber} style={styles.rowCard}><View style={styles.rowHead}><View style={styles.rowCopy}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.orderNumber}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{orderStatusLabels[row.status] || row.status} · {dateLabel(row.createdAt)}</Text></View><Text style={[styles.amount, { color: colors.primary }]}>{money(row.total, row.currency)}</Text></View></Card>)}</View>
</>}
    {capabilities.can(["referrals.manage", "data.all"]) && data.referrals ? <><SectionTitle title="الإحالات والهدايا" subtitle={`رمز ${data.referrals.code?.code || "—"} · ${data.referrals.referred.length} إحالة · ${data.referrals.rewards.length} هدية`} /><View style={styles.list}>{data.referrals.rewards.map((row) => <Card key={row.id} style={styles.rowCard}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.rewardLabel}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{row.coupon?.code || "اشتراك رقمي"} · {row.status}</Text></Card>)}{data.referrals.referred.map((row) => <Card key={`ref-${row.id}`} style={styles.rowCard}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.referred.fullName}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{row.referred.email} · {row.status === "qualified" ? "مؤهلة" : row.status === "rejected" ? "مرفوضة" : "قيد المراجعة"}</Text></Card>)}</View></> : null}
...
{capabilities.can(["students.devices.view"]) && <>
    {pager("sessions")}<SectionTitle title="الأجهزة والجلسات" subtitle={`${data.sessions.filter((row) => !row.revokedAt && Date.parse(row.expiresAt) > loadedAt).length} نشطة في الصفحة`} />
    <View style={styles.list}>{data.sessions.map((row) => <Card key={row.id} style={styles.rowCard}><View style={styles.rowHead}><View style={styles.rowCopy}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.deviceLabel}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{row.platform === "mobile" ? "تطبيق" : "ويب"} · آخر نشاط {dateLabel(row.lastSeenAt)}{row.revokedAt ? " · مُلغاة" : ""}</Text></View>{!row.revokedAt && Date.parse(row.expiresAt) > loadedAt ? <AdminCapability all={["students.devices.manage"]}><AppButton full={false} title="تسجيل خروج" variant="danger" loading={busy === `session-${row.id}`} onPress={() => act(`session-${row.id}`, { action: "revokeUserSession", sessionId: row.id }, "تم تسجيل خروج الجهاز.")} /></AdminCapability> : null}</View></Card>)}</View>
</>}
{capabilities.can(["support.manage"]) && <>
    {pager("support")}{onOpenSection && <AppButton title="فتح دعم الطالب" variant="ghost" onPress={() => onOpenSection("support", email)} />}{data.support.length ? <><SectionTitle title="الدعم" subtitle={`${data.support.length} تذكرة`} /><View style={styles.list}>{data.support.map((row) => <Card key={row.id} style={styles.rowCard}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.title}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{row.ticketNumber} · {row.status}</Text></Card>)}</View></> : null}
</>}
{capabilities.can(["requests.manage"]) && <>
    {pager("requests")}{onOpenSection && <AppButton title="فتح طلبات المواد للطالب" variant="ghost" onPress={() => onOpenSection("requests", email)} />}{data.requests.length ? <><SectionTitle title="طلبات المواد" subtitle={`${data.requests.length} طلب`} /><View style={styles.list}>{data.requests.map((row) => <Card key={row.id} style={styles.rowCard}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.courseName}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{row.status} · {dateLabel(row.createdAt)}</Text></Card>)}</View></> : null}
</>}
    <SectionTitle title="التقدم في الدروس" />{pager("progress")}<View style={styles.list}>{data.progress.map(row => <Card key={row.id}><Text style={{ color: colors.text }}>{courseName.get(row.courseSlug) || row.courseSlug} · {row.lessonId}</Text><Text style={{ color: colors.textSoft }}>{row.completed ? "مكتمل" : "قيد التعلم"} · {Math.round(row.watchedSeconds / 60)} دقيقة</Text></Card>)}</View>
{capabilities.can(["notifications.manage", "data.all"]) && <>
    <SectionTitle title="الإشعارات وحالة القراءة" />{pager("notifications")}<View style={styles.list}>{data.notifications.map(row => <Card key={row.id}><Text style={{ color: colors.text, fontWeight: "800" }}>{row.title}</Text><Text style={{ color: colors.textSoft }}>{row.body}</Text><Text style={{ color: colors.primary }}>{row.readAt ? "مقروء" : "غير مقروء"} · {row.pushStatus || "داخل المنصة"}</Text></Card>)}</View>
</>}
    {capabilities.can(["ai.manage", "data.all"])&&pager("ai")}{capabilities.can(["catalog.view"])&&<>{pager("waitlist")}{pager("tracks")}</>}{capabilities.can(["referrals.manage", "data.all"])&&<>{pager("referrals")}{pager("rewards")}</>}
  </>;
}
