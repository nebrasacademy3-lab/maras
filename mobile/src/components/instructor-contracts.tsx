...
function ContractCard({ contract, userId }: { contract: Contract; userId: number }) {
  ...
  return <Card style={{ gap: 12 }}><Text style={{ ...text, fontWeight: "800", fontSize: 21 }}>{contract.title}</Text><WorkspaceStatus label={statuses[contract.status]} tone={contract.status === "signed" ? "success" : contract.status === "withdrawn" || contract.status === "terminated" ? "danger" : "primary"} />
    <View style={{ flexDirection: "row-reverse", flexWrap: "wrap", gap: 9 }}>
      <View style={{ flex: 1, minWidth: 130, padding: 13, borderRadius: 15, backgroundColor: colors.surfaceAlt }}><Text style={{ ...text, color: colors.textSoft, fontSize: 12 }}>التعويض المتفق عليه</Text><Text style={{ ...text, color: colors.text, fontWeight: "900", fontSize: 20 }}>{(contract.rateHalalas / 100).toLocaleString("ar-SA")} ر.س</Text><Text style={{ ...text, fontSize: 12 }}>لكل {contract.compensationModel === "hourly" ? "ساعة" : "مادة"}</Text></View>
      <View style={{ flex: 1, minWidth: 130, padding: 13, borderRadius: 15, backgroundColor: colors.surfaceAlt }}><Text style={{ ...text, color: colors.textSoft, fontSize: 12 }}>الفترة التجريبية</Text><Text style={{ ...text, color: colors.text, fontWeight: "900", fontSize: 20 }}>{contract.trialDays} يومًا</Text><Text style={{ ...text, fontSize: 12 }}>نسخة العقد {contract.version}</Text></View>
    </View>
    <AppButton title={expanded ? "إخفاء بنود العقد" : "قراءة بنود العقد والتفاصيل"} icon={expanded ? "chevron-up-outline" : "document-text-outline"} variant="soft" onPress={() => setExpanded(value => !value)} />
    {expanded ? <View style={{ gap: 14 }}><Text selectable style={text}>{contract.organization.legal_name}{"\n"}السجل التجاري: {contract.organization.commercial_registration_number}{"\n"}الرقم الضريبي: {contract.organization.vat_number || "غير مسجل في العقد"}{"\n"}{contract.organization.legal_address}{"\n"}الشارح: {contract.instructor.fullName}</Text><Text selectable style={text}>المباشرة: {contract.employment.startDate} · النهاية: {contract.employment.endDate || "غير محددة"}{"\n"}مكان العمل: {contract.employment.workLocation}{"\n"}الساعات الأسبوعية: {contract.employment.weeklyHours}{"\n"}الجنسية: {contract.employment.nationality}</Text><Text selectable style={text}>{contract.termsAr}</Text><Text selectable style={text}>التجربة والتقييم: {contract.trialTermsAr}</Text><Text selectable style={text}>بدء التعويض: {contract.employment.compensationStart==="after_trial_approval"?"بعد تقييم التجربة واعتماد الاستمرار":"وفق بنود ومواعيد الدفع في هذه النسخة"}{"\n"}{contract.employment.compensationConditionsAr}</Text><Text selectable style={text}>الدفع: {contract.employment.paymentTermsAr}{"\n"}المزايا: {contract.employment.benefitsAr}</Text><Text selectable style={en}>{contract.termsEn}</Text><Text selectable style={en}>Compensation commencement: {contract.employment.compensationStart==="after_trial_approval"?"After probation assessment and approval":"Under the payment terms in this version"}{"\n"}{contract.employment.compensationConditionsEn}{"\n"}Trial and evaluation: {contract.trialTermsEn}{"\n"}Payment: {contract.employment.paymentTermsEn}{"\n"}Benefits: {contract.employment.benefitsEn}</Text>{contract.signedAt ? <Text style={text}>تاريخ التوقيع: {new Date(contract.signedAt).toLocaleString("ar-SA")}</Text> : null}
