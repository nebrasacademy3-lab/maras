...
type Sources = { resources: Source[]; lessons: Array<{ id: string; title: string }> };
...
export function AdminLessonSources({ courses }: { courses: Array<{ slug: string; title: string }> }) {
  ...
  const sources = useQuery({ queryKey: ["admin-lesson-sources", course], queryFn: () => api<Sources>(`/api/admin/course-resources?course=${encodeURIComponent(course)}`), enabled: Boolean(course) });
  ...
  return <>
    ...
    <Card style={{ gap: 14 }}>
      ...
      {course ? <>
        <SearchPicker placeholder="ملف مشترك" label="ربط الملف بالدرس" value={lesson} items={lessons} onSelect={item => { if (!busy) setLesson(item.key); }} />
        ...
        {sources.data?.resources.map(row => <View key={row.id} style={{ gap: 10, padding: 14, borderWidth: 1, borderColor: colors.border, borderRadius: 16 }}>
          ...
          <SearchPicker placeholder="ملف مشترك" label="الدرس المرتبط" value={row.lessonId || ""} items={lessons} onSelect={item => { if (!busy) void update(row, { lessonId: item.key || null }); }} />
