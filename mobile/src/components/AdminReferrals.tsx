...
type Tier = { id: number; name: string; description: string; requiredReferrals: number; rewardType: RewardType; rewardValue: number; rewardDurationDays: number | null; couponValidityDays: number | null; courseSlug: string | null; enabled: boolean; sortOrder: number };
type AdminReferralResponse = {
  ...
  tiers: Tier[];
  students: { userId: number; email: string; fullName: string; code: string; shareCount: number; counts: { total: number; qualified: number; pending: number; rejected: number }; nextTier: Tier | null }[];
  ...
  rewards: { id: number; user: { id: number; email: string; fullName: string }; rewardType: RewardType; rewardValue: number; sourceType: string; status: string; coupon: { id: number; code: string; status: string; used: boolean } | null; issuedAt: string; expiresAt: string | null; note: string | null }[];
};
...
export function AdminReferrals() {
  ...
  const [tier, setTier] = useState({ name: "", description: "", requiredReferrals: "5", rewardType: "coupon_percent" as RewardType, rewardValue: "25", rewardDurationDays: "30", couponValidityDays: "90", courseSlug: "", enabled: true, sortOrder: "10" });
  ...
  const createTier = async () => {
    const ok = await mutate("POST", { action: "create_tier", tier: { ...tier, requiredReferrals: Number(tier.requiredReferrals), rewardValue: Number(tier.rewardValue), rewardDurationDays: Number(tier.rewardDurationDays) || null, couponValidityDays: Number(tier.couponValidityDays) || null, courseSlug: tier.courseSlug.trim() || null, sortOrder: Number(tier.sortOrder) || 0 } }, "تم إنشاء مستوى المكافأة.", "tier-create");
  ...
  return <>
    ...
    {section === "tiers" ? <>
      <SectionTitle title="إنشاء مستوى مكافأة" subtitle="الإصدار تلقائي ولا يحتاج مطالبة من الطالب" />
      <Card>
    ...
    {section === "gifts" ? <>
      ...
      <View style={styles.list}>{data.rewards.slice(0, 50).map((row) => <Card key={row.id} style={styles.rowCard}><View style={styles.rowHead}><View style={[styles.rowIcon, { backgroundColor: colors.surfaceAlt }]}><Ionicons name={row.rewardType === "ai_subscription" ? "sparkles-outline" : "ticket-outline"} size={21} color={colors.primary} /></View><View style={styles.rowCopy}><Text style={[styles.rowTitle, { color: colors.text }]}>{row.user.fullName}</Text><Text style={[styles.rowMeta, { color: colors.textSoft }]}>{rewardLabels[row.rewardType]} · {row.rewardValue} · {row.sourceType}</Text></View><Status active={row.status === "active"} /></View>{row.coupon ? <Text selectable style={[styles.couponCode, { color: colors.primary }]}>{row.coupon.code} · {row.coupon.used ? "مستخدم" : row.coupon.status}</Text> : null}<AppButton title={row.status === "active" ? "إيقاف المكافأة" : "تفعيل المكافأة"} variant={row.status === "active" ? "danger" : "soft"} loading={busy === `reward-${row.id}`} onPress={() => void mutate("PATCH", { action: "reward_status", id: row.id, status: row.status === "active" ? "disabled" : "active" }, "تم تحديث حالة المكافأة والكوبون المرتبط.", `reward-${row.id}`)} /></Card>)}</View>
    </> : null}
  </>;
}
