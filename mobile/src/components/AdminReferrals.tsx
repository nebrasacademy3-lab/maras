...
const rewardLabels: Record<RewardType, string> = { coupon_percent: "كوبون نسبة %", coupon_fixed: "كوبون مبلغ ر.س", ai_subscription: "اشتراك أدوات مراس" };
...
export function AdminReferrals({ onStepUpRequired, users = [], courses = [] }: { onStepUpRequired?: (message: string) => void; users?: PickerItem[]; courses?: PickerItem[] } = {}) {
  ...
  return <>
    ...
    <SearchPicker label="أقسام الإحالات والهدايا" value={section} placeholder="اختر القسم" items={[{ key: "overview", label: "السياسة" }, { key: "tiers", label: "المستويات" }, { key: "students", label: "الطلاب" }, { key: "gifts", label: "الهدايا" }]} onSelect={(item) => setSection(item.key as typeof section)} />
    ...
    {section === "tiers" ? <>
      ...
      <Card>
        ...
        <Field label={tier.rewardType === "coupon_percent" ? "نسبة الخصم" : tier.rewardType === "coupon_fixed" ? "مبلغ الخصم" : "عدد أشهر الأدوات"} keyboardType="decimal-pad" value={tier.rewardValue} onChangeText={(rewardValue) => setTier({ ...tier, rewardValue: rewardValue.replace(/[^0-9.]/g, "") })} />
        {tier.rewardType !== "ai_subscription" ? <><Field label="صلاحية الكوبون بالأيام" keyboardType="number-pad" value={tier.couponValidityDays} onChangeText={(couponValidityDays) => setTier({ ...tier, couponValidityDays: couponValidityDays.replace(/\D/g, "") })} /><SearchPicker label="المادة — اختياري" value={tier.courseSlug} placeholder="كل المواد" items={courses} onSelect={(item) => setTier({ ...tier, courseSlug: item.key })} onClear={() => setTier({ ...tier, courseSlug: "" })} /></> : <Field label="مدة الاستحقاق بالأيام — اختياري" keyboardType="number-pad" value={tier.rewardDurationDays} onChangeText={(rewardDurationDays) => setTier({ ...tier, rewardDurationDays: rewardDurationDays.replace(/\D/g, "") })} />}
