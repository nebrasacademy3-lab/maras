...
export function Screen({ children, scroll = true, padded = true, keyboard = false, footer = true, style }: { children: React.ReactNode; scroll?: boolean; padded?: boolean; keyboard?: boolean; footer?: boolean; style?: ViewStyle }) {
  const { colors } = useTheme();
  const content = scroll ? <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, padded && styles.padded, style]}><View style={[styles.screenContent]}>{children}{footer ? <MobileFooter /> : null}</View></ScrollView> : <View style={[styles.flex, padded && styles.padded, style]}><View style={[styles.screenContent]}>{children}{footer ? <MobileFooter /> : null}</View></View>;
