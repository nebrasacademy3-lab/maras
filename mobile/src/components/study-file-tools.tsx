...
export function StudyFileTools({ action, resources, scope = "workspace" }: { action: StudyAction; resources?: Resource[]; scope?: string }) {
  ...
  return <Card style={styles.toolPanel}>
    ...
    <View style={[styles.submitPanel, { backgroundColor: colors.surfaceAlt, borderColor: colors.border }]}><View style={[styles.stepHead, { flexDirection: rowDirection }]}><View style={[styles.stepNumber, { backgroundColor: colors.primary }]}><Text style={styles.stepNumberText}>٣</Text></View><Text style={[styles.stepTitle, { color: colors.text }]}>أنشئ النتيجة وراجعها</Text></View><Text style={[styles.submitHint, { color: colors.textSoft }]}>{resources ? "سنستخدم الملف المعتمد للدرس، دون رفع جديد." : file ? "ملفك جاهز. يمكنك الآن بدء المعالجة." : "اختر ملفًا أولًا لتفعيل إنشاء النتيجة."}</Text><AppButton title={busy ? "جارٍ المعالجة" : names[action]} loading={busy} disabled={Boolean(pendingId) || (resources ? !resourceId : !file) || !language.trim()} onPress={() => void run()}/></View>
    ...
    {error ? <View style={[styles.errorPanel, { borderColor: colors.danger, backgroundColor: colors.surfaceAlt, flexDirection: rowDirection }]}><Ionicons name="alert-circle-outline" size={21} color={colors.danger} /><Text selectable accessibilityRole="alert" style={[styles.errorText, { color: colors.danger }]}>{error}</Text></View> : null}
    ...
    {result?.artifact && <View style={[styles.resultPanel, { borderColor: colors.border }]}><View style={[styles.resultHead, { flexDirection: rowDirection }]}><Ionicons name="checkmark-circle" size={23} color={colors.success} /><Text style={[styles.resultTitle, { color: colors.text }]}>{result.artifact.title}</Text></View><StudyArtifactDownload id={result.artifact.id}/><StudyRichText content={result.artifact.content}/></View>}
    {result?.quiz && <InlineQuiz key={result.quiz.id} id={result.quiz.id}/>}
    <View style={[styles.disclaimer, { borderTopColor: colors.border, flexDirection: rowDirection }]}><Ionicons name="shield-checkmark-outline" size={17} color={colors.textSoft} /><Text style={[styles.disclaimerText, { color: colors.textSoft }]}>إعداد وتنسيق مراس العلم. المحتوى مساعدة دراسية؛ راجعه مع مرجع المقرر. حقوق المصدر لأصحابه.</Text></View>
  </Card>;
}

const styles = StyleSheet.create({
  flex: { flex: 1, minWidth: 0 }, toolPanel: { gap: 17, padding: 19, borderRadius: 23 }, toolHead: { alignItems: "center", gap: 11 }, toolMark: { width: 49, height: 49, borderRadius: 16, alignItems: "center", justifyContent: "center" }, toolTitle: { fontSize: 21, lineHeight: 31, fontWeight: "900" }, toolIntro: { fontSize: 12, lineHeight: 21 }, stepHead: { alignItems: "center", gap: 9 }, stepNumber: { width: 27, height: 27, borderRadius: 14, alignItems: "center", justifyContent: "center" }, stepNumberText: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" }, stepTitle: { fontSize: 15, lineHeight: 24, fontWeight: "900" }, submitPanel: { borderWidth: 1, borderRadius: 19, padding: 15, gap: 12 }, submitHint: { fontSize: 12, lineHeight: 21 }, errorPanel: { borderWidth: 1, borderRadius: 15, padding: 13, alignItems: "center", gap: 9 }, errorText: { flex: 1, fontSize: 13, lineHeight: 22 }, resultPanel: { borderWidth: 1, borderRadius: 19, padding: 15, gap: 16 }, resultHead: { alignItems: "center", gap: 9 }, resultTitle: { flex: 1, fontSize: 18, lineHeight: 27, fontWeight: "900" }, disclaimer: { borderTopWidth: 1, paddingTop: 14, gap: 8, alignItems: "flex-start" }, disclaimerText: { flex: 1, fontSize: 12, lineHeight: 21 },
});
...
export function LessonStudyTools({ courseSlug, lessonId }: { courseSlug: string; lessonId: string }) {
  ...
  return <View style={{ gap: 12, marginVertical: 16 }}><SectionTitle title="ثبّت فهمك بعد الدرس" subtitle="أدوات مباشرة من ملف الدرس دون إعادة رفعه"/>{query.isLoading ? <LoadingState label="تحميل ملف الدرس…"/> : query.isError ? <><Text selectable style={{ color: colors.danger }}>{query.error.message}</Text><AppButton title="إعادة المحاولة" onPress={() => void query.refetch()}/></> : !resources.length ? <EmptyState icon="document-outline" title="بانتظار ملف الدرس" text="ستظهر الأدوات هنا عندما تربط الإدارة ملفًا مدعومًا بهذا الدرس." /> : action ? <><AppButton title="كل أدوات الدرس" variant="ghost" onPress={() => setAction(null)}/><StudyFileTools key={`${user?.id}.${lessonId}.${action}`} action={action} resources={resources} scope={`${courseSlug}.${lessonId}`}/></> : <View style={{ gap: 12 }}>{(["summary", "translation", "quiz"] as const).map(item => <Pressable accessibilityRole="button" key={item} onPress={() => setAction(item)} style={({ pressed }) => ({ backgroundColor: pressed ? colors.surfaceAlt : colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 18, padding: 20, gap: 8, minHeight: 116 })}><View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}><View style={{ width: 42, height: 42, borderRadius: 14, backgroundColor: colors.surfaceAlt, alignItems: "center", justifyContent: "center" }}><Ionicons name={item === "quiz" ? "help-circle-outline" : item === "translation" ? "language-outline" : "document-text-outline"} size={22} color={colors.primary} /></View><Text style={{ color: colors.text, fontWeight: "900", fontSize: 17, flex: 1 }}>{names[item]}</Text><Ionicons name="arrow-back" size={20} color={colors.primary} /></View><Text style={{ color: colors.textSoft, lineHeight: 22 }}>{item === "quiz" ? "بطاقات وأسئلة مع الشرح وإعادة المحاولة" : "ملف PDF منظم باسمك يحمل هوية مراس وروابط التواصل"}</Text></Pressable>)}</View>}</View>;
}
