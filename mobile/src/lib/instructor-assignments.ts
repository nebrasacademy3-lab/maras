...
export type InstructorLesson={id:number;sourceLessonId?:string|null;existingVideo?:boolean;title:string;description:string;position:number;video:{id:number;status:string;processingStatus:string;processingProgress:number;durationSeconds:number}|null};
export type InstructorUnit={id:number;sourceUnitId?:number|null;title:string;description:string;position:number;lessons:InstructorLesson[]};
