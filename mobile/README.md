...
راجع [تقرير التسليم الحالي](../verification/README_AR.md) و[جاهزية التطبيقات وسياسة الشراء](../docs/mobile-readiness-2026-09-20.md) قبل إعداد الربط. لم تُبنَ ملفات متجر موقعة ولم تُجرّب الأجهزة الأصلية في الجولة الحالية.
...
- وضع `reader` للمتاجر لاستخدام المحتوى المفعّل بالحساب؛ الاشتراك من الويب. وضع `direct` للتوزيع الداخلي والويب فقط. أزيلت مكتبة RevenueCat ومفاتيحها من التطبيق.

## المتطلبات

- Node.js 22.13 أو أحدث.
- خادم مراس مهيأ عبر HTTPS ومتصل ببيئة الخدمات المناسبة.
...
نتائج التحقق مؤرخة في [تقرير الجاهزية](../docs/mobile-readiness-2026-09-20.md). تصدير JavaScript لا يعادل تشغيل التطبيق على جهاز أو قبوله في المتجر.
...
- `reader`: إعداد الإنتاج. يعرض المحتوى والاشتراكات الحالية وسجل الطلبات دون شراء داخلي. يعرض Android إرشادًا نصيًا غير قابل للنقر للاشتراك عبر الموقع. لا يعرض iOS دعوة لشراء خارجي دون استحقاق وموافقة مناسبين من Apple.
- `direct`: دفع الويب لنسخ التوزيع الداخلي فقط. يرفض إعداد التطبيق استخدامه لأي ملف بناء متجر، حتى لو كان اسم الملف جديدًا.
- سجل مشتريات المتاجر السابقة وحقوق الوصول المخزنة بالخادم يبقيان متاحين. لا توجد عملية شراء أو استعادة جديدة عبر مكتبة المتجر.

ملف `staging-reader` في EAS يحتاج عنوان HTTPS لبيئة الاختبار. ملفات preview/development الحالية تشير إلى النطاق المحفوظ للمنصة؛ استخدم بيئة منفصلة قبل اختبارات كتابة البيانات. أعِد بناء التطبيق بعد تغيير الإعدادات العامة.
