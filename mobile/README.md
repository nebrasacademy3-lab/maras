...
- أيقونة خارجية مربعة 1024×1024 تحمل العلامة الرسمية **M مع النجمة كاملة** ضمن مساحة أمان تمنع القص على Android وiOS.
- Splash وشعار متكيف وفافيكون وأيقونة إشعارات Android أحادية اللون، كلها مشتقة من الشعار الرسمي نفسه وبدقة عالية للوضعين الفاتح والليلي.
...
- مسارات المساعد محصورة في وجهات داخلية آمنة، وتحافظ على عبارة البحث، وتغطي السلة والمفضلة والطلبات والفواتير وصفحات السياسات والمعلومات العربية دون تحويلها خطأً إلى الدعم.
- مساحة مشرف: طابور الطلبات، تحديث الحالات، إنشاء الوحدات والدروس، ورفع فيديو خاص ضمن نطاق الإشراف فقط.
- لوحة إدارة متكافئة مع الويب: الحسابات والأدوار، نطاقات المشرفين، الجهات والشعارات، التخصصات والمواد والوحدات والدروس، الطلبات والدعم، منح الوصول، الكوبونات، المبيعات، التقييمات، الإعلانات، الإشعارات المتقدمة وروابطها الداخلية، ووسائل التواصل.
- عزل ذاكرة الاستعلامات والفلاتر والإعلانات المغلقة بحسب هوية الحساب، مع حفظ ذري للجلسة في SecureStore وإلغاء تسجيل Push عند الخروج أو تبديل الحساب.
...
يفحص `npm run test:contracts` استعادة الجلسة وحفظها الذري، وعزل مرشحات الكتالوج والإعلانات وذاكرة الاستعلامات وإلغاء تسجيل Push، كما يتحقق من بقاء بحث المساعد مستقلًا عن الفلاتر. لا يغني ذلك عن اختبار Push والاستعادة على جهاز فعلي.

تُسجل نتيجة فحوص نسخة التسليم الأحدث وتاريخها في `../VALIDATION.md` بعد كل تشغيل؛ لا تعتمد على نتيجة إصدار سابق عند النشر.
