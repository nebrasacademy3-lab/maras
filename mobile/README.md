...
راجع [تقرير التسليم الحالي](../verification/README_AR.md) و[دليل RevenueCat](../docs/revenuecat-setup-ar.md) قبل إعداد الربط. لم تُبنَ ملفات متجر موقعة ولم تُجرّب الأجهزة الأصلية في الجولة الحالية.
...
- وضع `iap` لمشتريات المتاجر عبر RevenueCat، ووضع `direct` للتوزيع الداخلي، و`reader` للقراءة دون شراء.
...
نتائج 27 أغسطس 2026 تاريخية. في التسليم الحالي نجحت اختبارات التطبيق السبعون وفحص الأنواع والتحليل وتصدير Android وiOS؛ تصدير JavaScript لا يعادل تشغيل جهاز. راجع التقرير الحالي للأدلة والاختبارات المعلقة.
...
- `iap`: إعداد بناء الإنتاج الحالي. يعرض أسعار المتجر الفعلية ويشتري ويستعيد عبر RevenueCat مع تحقق الخادم. يلزم إعداد مفاتيح SDK العامة والمنتجات وحسابات المتاجر.
- `reader`: قراءة المواد المفعلة دون زر شراء؛ يُستخدم أيضًا عند طلب IAP داخل Expo Go الذي لا يشغّل SDK الأصلي.
- `direct`: دفع مباشر لنسخ التوزيع الداخلي فقط. لا تستخدمه لإرسال نسخة المتجر.

ملف `staging-iap` في EAS يحتاج استبدال عنوانه التجريبي بعنوان HTTPS لبيئة الاختبار. ملفات preview/development الحالية تشير إلى النطاق المحفوظ للمنصة؛ غيّر عنوانها إلى staging قبل أي اختبار يكتب بيانات. أدوات مراس تُجدّد يدويًا لمدة 30 يومًا، ولا يوجد تجديد تلقائي.
