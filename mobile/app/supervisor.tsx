...
import { WorkspaceHero, WorkspaceStat } from "@/src/components/staff-workspace-ui";
...
export default function Supervisor() {
  ...
  return <Screen keyboard>
    <AppHeader title="مساحة المشرف" subtitle="العمل ضمن نطاقك" back />
    <WorkspaceHero eyebrow="مركز الإشراف" title={user?.fullName || "مساحة المشرف"} description="تابع طلبات الطلاب، وأنجز المحتوى المسند إليك في خطوات واضحة." icon="layers-outline" />
    <View style={{ flexDirection: "row-reverse", flexWrap: "wrap", gap: 9, marginBottom: 17 }}>
      <WorkspaceStat value={String(workspace.data.courses.length)} label="مواد ضمن النطاق" icon="library-outline" />
      <WorkspaceStat value={String(requests.data.requests.length)} label="طلبات الطلاب" icon="file-tray-full-outline" />
      <WorkspaceStat value={String(workspace.data.lessons.length)} label="دروس المحتوى" icon="play-circle-outline" />
    </View>
...
function TabButton({ active, label, icon, onPress, colors }: { active: boolean; label: string; icon: React.ComponentProps<typeof Ionicons>["name"]; onPress: () => void; colors: ReturnType<typeof useTheme>["colors"] }) {
  return <Pressable accessibilityRole="tab" accessibilityState={{ selected: active }} accessibilityLabel={label} onPress={onPress} style={[styles.tab, { backgroundColor: active ? colors.primary : colors.surface, borderColor: active ? colors.primary : colors.border }]}><Ionicons name={icon} size={20} color={active ? "#FFF" : colors.primary} /><Text style={{ color: active ? "#FFF" : colors.text, fontSize: 12, fontWeight: "900", textAlign: "center" }}>{label}</Text></Pressable>;
}
...
function RequestQueue({ rows, colors, run, openingFile, onOpenFile }: { rows: RequestRow[]; colors: ReturnType<typeof useTheme>["colors"]; run: (task: () => Promise<unknown>, success: string) => Promise<void>; openingFile: number | null; onOpenFile: (file: RequestRow["files"][number]) => Promise<void> }) {
  ...
  return <>
    ...
    {rows.length ? rows.map((row) => <Card key={row.id} style={styles.request}>
      ...
      <SearchPicker label="حالة الطلب" value={row.status} placeholder="اختر الحالة" items={statuses.map((key) => ({ key, label: labels[key] || key }))} onSelect={(item) => run(() => api("/api/supervisor/requests", { method: "PATCH", body: jsonBody({ id: row.id, status: item.key }) }), "تم تحديث الطلب وإشعار الطالب")} />
...
function ContentManager({ data, colors, run }: { data: Workspace; colors: ReturnType<typeof useTheme>["colors"]; run: (task: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  return <>
    ...
    <SearchPicker label="المادة المسندة" value={courseSlug} placeholder="ابحث عن المادة" items={data.courses.map((course) => ({ key: course.slug, label: course.title, detail: `${course.code || ""} · ${course.university}` }))} onSelect={(item) => changeCourse(item.key)} />
    ...
    <Card>
      ...
      <SearchPicker label="الوحدة" hideLabel value={String(activeUnitId || "")} placeholder="ابحث عن الوحدة" items={units.map((unit) => ({ key: String(unit.id), label: unit.title }))} onSelect={(item) => setUnitId(Number(item.key))} />
...
const styles = StyleSheet.create({ flex: { flex: 1 }, tabs: { flexDirection: "row-reverse", gap: 8, marginBottom: 5 }, tab: { flex: 1, minHeight: 78, borderWidth: 1, borderRadius: 18, flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, paddingHorizontal: 5 }, feedback: { fontSize: 13, fontWeight: "800", textAlign: "center", marginTop: 12 }, request: { marginBottom: 10 }, requestHead: { flexDirection: "row", alignItems: "center", gap: 10 }, counter: { minWidth: 48, minHeight: 40, borderRadius: 13, alignItems: "center", justifyContent: "center" }, title: { fontSize: 17, fontWeight: "900", textAlign: "right" }, meta: { fontSize: 12, lineHeight: 19, marginTop: 4, textAlign: "right" }, notes: { fontSize: 14, lineHeight: 23, textAlign: "right", marginVertical: 10 }, file: { minHeight: 48, borderTopWidth: 1, flexDirection: "row", alignItems: "center", gap: 8 }, fileName: { flex: 1, fontSize: 13, textAlign: "right" }, fileSize: { fontSize: 11 }, statuses: { flexDirection: "row", gap: 6, paddingVertical: 9 }, status: { minHeight: 35, paddingHorizontal: 11, borderRadius: 11, alignItems: "center", justifyContent: "center" }, courses: { flexDirection: "row", gap: 8, paddingBottom: 12 }, courseChip: { minHeight: 44, borderWidth: 1, borderRadius: 14, paddingHorizontal: 14, alignItems: "center", justifyContent: "center" }, fieldLabel: { fontSize: 12, fontWeight: "800", textAlign: "right", marginBottom: 5 }, unitCard: { marginBottom: 10, paddingVertical: 7 }, unitTitle: { fontSize: 14, fontWeight: "900", textAlign: "right", padding: 9 }, lesson: { minHeight: 75, borderTopWidth: 1, flexDirection: "row", alignItems: "center", gap: 9, paddingVertical: 10 }, lessonTitle: { fontSize: 14, fontWeight: "900", textAlign: "right" }, videoState: { width: 42, height: 42, borderRadius: 13, alignItems: "center", justifyContent: "center" } });
