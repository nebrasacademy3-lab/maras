...
export default function AiConversationScreen() {
  ...
  if (!user) return <Screen><AppHeader title="مراس AI" back /><EmptyState title="سجّل الدخول أولًا" text="هذه المحادثة خاصة بحساب صاحبها." action={<AppButton title="تسجيل الدخول" onPress={() => router.replace("/(auth)/login")} />} /></Screen>;
  if (!Number.isInteger(id) || id <= 0) return <Screen><AppHeader title="مراس AI" back /><EmptyState title="رابط المحادثة غير صالح" text="ارجع إلى سجل مراس AI واختر المحادثة من هناك." /></Screen>;
  if (query.isLoading) return <Screen><AppHeader title="مراس AI" back /><LoadingState label="تحميل المحادثة…" /></Screen>;
  if (query.isError || !query.data) return <Screen><AppHeader title="مراس AI" back /><EmptyState icon="cloud-offline-outline" title="تعذر فتح المحادثة" text={query.error instanceof Error ? query.error.message : "حاول مرة أخرى."} action={<AppButton title="إعادة المحاولة" onPress={() => void query.refetch()} />} /></Screen>;
  ...
  return <Screen keyboard>
    <AppHeader title={data.conversation.title || "محادثة مراس AI"} subtitle="سجل خاص محفوظ في حسابك" back />
    ...
    <Card style={styles.composerCard}><TextInput multiline value={input} onChangeText={setInput} editable={!sending} placeholder="اكتب سؤالك لمراس AI…" placeholderTextColor={colors.textSoft} style={[styles.input, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} /><AppButton title={sending ? "يفكر مراس AI…" : "إرسال"} icon="arrow-up-outline" loading={sending} disabled={input.trim().length < 2} onPress={() => void send()} />{error ? <Text style={[styles.error, { color: colors.danger }]}>{error}</Text> : null}</Card>
