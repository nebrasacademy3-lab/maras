...
import { EmailChangePanel } from "@/src/components/EmailChangePanel";

export default function Profile() {
  const { user, refresh } = useAuth(); const { colors } = useTheme(); const catalog = useQuery({ queryKey: ["catalog"], queryFn: () => api<Catalog>("/api/mobile/catalog") }); const [form, setForm] = useState({ fullName: user?.fullName || "", phone: user?.phone || "", universitySlug: user?.universitySlug || "", specialty: user?.specialty || "", academicLevel: user?.academicLevel || "" }); const [programs, setPrograms] = useState<{ name: string; degree: string }[]>([]); const [saving, setSaving] = useState(false); const [message, setMessage] = useState("");
