...
function openNotificationAction(actionUrl?: string | null) {
  const destination = resolveNotificationRoute(actionUrl);
  if (destination) router.push(destination);
}

export default function Notifications() {
  const { user, authReady, restoration } = useAuthRestoreState({ title: "الإشعارات", loadingLabel: "جارٍ استعادة إشعاراتك...", back: true }); const { colors } = useTheme(); const client = useQueryClient(); const query = useQuery({ queryKey: ["notifications", user?.id], queryFn: () => api<{ notifications: Notice[]; unreadCount?: number }>("/api/mobile/notifications"), enabled: authReady && Boolean(user) });
  useEffect(() => {
  ...
  if (!user) return <Screen><AppHeader title="الإشعارات" back /><EmptyState title="سجّل الدخول" text="إشعارات الحساب والمواد والطلبات خاصة بك." action={<AppButton title="تسجيل الدخول" onPress={() => router.push("/(auth)/login?return_to=%2Fnotifications")} />} /></Screen>;
  if (query.isLoading) return <Screen><LoadingState /></Screen>;
  if (query.isError) return <Screen><AppHeader title="الإشعارات" back /><ErrorState title="تعذر تحميل الإشعارات" text="تحقق من اتصالك ثم حاول مرة أخرى." onRetry={() => void query.refetch()} /></Screen>;
