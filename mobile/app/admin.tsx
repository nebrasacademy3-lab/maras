...
const uiLabels: Record<string, string> = {
  student: "طالب", supervisor: "مشرف", admin: "مدير", public: "الجميع",
  active: "نشط", suspended: "موقوف", published: "منشور", hidden: "مخفي", draft: "مسودة",
  new: "جديد", assigned: "مسند", reviewing: "قيد المراجعة", planned: "مخطط", producing: "قيد الإنتاج", available: "متاح", declined: "متعذر",
  open: "مفتوحة", waiting: "بانتظار الطالب", resolved: "محلولة", closed: "مغلقة",
  paid: "مدفوع", pending: "معلّق", initiated: "بدأ الدفع", failed: "فشل", captured: "مكتمل", refunded: "مسترد",
  percent: "نسبة مئوية", fixed: "مبلغ ثابت", rejected: "مرفوض",
  inbox: "مركز الإشعارات", banner: "شريط علوي", modal: "نافذة", all: "كل القنوات",
  in_app: "داخل مراس", email: "البريد", whatsapp: "واتساب", true: "مفعّل", false: "متوقف",
};
function uiLabel(value: string | null | undefined) { return value ? uiLabels[value] || value : "—"; }
...
export default function Admin() {
  ...
  return <Screen keyboard>
    ...
    <View style={styles.tabs}>{tabs.map((item) => <Pressable key={item.key} onPress={() => setTab(item.key)} style={[styles.tab, { backgroundColor: tab === item.key ? colors.primary : colors.surface, borderColor: tab === item.key ? colors.primary : colors.border }]}><Ionicons name={item.icon} size={18} color={tab === item.key ? "#FFF" : colors.primary} /><Text style={{ color: tab === item.key ? "#FFF" : colors.text, fontSize: 9, fontWeight: "900" }}>{item.label}</Text></Pressable>)}</View>
...
function AdminRequestStatus({ status, colors }: { status: string; colors: Colors }) { const label = ({ new: "جديد", assigned: "مسند", reviewing: "قيد المراجعة", planned: "مخطط له", producing: "قيد الإنتاج", available: "متاح", declined: "متعذر" } as Record<string, string>)[status] || status; return <Text style={{ color: status === "available" ? colors.success : colors.primary, fontSize: 9, fontWeight: "900" }}>{label}</Text>; }
...
function Communication({ data, colors, mutate, onDelete }: { data: AdminData; colors: Colors; mutate: Mutate; onDelete: DeleteEntity }) {
  const [settings, setSettings] = useState({
    ...
    home_hero_kicker: data.settings.home_hero_kicker || "منصة تعليم جامعي سعودية",
    home_hero_title: data.settings.home_hero_title || "شرح جامعتك،",
    home_hero_highlight: data.settings.home_hero_highlight || "في مكان واحد.",
    home_hero_subtitle: data.settings.home_hero_subtitle || "اختر جامعتك وتخصصك، استعرض محتوى المادة، وابدأ التعلّم بخطوات واضحة حتى الاختبار.",
    mobile_welcome_title: data.settings.mobile_welcome_title || "كل شيء أوضح، أرتب، وأقرب لك",
    mobile_welcome_subtitle: data.settings.mobile_welcome_subtitle || "تجربة عربية من اليمين لليسار، مواد منظّمة، بحث أسهل، ومساعد مراس معك في كل خطوة.",
    assistant_enabled: data.settings.assistant_enabled || "true",
    course_requests_enabled: data.settings.course_requests_enabled || "true",
    guest_browsing_enabled: data.settings.guest_browsing_enabled || "true",
    student_registration_enabled: data.settings.student_registration_enabled || "true",
    payments_enabled: data.settings.payments_enabled || "true",
  });
  ...
  const sendNotice = async () => {
    const ok = await mutate({ action: "createNotification", ...notice, userEmail: notice.userEmail || null, startsAt: notice.startsAt || null, expiresAt: notice.expiresAt || null }, "تم نشر الإعلان والإشعار وتحديث الأجهزة");
    if (ok) setNotice((current) => ({ ...current, title: "", body: "", userEmail: "", startsAt: "", expiresAt: "" }));
  };
  return <>
    <SectionTitle title="قنوات التواصل والواجهة" subtitle="تنعكس القيم مباشرة على الويب والتطبيق وصفحات الدعم والتذييل" />
    <Card>
      ...
      <Text style={[styles.formLabel, { color: colors.text }]}>محتوى الواجهة الرئيسية</Text>
      <Field label="العبارة أعلى العنوان" value={settings.home_hero_kicker} onChangeText={(value) => update("home_hero_kicker", value)} />
      <Field label="عنوان الرئيسية" value={settings.home_hero_title} onChangeText={(value) => update("home_hero_title", value)} />
      <Field label="الجزء البارز من العنوان" value={settings.home_hero_highlight} onChangeText={(value) => update("home_hero_highlight", value)} />
      <TextInput value={settings.home_hero_subtitle} onChangeText={(value) => update("home_hero_subtitle", value)} placeholder="وصف الصفحة الرئيسية" placeholderTextColor={colors.textSoft} multiline textAlign="right" style={[styles.area, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} />
      <Text style={[styles.formLabel, { color: colors.text }]}>ترحيب التطبيق</Text>
      <Field label="عنوان شاشة الترحيب" value={settings.mobile_welcome_title} onChangeText={(value) => update("mobile_welcome_title", value)} />
      <TextInput value={settings.mobile_welcome_subtitle} onChangeText={(value) => update("mobile_welcome_subtitle", value)} placeholder="وصف شاشة الترحيب" placeholderTextColor={colors.textSoft} multiline textAlign="right" style={[styles.area, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} />
      <Text style={[styles.formLabel, { color: colors.text }]}>الخصائص العامة</Text>
      <View style={styles.featureToggleList}>
        {[
          ["assistant_enabled", "مساعد مراس", "إظهار المساعد والسماح باستخدامه"],
          ["course_requests_enabled", "طلبات المواد", "السماح للطلاب بإرسال طلب مادة"],
          ["guest_browsing_enabled", "التصفح كضيف", "إتاحة دخول الكتالوج قبل تسجيل الحساب"],
          ["student_registration_enabled", "إنشاء حسابات الطلاب", "فتح أو إيقاف التسجيل العام للحسابات الجديدة"],
          ["payments_enabled", "الدفع والاشتراكات الجديدة", "التحكم في بدء عمليات الدفع الجديدة من المنصة"],
        ].map(([key, label, description]) => { const enabled = settings[key as keyof typeof settings] === "true"; return <Pressable key={key} onPress={() => update(key as keyof typeof settings, enabled ? "false" : "true")} style={[styles.featureToggle, { backgroundColor: enabled ? `${colors.primary}12` : colors.surfaceAlt, borderColor: enabled ? colors.primary : colors.border }]}><View style={[styles.switchDot, { backgroundColor: enabled ? colors.primary : colors.textSoft }]}><Ionicons name={enabled ? "checkmark" : "close"} size={14} color="#FFF" /></View><View style={styles.toggleCopy}><Text style={[styles.toggleTitle, { color: colors.text }]}>{label}</Text><Text style={[styles.toggleText, { color: colors.textSoft }]}>{description} · {enabled ? "مفعّل" : "متوقف"}</Text></View></Pressable>; })}
      </View>
      <Text style={[styles.dataMeta, { color: colors.textSoft }]}>تنبيه عام مختصر يظهر في الواجهات التي تدعمه</Text>
      <TextInput value={settings.announcement} onChangeText={(value) => update("announcement", value)} placeholder="مثال: أضيفت مواد جديدة هذا الأسبوع" placeholderTextColor={colors.textSoft} multiline textAlign="right" style={[styles.area, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} />
      <Text style={[styles.dataMeta, { color: colors.textSoft }]}>الشبكات الاجتماعية</Text>
      {[["social_x", "X"], ["social_instagram", "Instagram"], ["social_tiktok", "TikTok"], ["social_youtube", "YouTube"], ["social_telegram", "Telegram"], ["social_linkedin", "LinkedIn"], ["social_facebook", "Facebook"], ["social_snapchat", "Snapchat"], ["social_threads", "Threads"]].map(([key, label]) => <Field key={key} label={`رابط ${label}`} value={settings[key as keyof typeof settings]} onChangeText={(value) => update(key as keyof typeof settings, value)} autoCapitalize="none" />)}
      <AppButton title="حفظ التواصل والواجهة" icon="save-outline" onPress={() => mutate({ action: "saveSettings", values: settings }, "تم تحديث القيم في الويب والتطبيق")} />
    </Card>

    <SectionTitle title="مركز الإعلانات والإشعارات" subtitle="تحكم بالجمهور ومكان الظهور وإشعارات الجهاز والجدولة من مكان واحد" />
    <Card>
      <Text style={[styles.formLabel, { color: colors.text }]}>الجمهور</Text>
      <ChoiceRow values={["student", "public", "supervisor", "admin"]} selected={notice.audience} onSelect={(value) => setNotice({ ...notice, audience: value })} colors={colors} />
      <Field label="بريد مستخدم محدد — اختياري" value={notice.userEmail} onChangeText={(value) => setNotice({ ...notice, userEmail: value })} keyboardType="email-address" autoCapitalize="none" />
      <Field label="عنوان الإعلان أو الإشعار" value={notice.title} onChangeText={(value) => setNotice({ ...notice, title: value })} />
      <TextInput value={notice.body} onChangeText={(value) => setNotice({ ...notice, body: value })} placeholder="اكتب الرسالة التي ستصل للطالب أو تظهر في الواجهة" placeholderTextColor={colors.textSoft} multiline textAlign="right" style={[styles.area, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} />
      <Text style={[styles.formLabel, { color: colors.text }]}>طريقة الظهور</Text>
      ...
      <Field label="الرابط الداخلي — اختياري" value={notice.actionUrl} onChangeText={(value) => setNotice({ ...notice, actionUrl: value })} autoCapitalize="none" />
      <Field label="نص زر الإجراء" value={notice.actionLabel} onChangeText={(value) => setNotice({ ...notice, actionLabel: value })} />
      <View style={styles.twoColumnFields}>
        <View style={styles.halfField}><Field label="يبدأ في — اختياري" value={notice.startsAt} onChangeText={(value) => setNotice({ ...notice, startsAt: value })} placeholder="2026-08-30T09:00:00+03:00" autoCapitalize="none" /></View>
        <View style={styles.halfField}><Field label="ينتهي في — اختياري" value={notice.expiresAt} onChangeText={(value) => setNotice({ ...notice, expiresAt: value })} placeholder="2026-09-02T23:59:00+03:00" autoCapitalize="none" /></View>
      </View>
      <View style={styles.toggleGrid}>
        <Pressable onPress={() => setNotice({ ...notice, pushEnabled: !notice.pushEnabled })} style={[styles.toggleCard, { backgroundColor: notice.pushEnabled ? `${colors.primary}14` : colors.surfaceAlt, borderColor: notice.pushEnabled ? colors.primary : colors.border }]}>
          <Ionicons name={notice.pushEnabled ? "notifications" : "notifications-off-outline"} size={20} color={notice.pushEnabled ? colors.primary : colors.textSoft} />
          <View style={styles.toggleCopy}><Text style={[styles.toggleTitle, { color: colors.text }]}>إشعار للجهاز</Text><Text style={[styles.toggleText, { color: colors.textSoft }]}>{notice.pushEnabled ? "مفعل" : "متوقف"}</Text></View>
        </Pressable>
        <Pressable onPress={() => setNotice({ ...notice, dismissible: !notice.dismissible })} style={[styles.toggleCard, { backgroundColor: notice.dismissible ? `${colors.primary}14` : colors.surfaceAlt, borderColor: notice.dismissible ? colors.primary : colors.border }]}>
          <Ionicons name={notice.dismissible ? "close-circle-outline" : "lock-closed-outline"} size={20} color={notice.dismissible ? colors.primary : colors.textSoft} />
          <View style={styles.toggleCopy}><Text style={[styles.toggleTitle, { color: colors.text }]}>السماح بالإخفاء</Text><Text style={[styles.toggleText, { color: colors.textSoft }]}>{notice.dismissible ? "نعم" : "لا"}</Text></View>
        </Pressable>
      </View>
      <AppButton title="نشر الآن" icon="send-outline" disabled={notice.title.trim().length < 3 || notice.body.trim().length < 3} onPress={() => void sendNotice()} />
    </Card>

    <SectionTitle title="آخر الإعلانات والإشعارات" subtitle="يمكن حذف أي حملة مع إبقاء سجل التدقيق" />
    <View>{data.notifications.slice(0, 40).map((row) => <Card key={row.id} style={styles.dataCard}>
      <View style={styles.dataHead}><Text style={[styles.role, { color: colors.primary }]}>{uiLabel(row.presentation || "inbox")}</Text><Text style={[styles.dataTitle, { color: colors.text }]}>{row.title}</Text></View>
      <Text style={[styles.ticketBody, { color: colors.text }]}>{row.body}</Text>
      <Text style={[styles.dataMeta, { color: colors.textSoft }]}>{uiLabel(row.audience)} · {row.userEmail || "جمهور عام"} · {row.pushEnabled === false ? "بدون إشعار جهاز" : "إشعار الجهاز مفعّل"}</Text>
      {row.actionUrl ? <Text style={[styles.dataMeta, { color: colors.primary }]}>{row.actionLabel || "فتح"} ← {row.actionUrl}</Text> : null}
      {(row.startsAt || row.expiresAt) ? <Text style={[styles.dataMeta, { color: colors.textSoft }]}>من {row.startsAt || "الآن"} إلى {row.expiresAt || "بدون انتهاء"}</Text> : null}
      <AppButton full={false} title="حذف الإشعار" variant="danger" onPress={() => onDelete("notification", row.id, row.title, "سيُحذف الإشعار من صناديق المستلمين وواجهات الإعلان، مع إبقاء سجل التدقيق محفوظًا.")} />
    </Card>)}</View>
  </>;
}
function ChoiceRow({ values, selected, onSelect, colors }: { values: string[]; selected: string; onSelect: (value: string) => void; colors: Colors }) {
  return <View style={styles.choices}>{values.map((value) => <Pressable key={value} onPress={() => onSelect(value)} style={[styles.choice, { backgroundColor: selected === value ? colors.primary : colors.surfaceAlt }]}><Text style={{ color: selected === value ? "#FFF" : colors.text, fontSize: 9, fontWeight: "800" }}>{uiLabel(value)}</Text></Pressable>)}</View>;
}

const styles = StyleSheet.create({
  tabs: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 8, paddingBottom: 14 }, tab: { flexBasis: "30%", flexGrow: 1, minHeight: 58, borderWidth: 1, borderRadius: 16, alignItems: "center", justifyContent: "center", gap: 5 }, message: { fontSize: 10, textAlign: "center", marginBottom: 8, fontWeight: "800" },
  ...
  dataCard: { marginBottom: 9 }, coverPreview: { width: "100%", height: 150, borderRadius: 16, marginTop: 10, backgroundColor: "#CBD5E1" }, requestFiles: { marginTop: 6, gap: 5 }, requestFile: { flexDirection: "row-reverse", alignItems: "center", gap: 6, paddingVertical: 6 }, dataHead: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 8 }, dataTitle: { flex: 1, fontSize: 13, fontWeight: "900", textAlign: "right" }, role: { fontSize: 9, fontWeight: "900" }, dataMeta: { fontSize: 8, textAlign: "right", marginTop: 5 }, actionRow: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 7, marginTop: 12 }, statuses: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 6, marginTop: 11 }, status: { minHeight: 34, borderRadius: 11, paddingHorizontal: 10, alignItems: "center", justifyContent: "center" },
  ticketBody: { fontSize: 10, lineHeight: 18, textAlign: "right", marginTop: 9 }, adminThread: { gap: 7, padding: 10, borderRadius: 13, marginTop: 10 }, adminBubble: { padding: 10, borderRadius: 12 }, adminBubbleLabel: { color: "#FFF", fontSize: 9, fontWeight: "900", textAlign: "right" }, adminBubbleText: { color: "#FFF", fontSize: 10, lineHeight: 18, textAlign: "right", marginTop: 4 }, adminAttachment: { flexDirection: "row-reverse", alignItems: "center", gap: 5, paddingTop: 6 }, area: { minHeight: 90, borderWidth: 1, borderRadius: 14, padding: 11, marginTop: 11, marginBottom: 12, textAlignVertical: "top", writingDirection: "rtl" }, choices: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 7, paddingBottom: 12 }, choice: { minHeight: 36, borderRadius: 11, paddingHorizontal: 13, alignItems: "center", justifyContent: "center" }, spacer: { height: 10 }, amount: { fontSize: 15, fontWeight: "900", textAlign: "right", marginTop: 10 }, stars: { color: "#F7A810", letterSpacing: 2 }, formLabel: { fontSize: 11, fontWeight: "900", textAlign: "right", marginBottom: 8, writingDirection: "rtl" }, twoColumnFields: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 8 }, halfField: { flexBasis: "48%", flexGrow: 1 }, toggleGrid: { flexDirection: "row-reverse", gap: 8, marginBottom: 14 }, toggleCard: { flex: 1, minHeight: 66, borderWidth: 1, borderRadius: 15, padding: 10, flexDirection: "row-reverse", alignItems: "center", gap: 8 }, toggleCopy: { flex: 1, alignItems: "flex-end" }, toggleTitle: { fontSize: 10, fontWeight: "900", textAlign: "right" }, toggleText: { fontSize: 8, marginTop: 3, textAlign: "right" }, featureToggleList: { gap: 8, marginBottom: 14 }, featureToggle: { minHeight: 66, borderWidth: 1, borderRadius: 15, padding: 10, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, switchDot: { width: 32, height: 32, borderRadius: 12, alignItems: "center", justifyContent: "center" },
});
