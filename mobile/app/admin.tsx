...
function contentSlug(kind: string, title: string, nonce: string) {
  const suffix = nonce.replace(/-/g, "").slice(0, 20);
  const stem = `${kind}-${asciiSlug(title)}`.slice(0, 59).replace(/-+$/g, "");
  return `${stem}-${suffix}`;
}
...
function CatalogAdmin({ data, colors, mutate, refresh, onDelete, section }: { section:string; data: AdminData; colors: Colors; mutate: Mutate; refresh: () => Promise<void>; onDelete: DeleteEntity }) {
  ...
  const saveInstitution = async () => {
    const institutionKey = institution.slug.trim() || contentSlug("institution", institution.name, operationKey);
    const saved = await mutate({ action: "saveInstitution", intent: "create", ...institution, slug: institutionKey, status: "published", featured: false }, "تم حفظ الجهة التعليمية");
    if (!saved) return;
    setOperationKey(randomUUID());
  ...
  const saveCourse = async () => {
    ...
    const courseKey = course.slug.trim() || contentSlug("course", `${specialtyName}-${course.title}`, operationKey);
    const saved = await mutate({ action: "saveCourse", intent: "create", ...course, slug: courseKey, price: Number(course.price), oldPrice: Number(course.oldPrice), status: "draft", featured: false, coverTheme: "blue-violet" }, "تم حفظ المادة كمسودة");
    if (!saved) return;
    setOperationKey(randomUUID());
...
function MobileBundleAdmin({ colors, institutions }:{ colors:Colors;institutions:AdminData["institutions"] }) {
  ...
  const save=async()=>{
    if(form.title.trim().length<2||(editingId&&form.slug.trim().length<2)||form.courseSlugs.length<2){setFeedback("أدخل اسم الباقة واختر مادتين على الأقل؛ المعرّف تلقائي عند تركه فارغًا.");return;}
  ...
  return <>
    ...
    <Card style={styles.dataCard}>
      ...
      <Field label="اسم الباقة" value={form.title} onChangeText={(value)=>setForm({...form,title:value})}/>
      <Field label="المعرّف (اختياري عند الإنشاء)" value={form.slug} onChangeText={(value)=>setForm({...form,slug:value.toLowerCase().replace(/[^a-z0-9._-]/g,"-")})} autoCapitalize="none"/>
