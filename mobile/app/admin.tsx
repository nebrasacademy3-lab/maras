...
type AdminData = {
  ...
  tickets: { id: number; ticketNumber: string; title: string; message: string; userEmail: string | null; contactChannel?: string; status: string; createdAt: string; replies?: { id: number; body: string; authorRole?: string; internal?: boolean; createdAt: string; files?: { id: number; originalName: string }[] }[] }[];
...
function Support({ rows, colors, mutate }: { rows: AdminData["tickets"]; colors: Colors; mutate: Mutate }) {
  const [replies, setReplies] = useState<Record<number, string>>({}); const [files, setFiles] = useState<Record<number, DocumentPicker.DocumentPickerAsset | null>>({});
  const send = async (row: AdminData["tickets"][number]) => { const file = files[row.id]; const text = replies[row.id] || ""; if (file) { const form = new FormData(); form.append("ticketId", String(row.id)); form.append("message", text); form.append("files", { uri: file.uri, name: file.name, type: file.mimeType || "application/octet-stream" } as unknown as Blob); try { await api("/api/support", { method: "POST", body: form }); setFiles({ ...files, [row.id]: null }); setReplies({ ...replies, [row.id]: "" }); await mutate({ action: "updateTicket", id: row.id, status: row.status, reply: "" }, "تم إرسال الرسالة والمرفق"); } catch { /* The shared mutation layer will surface later refresh errors. */ } } else await mutate({ action: "updateTicket", id: row.id, status: row.status, reply: text, internal: false }, "تم إرسال الرد"); };
  ...
  return <><SectionTitle title="محادثات الدعم" subtitle="تابع thread الطالب، أرسل ملفات، ولا تُغلق المحادثة إلا من الإدارة" />{rows.map((row) => <Card key={row.id} style={styles.dataCard}><View style={styles.dataHead}><Text style={[styles.role, { color: colors.primary }]}>{row.status}</Text><Text style={[styles.dataTitle, { color: colors.text }]}>{row.title}</Text></View><Text style={[styles.dataMeta, { color: colors.textSoft }]}>{row.ticketNumber} · {row.userEmail || "—"} · قناة {row.contactChannel || "in_app"}</Text><View style={[styles.adminThread, { backgroundColor: colors.surfaceAlt }]}><View style={[styles.adminBubble, { backgroundColor: colors.primary }]}><Text style={styles.adminBubbleLabel}>الطالب</Text><Text style={styles.adminBubbleText}>{row.message}</Text></View>{row.replies?.filter((reply) => !reply.internal && (reply.body || reply.files?.length)).map((reply) => <View key={reply.id} style={[styles.adminBubble, { backgroundColor: reply.authorRole === "student" ? colors.primary : colors.surface }]}><Text style={[styles.adminBubbleLabel, { color: reply.authorRole === "student" ? "#FFF" : colors.primary }]}>{reply.authorRole === "student" ? "الطالب" : "فريق مراس"}</Text>{reply.body ? <Text style={[styles.adminBubbleText, { color: reply.authorRole === "student" ? "#FFF" : colors.text }]}>{reply.body}</Text> : null}{reply.files?.map((file) => <Text key={file.id} style={{ color: reply.authorRole === "student" ? "#FFF" : colors.primary, fontSize: 9, marginTop: 5 }}>📎 {file.originalName}</Text>)}</View>)}</View><TextInput value={replies[row.id] || ""} onChangeText={(value) => setReplies({ ...replies, [row.id]: value })} placeholder="اكتب ردك للطالب..." placeholderTextColor={colors.textSoft} style={[styles.area, { color: colors.text, backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} textAlign="right" multiline /><View style={styles.actionRow}><AppButton full={false} title={files[row.id] ? `ملف: ${files[row.id]?.name}` : "إرفاق ملف"} variant="ghost" onPress={() => void pick(row.id)} /><AppButton full={false} title="إرسال الرد" variant="soft" disabled={!replies[row.id]?.trim() && !files[row.id]} onPress={() => void send(row)} /></View><View style={styles.actionRow}><ChoiceRow values={["open", "waiting", "resolved", "closed"]} selected={row.status} onSelect={(status) => mutate({ action: "updateTicket", id: row.id, status, reply: "" }, "تم تحديث حالة المحادثة")} colors={colors} /></View></Card>)}</>;
}
...
const styles = StyleSheet.create({
  ...
  ticketBody: { fontSize: 10, lineHeight: 18, textAlign: "right", marginTop: 9 }, adminThread: { gap: 7, padding: 10, borderRadius: 13, marginTop: 10 }, adminBubble: { padding: 10, borderRadius: 12 }, adminBubbleLabel: { color: "#FFF", fontSize: 9, fontWeight: "900", textAlign: "right" }, adminBubbleText: { color: "#FFF", fontSize: 10, lineHeight: 18, textAlign: "right", marginTop: 4 }, area: { minHeight: 90, borderWidth: 1, borderRadius: 14, padding: 11, marginTop: 11, marginBottom: 12, textAlignVertical: "top", writingDirection: "rtl" }, choices: { flexDirection: "row-reverse", gap: 7, paddingBottom: 12 }, choice: { minHeight: 36, borderRadius: 11, paddingHorizontal: 13, alignItems: "center", justifyContent: "center" }, spacer: { height: 10 }, amount: { fontSize: 15, fontWeight: "900", textAlign: "right", marginTop: 10 }, stars: { color: "#F7A810", letterSpacing: 2 },
});
