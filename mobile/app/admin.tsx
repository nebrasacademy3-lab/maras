import { AdminCatalogPager, useCatalogPage } from "@/src/components/AdminCatalogPager";
...
function CatalogAdmin({ data, colors, mutate, refresh, onDelete }: { data: AdminData; colors: Colors; mutate: Mutate; refresh: () => Promise<void>; onDelete: DeleteEntity }) {
  const { t } = useLanguage();
  const institutionsPage = useCatalogPage(data.institutions, row => `${row.name} ${row.slug} ${row.region}`);
  const specialtiesPage = useCatalogPage(data.specialties, row => `${row.name} ${row.slug}`);
  const coursesPage = useCatalogPage(data.courses, row => `${row.title} ${row.slug} ${row.university}`);
  const unitsPage = useCatalogPage(data.units, row => `${row.title} ${row.courseSlug}`);
  const lessonsPage = useCatalogPage(data.lessons, row => `${row.title} ${row.courseSlug}`);
  ...
  const [course, setCourse] = useState({ slug: "", institutionSlug: "", specialtySlug: "", title: "", titleEn: "", code: "", description: "", coverImageUrl: "", price: "", oldPrice: "", accessLabel: "90 يومًا", accessDurationDays: "90", audienceScope: "specialty" });
  ...
  const saveCourse = async () => {
    ...
    const saved = await mutate({ action: "saveCourse", ...course, accessDurationDays: Number(course.accessDurationDays), slug: courseKey, price: Number(course.price), oldPrice: Number(course.oldPrice), status: "draft", featured: false, coverTheme: "blue-violet" }, "تم حفظ المادة كمسودة");
