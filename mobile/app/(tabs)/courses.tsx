...
export default function Courses() {
  ...
  const universities = useMemo(() => {
    ...
    (catalog.data?.courses || []).forEach((course) => { if (course.universitySlug) values.set(course.universitySlug, course.university); });
    return [...values.entries()].sort((a, b) => a[1].localeCompare(b[1], "ar"));
  }, [catalog.data]);
  const specialties = useMemo(() => [...new Set((catalog.data?.courses || []).map((course) => course.specialty))].sort((a, b) => a.localeCompare(b, "ar")), [catalog.data]);
  ...
  return <Screen><AppHeader title="المواد والشروحات" subtitle={`${rows.length} مادة ظاهرة`} /><SearchBox value={query} onChangeText={setQuery} placeholder="اسم المادة، الرمز، الجامعة أو التخصص" />
    ...
    <Text style={[styles.label, { color: colors.textSoft }]}>الجامعة</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.picker}>{[ [ALL_UNIVERSITIES, ALL_UNIVERSITIES] as const, ...universities].map(([key, label]) => <Pressable key={key} onPress={() => chooseUniversity(key)} style={[styles.pickerItem, { backgroundColor: activeUniversity === key ? colors.surfaceAlt : colors.surface, borderColor: activeUniversity === key ? colors.primary : colors.border }]}><Text numberOfLines={1} style={{ color: activeUniversity === key ? colors.primary : colors.textSoft, fontSize: 9, fontWeight: "800" }}>{label}</Text></Pressable>)}</ScrollView>
    <Text style={[styles.label, { color: colors.textSoft }]}>التخصص</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.picker}>{[ALL_SPECIALTIES, ...specialties].map((item) => <Pressable key={item} onPress={() => chooseSpecialty(item)} style={[styles.pickerItem, { backgroundColor: activeSpecialty === item ? colors.surfaceAlt : colors.surface, borderColor: activeSpecialty === item ? colors.primary : colors.border }]}><Text numberOfLines={1} style={{ color: activeSpecialty === item ? colors.primary : colors.textSoft, fontSize: 9, fontWeight: "800" }}>{item}</Text></Pressable>)}</ScrollView>
    <View style={styles.list}>{rows.map((course) => <CourseCard compact key={course.slug} course={course} />)}</View>{!rows.length && <Text style={[styles.empty, { color: colors.textSoft }]}>لا توجد مواد بهذه الفلاتر. اختر عرض الكل أو جرّب جامعة وتخصصًا آخر.</Text>}
  </Screen>;
}

const styles = StyleSheet.create({
  context: { borderRadius: 17, padding: 13, marginTop: 12 }, contextTitle: { textAlign: "right", writingDirection: "rtl", fontSize: 13, fontWeight: "900" }, contextText: { marginTop: 4, textAlign: "right", writingDirection: "rtl", fontSize: 10 }, contextButtons: { flexDirection: "row-reverse", gap: 8, marginTop: 10 }, contextButton: { flex: 1, minHeight: 36, borderWidth: 1, borderRadius: 11, alignItems: "center", justifyContent: "center" }, filters: { gap: 8, paddingVertical: 14 }, filter: { minWidth: 76, minHeight: 38, paddingHorizontal: 14, borderWidth: 1, borderRadius: 13, alignItems: "center", justifyContent: "center" }, label: { marginTop: 4, marginBottom: 7, textAlign: "right", writingDirection: "rtl", fontSize: 10, fontWeight: "800" }, picker: { gap: 7, paddingBottom: 6 }, pickerItem: { maxWidth: 210, minHeight: 34, paddingHorizontal: 11, borderWidth: 1, borderRadius: 11, alignItems: "center", justifyContent: "center" }, list: { marginTop: 14 }, empty: { padding: 24, textAlign: "center", writingDirection: "rtl", fontSize: 11, lineHeight: 20 },
});
