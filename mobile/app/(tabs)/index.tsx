...
import { StudyLaunchpad } from "@/src/components/study-launchpad";
...
export default function Home() {
  ...
  const first = dashboard.data?.owned.find((course) => course.accessState === "active" && course.progress < 100) || dashboard.data?.owned.find((course) => course.accessState === "active");
  ...
  return <Screen>
    ...
    {catalog.isError && catalog.data ? <Pressable accessibilityRole="button" accessibilityLabel="إعادة تحديث كتالوج المواد" accessibilityState={{ busy: catalog.isFetching }} onPress={() => void catalog.refetch()} style={({ pressed }) => [styles.catalogWarning, { backgroundColor: colors.surfaceAlt, borderColor: colors.border, flexDirection: rowDirection, opacity: pressed ? .78 : 1 }]}><Ionicons name="cloud-offline-outline" size={19} color={colors.warning} /><Text style={[styles.catalogWarningText, { color: colors.text }]}>نعرض المواد المحفوظة. اضغط لتحديث الكتالوج.</Text><Ionicons name="refresh-outline" size={18} color={colors.primary} /></Pressable> : null}

    <FadeIn>
      <LinearGradient colors={[colors.hero, colors.heroEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.hero, wide && styles.heroWide, width < 380 && { padding: 18, borderRadius: 26 }, { direction }]}>
        ...
        <View style={[styles.heroLayout, wide && { flexDirection: rowDirection }]}>
          <View style={styles.heroCopyColumn}>
            ...
            <View style={[styles.heroActions, wide && { flexDirection: rowDirection }]}>
              ...
              <Pressable accessibilityRole="button" accessibilityLabel="تصفح الجامعات والكليات" onPress={() => router.push("/(tabs)/universities")} style={({ pressed }) => [styles.secondaryAction, { borderColor: `${colors.onPrimary}66`, flexDirection: rowDirection, opacity: pressed ? .7 : 1 }]}><Text style={[styles.secondaryActionText, { color: colors.onPrimary }]}>تصفح الجامعات</Text><Ionicons name="school-outline" size={18} color={colors.onPrimary} /></Pressable>
            </View>
          </View>

          {first ? <View style={[styles.studyCard, wide && styles.studyCardWide, { backgroundColor: colors.surface, borderColor: colors.border, boxShadow: "0 12px 30px rgba(10,28,63,.10)" }]}>
            ...
            <Pressable accessibilityRole="button" onPress={() => router.push({ pathname: "/learn/[slug]", params: { slug: first.slug } })} style={({ pressed }) => [styles.resume, { backgroundColor: colors.action, flexDirection: rowDirection, opacity: pressed ? .8 : 1 }]}><Text style={[styles.resumeText, { color: colors.onPrimary }]}>أكمل التعلّم</Text><Ionicons name="play" size={17} color={colors.onPrimary} /></Pressable>
          </View> : <View style={[styles.studyCard, wide && styles.studyCardWide, { backgroundColor: colors.surface, borderColor: colors.border, boxShadow: "0 12px 30px rgba(10,28,63,.10)" }]}>
...
const styles = StyleSheet.create({
  announcement: { minHeight: 48, borderRadius: 15, padding: 13, alignItems: "center", gap: 10, marginBottom: 12 }, announcementText: { flex: 1, fontSize: 12, lineHeight: 20 },
  catalogWarning: { minHeight: 52, borderWidth: 1, borderRadius: 15, paddingHorizontal: 12, paddingVertical: 10, alignItems: "center", gap: 9, marginBottom: 12 }, catalogWarningText: { flex: 1, fontSize: 12, lineHeight: 20 },
  hero: { borderRadius: 28, padding: 25, overflow: "hidden", marginTop: 8 }, heroWide: { padding: 38 }, heroLayout: { gap: 32 }, heroCopyColumn: { flex: 1, minWidth: 0 },
  ...
  kicker: { alignSelf: "flex-start", maxWidth: "100%", alignItems: "center", gap: 9, marginBottom: 18, paddingVertical: 10, paddingHorizontal: 13, borderRadius: 999, borderWidth: 1 }, kickerDot: { width: 6, height: 6, borderRadius: 5 }, kickerText: { fontSize: 12, fontWeight: "900", flexShrink: 1, lineHeight: 22 },
  heroTitle: { fontSize: 40, lineHeight: 55, fontWeight: "900", letterSpacing: -.7 }, heroTitleWide: { fontSize: 54, lineHeight: 72 }, heroCopy: { fontSize: 13, lineHeight: 25, marginTop: 16, maxWidth: 490, opacity: .9 }, heroActions: { flexWrap: "wrap", gap: 10, marginTop: 27 },
  ...
  quickGrid: { flexWrap: "wrap", justifyContent: "space-between", rowGap: 12 }, quick: { minHeight: 172, padding: 16 }, quickIcon: { width: 43, height: 43, borderRadius: 15, alignItems: "center", justifyContent: "center" }, quickTitle: { fontSize: 14, lineHeight: 23, fontWeight: "900", marginTop: 13 }, quickCopy: { fontSize: 12, lineHeight: 19, marginTop: 3 }, quickArrow: { alignSelf: "flex-end", marginTop: 8 },
