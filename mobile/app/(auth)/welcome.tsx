...
export default function Welcome() {
  ...
  return <Screen showFooter={false}>
    <LinearGradient colors={["#04143D", "#0D58D7", "#7640EC"]} start={{ x: .1, y: 0 }} end={{ x: .9, y: 1 }} style={styles.hero}>
      ...
      <FadeIn delay={160} style={styles.benefits}>{benefits.map((item) => <View key={item.title} style={styles.benefit}><Ionicons name={item.icon} size={19} color="#FFF" /><Text style={styles.benefitTitle}>{item.title}</Text><Text style={styles.benefitText}>{item.text}</Text></View>)}</FadeIn>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  orbLarge: { position: "absolute", width: 430, height: 430, borderRadius: 215, backgroundColor: "rgba(255,255,255,.07)", top: -240, right: -150 },
  orbSmall: { position: "absolute", width: 160, height: 160, borderRadius: 80, borderWidth: 1, borderColor: "rgba(255,255,255,.12)", bottom: -60, left: -50 },
  kicker: { minHeight: 34, borderRadius: 17, paddingHorizontal: 13, marginTop: 22, backgroundColor: "rgba(255,255,255,.12)", flexDirection: "row-reverse", alignItems: "center", gap: 6 },
  ...
  benefits: { width: "100%", flexDirection: "row-reverse", gap: 8, marginTop: 24 },
