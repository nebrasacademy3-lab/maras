...
export default function Welcome() {
  const { colors } = useTheme(); const controls = usePlatformControls(); const registrationEnabled = controls.enabled("registration");
  return <Screen><LinearGradient colors={["#061B49", "#174EC2", "#7B3FF2"]} style={styles.hero}><View style={styles.orb} /><FadeIn><BrandMark size={104} whiteTile /></FadeIn><FadeIn delay={100}><Text style={styles.kicker}>منصتك الجامعية الذكية</Text><Text style={styles.title}>تعلّم مقررات جامعتك{`\n`}بطريقة أوضح وأعمق</Text><Text style={styles.copy}>جامعات وتخصصات ومواد ودروس منظمة، ومساعد مراس معك في كل خطوة.</Text></FadeIn></LinearGradient><FadeIn delay={180} style={styles.actions}><BrandLogo width={150} /><AppButton title="تسجيل الدخول" icon="log-in-outline" onPress={() => router.push("/(auth)/login")} />{registrationEnabled ? <AppButton title="إنشاء حساب طالب" icon="person-add-outline" variant="soft" disabled={controls.loading} onPress={() => router.push("/(auth)/register")} /> : <FeatureDisabledNotice title="التسجيل متوقف مؤقتًا" message={controls.messageFor("يمكنك تصفح المنصة كضيف أو تسجيل الدخول إلى حسابك الحالي.")} />}<AppButton title="تصفح المنصة كضيف" icon="compass-outline" variant="ghost" onPress={() => router.replace("/(tabs)")} /><Text style={[styles.legal, { color: colors.textSoft }]}>بالمتابعة أنت توافق على شروط الاستخدام وسياسة الخصوصية.</Text></FadeIn></Screen>;
}
