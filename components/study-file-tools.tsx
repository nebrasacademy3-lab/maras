...
export function StudyFileTools({ action, resources, storageScope = "workspace", onBack }: { action: StudyAction; resources?: StudyResource[]; storageScope?: string; onBack?: () => void }) {
  ...
  const resume = useCallback(async (id: string, signal: AbortSignal) => {
    ...
    try {
      const value = await observeStudyJob<StudyActionResult>({ id, status: "queued" }, { signal, onStatus: setPhase, onProgress: setProgress });
  ...
  async function control(action: "pause" | "resume" | "cancel") {
    ...
    const id=pendingId, signal=controller.current?.signal;
    if (signal?.aborted) return;
    setControlBusy(true); setError("");
    try {
      const {job}=await controlStudyJob<StudyActionResult>(id,action,signal);
      if (signal?.aborted) return;
      setPhase(job.status); setProgress(normalizeStudyProgress(job.progress,job.status));
      if (job.status==="cancelled") { controller.current?.abort(); rememberedJob(storageKey,null);setPendingId(null);setBusy(false);setError("أُلغي الطلب. قد يكون نداء المزوّد الجاري قد استهلك حصة؛ لم نحذف النتيجة السابقة."); }
      else if (job.status==="paused") { controller.current?.abort();controller.current=new AbortController();setBusy(false); }
      else if (action==="resume" && !busy) { const abort=new AbortController();controller.current=abort;void resume(id,abort.signal); }
  ...
  return <section className={styles.panel} dir="rtl">
    ...
    {progress && <div className={styles.hint} role="status" aria-live="polite"><p>{studyProgressLabel(progress)}</p><progress value={progress.percent} max={100} aria-label="تقدم معالجة المستند" style={{width:"100%"}}/></div>}
    {pendingId && progress && <div className={styles.formGrid}>
      <button type="button" className={styles.secondary} disabled={controlBusy || progress.phase==="pausing"} onClick={()=>void control(phase==="paused" ? "resume" : "pause")}>{phase==="paused" ? "استئناف الأجزاء المتبقية" : "إيقاف المعالجة مؤقتًا"}</button>
      <details><summary>إلغاء المعالجة</summary><p>لن تبدأ أجزاء جديدة بعد الإلغاء. قد يكون نداء المزوّد الجاري قد استهلك حصة؛ الأجزاء المعتمدة تبقى مؤقتًا وفق سياسة الاحتفاظ.</p><button type="button" className={styles.secondary} disabled={controlBusy} onClick={()=>void control("cancel")}>تأكيد إلغاء الطلب</button></details>
    </div>}
...
export function LessonStudyTools({ courseSlug, lessonId, userId }: { courseSlug: string; lessonId: string; userId: number }) {
