...
export function StudyFileTools({ action, resources, storageScope = "workspace", onBack }: { action: StudyAction; resources?: StudyResource[]; storageScope?: string; onBack?: () => void }) {
  ...
  async function upload(selected?: File) {
    ...
    if (selected.size > 20 * 1024 * 1024) { setError("الحد الافتراضي 20 ميجابايت للملف. قسّم الملف الكبير قبل رفعه."); return; }
    setBusy(true); setPhase("upload"); setError(""); setResult(null);
    const abort = new AbortController(); controller.current = abort;
    try {
      const form = new FormData(); form.append("file", selected);
      const response = await studyJson<{ file: AiFilePayload }>("/api/ai/files", { method: "POST", body: form, signal: abort.signal });
