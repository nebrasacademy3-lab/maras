...
export function RequestCourseForm({ universityName, specialty, studentName }: { universityName:string; specialty:string; studentName:string }) {
  ...
  return <form className="request-form" onSubmit={submit} encType="multipart/form-data">
    ...
    <div className="request-guidance"><Sparkles size={19}/><div><strong>ساعدنا نراجع طلب المادة بسرعة</strong><p>ارفع سلايدات الدكتور أو توصيف المقرر وأي ملفات تساعدنا. إذا عندك رابط للمادة أو مجلد Google Drive ضعه في الحقل المخصص أدناه بدل خلطه مع الملاحظات.</p></div></div>
