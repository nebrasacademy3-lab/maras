  ...
  return <section className={styles.panel}>
    ...
    {contracts?.map(contract => <details className={styles.contract} key={contract.id} open={contract.status === "offered"}>
      ...
      <div className={styles.contractBody}>
        ...
        <div className={styles.bilingual}><section dir="rtl" lang="ar"><h3>بنود العقد</h3><p>{contract.termsAr}</p><h3>التجربة والتقييم</h3><p>{contract.trialTermsAr}</p><h3>بدء التعويض المتفق عليه</h3><p>{contract.employment.compensationStart==="after_trial_approval"?"بعد تقييم التجربة واعتماد الاستمرار وفق الشروط":"وفق بنود ومواعيد الدفع في هذه النسخة"}</p><p>{contract.employment.compensationConditionsAr}</p><h3>آلية ومواعيد الأجر</h3><p>{contract.employment.paymentTermsAr}</p><h3>المزايا</h3><p>{contract.employment.benefitsAr}</p></section><section dir="ltr" lang="en"><h3>Agreement terms</h3><p>{contract.termsEn}</p><h3>Probation and assessment</h3><p>{contract.trialTermsEn}</p><h3>Compensation commencement</h3><p>{contract.employment.compensationStart==="after_trial_approval"?"Following probation assessment and approval, under the specified conditions":"Under the payment terms in this version"}</p><p>{contract.employment.compensationConditionsEn}</p><h3>Payment terms</h3><p>{contract.employment.paymentTermsEn}</p><h3>Benefits</h3><p>{contract.employment.benefitsEn}</p></section></div>
