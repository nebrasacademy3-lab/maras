...
type HeaderProps = { appMode?: boolean; userName?: string };
export function SiteHeader(props: HeaderProps) {
  // Search params are isolated behind Suspense so statically rendered public pages still build.
  return <Suspense fallback={<header className="site-header"><div className="container header-inner"><BrandLogo compact /><nav aria-label="التنقل"><Link className="button button-ghost" href="/courses">استكشف المواد</Link></nav></div></header>}><HeaderContent {...props} /></Suspense>;
}
function HeaderContent({ appMode = false, userName = "طالب مراس" }: HeaderProps) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const query = searchParams.toString();
  const menuButton = useRef<HTMLButtonElement>(null);
  const header = useRef<HTMLElement>(null);
  const isActive = (href: string) => navigationIsActive(href, pathname, query);
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener("pointerdown", closeOutside); document.removeEventListener("keydown", escape); window.removeEventListener("resize", onResize); };
  }, [menuOpen]);
  ...
  return <>
    <header ref={header} className={`site-header ${signedIn ? "site-header-app" : ""}`}>
      <div className="container header-inner">
        <BrandLogo compact />
        <nav className="desktop-nav" aria-label={signedIn ? "تنقل حساب الطالب" : "التنقل الرئيسي"}>{activeLinks.filter((link) => !link.mobileOnly).map((link) => <Link key={link.href} href={link.href} aria-current={isActive(link.href) ? "page" : undefined} onClick={() => setMenuOpen(false)} className={isActive(link.href) ? "active" : ""}><link.icon size={15} aria-hidden="true" /><span>{link.label}</span></Link>)}</nav>
        <div className="header-actions">
          ...
          <button ref={menuButton} type="button" aria-controls="site-mobile-navigation" className="icon-button mobile-menu-button" onClick={() => setMenuOpen((open) => !open)} aria-label="القائمة" aria-expanded={menuOpen}>{menuOpen ? <X size={21} /> : <Menu size={21} />}</button>
        </div>
      </div>
