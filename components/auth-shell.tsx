...
export function RegisterForm({ institutions }: { institutions: Institution[] }) {
  ...
  return <form className="auth-form register-form" onSubmit={submit}>
    ...
    {step === 2 && <>
      ...
      <label className="form-label">الجامعة أو الكلية<div className="input-with-icon"><GraduationCap size={18} /><SearchableSelect required value={data.universitySlug} onChange={(event) => setData((current) => ({ ...current, universitySlug: event.target.value, specialty: "" }))}><option value="">اختر من القائمة الرسمية</option>{institutions.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</SearchableSelect></div></label>
      <label className="form-label">التخصص<div className="input-with-icon"><Sparkles size={18} /><SearchableSelect required disabled={!data.universitySlug || catalog.loading} value={data.specialty} onChange={(event) => update("specialty", event.target.value)}><option value="">{catalog.loading ? "جارٍ تحميل التخصصات الرسمية..." : data.universitySlug ? "اختر تخصصك في هذه الجهة" : "اختر الجامعة أولًا"}</option>{catalog.programs.map((item) => <option key={`${item.name}-${item.degree}`} value={item.name}>{item.name} — {item.degree}</option>)}</SearchableSelect></div>{data.universitySlug && !catalog.loading && !catalog.error && <small className="catalog-status"><CheckCircle2 size={13} /> {catalog.verified ? "تمت مطابقة القائمة مع المصدر الرسمي الآن" : "قائمة أكاديمية منظمة مع رابط المصدر الرسمي"} · {catalog.programs.length} برنامجًا</small>}{catalog.error && <small className="catalog-status">تعذر تحميل المصدر الرسمي، استخدم القائمة المتاحة</small>}</label>
      <label className="form-label">المستوى الدراسي<div className="input-with-icon"><GraduationCap size={18} /><SearchableSelect required value={data.academicLevel} onChange={(event) => update("academicLevel", event.target.value)}><option value="">اختر مستواك الحالي</option>{ACADEMIC_LEVELS.map((level) => <option key={level} value={level}>{level}</option>)}</SearchableSelect></div><small className="catalog-status">اختر «خريج» إذا أنهيت دراستك، ويمكن تحديث المستوى من الملف الشخصي لاحقًا.</small></label>
