...
const ticketStatusLabel:Record<string,string>={open:"مفتوحة",waiting:"بانتظار الطالب",resolved:"محلولة",closed:"مغلقة"};
const priorityLabel:Record<string,string>={low:"منخفضة",normal:"عادية",medium:"متوسطة",high:"عالية",urgent:"عاجلة"};
const n=(value:number|undefined|null)=>Number(value||0).toLocaleString("ar-SA");
function describeTask(label:string,payload:TaskResult){
  const parts:string[]=[];
  if(payload.lifecycle)parts.push(`تنبيهات السلة ${n(payload.lifecycle.cartReminders)} · تذكير الدفع ${n(payload.lifecycle.paymentReminders)} · انتهاء الاشتراك ${n(payload.lifecycle.expiryReminders)} · إشعارات الإطلاق ${n(payload.lifecycle.launchNotifications)}`);
  const push=payload.push||payload.result;
  if(push)parts.push(push.attempted?`إشعارات فورية: قُبل ${n(push.accepted)} من ${n(push.attempted)}${push.rejected?` · رُفض ${n(push.rejected)}`:""}${push.invalidated?` · أُبطل ${n(push.invalidated)} جهاز`:""}`:"لا توجد إشعارات فورية مستحقة الآن");
  if(push?.providerErrors?.length)parts.push(`أخطاء المزود: ${push.providerErrors.slice(0,2).join(" · ")}`);
  if(payload.summary)parts.push(payload.summary.scanned?`فُحص ${n(payload.summary.scanned)} ملف: سليم ${n(payload.summary.clean)} · محجوز ${n(payload.summary.quarantined)} · ما زال معلقًا ${n(payload.summary.pending)}`:"لا توجد ملفات معلقة للفحص");
