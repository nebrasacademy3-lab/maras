...
import { dashboardHref, dashboardView } from "@/lib/dashboard-navigation";
...
export function StudentDashboard({ initialView: _initialView = "overview", returnOrder = "", notice = "", user, owned, expired, orders, requests, notices, tickets, institutions, recommended }: { initialView?:string; returnOrder?:string; notice?:string; user:DashboardUser; owned:DashboardCourse[]; expired:DashboardCourse[]; orders:DashboardOrder[]; requests:DashboardRequest[]; notices:DashboardNotice[]; tickets:DashboardTicket[]; institutions:Institution[]; recommended:DashboardRecommendation[] }) {
  void _initialView;
  const searchParams = useSearchParams();
  const active = searchParams.get("payment") === "return" && returnOrder ? "orders" : dashboardView(searchParams.get("view"));
  ...
  return <div className="student-app"><aside className="student-sidebar" aria-label="خدمات الطالب"><div className="student-profile-mini"><div>{live.user.fullName[0]}</div><span><strong>{live.user.fullName}</strong><small>{institution?.name || "مراس العلم"}</small></span></div><nav>{nav.map((item) => { const Icon=item.icon; return <button key={item.id} aria-current={active===item.id?"page":undefined} className={active===item.id?"active":""} onClick={() => setActive(item.id)}><Icon size={18} />{item.label}{item.id==="notifications"&&unread>0&&<i>{unread}</i>}</button>; })}<span className="student-nav-group">خدمات</span>{serviceLinks.map((item)=>{const Icon=item.icon;return <Link key={item.href} href={item.href}><Icon size={18} />{item.label}</Link>;})}</nav><div className="student-sidebar-help"><LifeBuoy size={21} /><strong>تحتاج مساعدة؟</strong><small>فريق مراس معك</small><button onClick={() => setActive("support")}>تواصل معنا</button></div><button className="student-logout" onClick={logout}><LogOut size={17} /> تسجيل الخروج</button></aside><div className="student-content"><div className="student-mobile-tabs">{nav.map((item)=>{const Icon=item.icon;return <button key={item.id} aria-current={active===item.id?"page":undefined} onClick={()=>setActive(item.id)} className={active===item.id?"active":""}><Icon size={18}/><span>{item.label}</span></button>;})}</div>{notice&&!dismissedNotice&&<div className="dashboard-inline-notice" role="status"><span>{notice}</span><button type="button" onClick={()=>setDismissedNotice(true)} aria-label="إخفاء">×</button></div>}<div className="student-view" key={active}>{content}</div></div></div>;
}
