...
import { ArrowLeft, Bell, BookOpen, Bot, CheckCircle2, FileUp, Gift, Heart, Laptop, LayoutDashboard, LifeBuoy, LoaderCircle, LogOut, Paperclip, Receipt, RefreshCw, Settings, ShieldCheck, ShoppingCart, Smartphone, Sparkles, Trash2, UserRound } from "lucide-react";
...
export type DashboardCourse = { slug:string; title:string; university:string; color:string; icon:string; progress:number; current:string; remaining:string; accessState:"active"|"expired"|"suspended"|"scheduled"; expiresAt?:string|null };
...
export function StudentDashboard({ returnOrder = "", notice = "", user, owned, expired, orders, requests, notices, tickets, institutions, recommended }: { initialView?:string; returnOrder?:string; notice?:string; user:DashboardUser; owned:DashboardCourse[]; expired:DashboardCourse[]; orders:DashboardOrder[]; requests:DashboardRequest[]; notices:DashboardNotice[]; tickets:DashboardTicket[]; institutions:Institution[]; recommended:DashboardRecommendation[] }) {
  ...
  if (active === "overview") content = <StudentOverview user={live.user} owned={live.owned} requests={live.requests} recommended={live.recommended} setActive={setActive} />;
...
function Account({user,institutions}:{user:DashboardUser;institutions:Institution[]}) {
  ...
  const save = async (event:React.FormEvent<HTMLFormElement>) => {
    ...
    } catch (error) { setMessage(error instanceof Error && error.name !== "TimeoutError" ? error.message : "تعذر الاتصال. تحقق من الشبكة وحاول مجددًا."); }
    finally { setSaving(false); }
  };
  return <><DashboardTitle title="إعدادات الحساب" description="يجب أن تبقى بياناتك مكتملة لتتمكن من الشراء وطلب المواد."/><aside className="dashboard-panel" style={{marginBottom:20}} aria-label="حالة توثيق البريد"><p className={user.emailVerified?"auth-success":"form-error"}>{user.emailVerified?"بريدك الإلكتروني موثّق. لن تحتاج إلى توثيقه عند كل عملية شراء.":"وثّق بريدك الإلكتروني مرة واحدة، وأكمل بياناتك ليتاح لك الشراء."}</p>{!user.emailVerified&&<Link className="button button-soft" href="/verify-email?return_to=%2Fdashboard">تأكيد البريد الإلكتروني <ArrowLeft size={15}/></Link>}</aside><div className="dashboard-two-columns account-grid"><form className="dashboard-panel account-form" onSubmit={save}><div className="account-avatar"><span>{form.fullName[0]}</span><b>{institution?.name}</b></div><label>الاسم الكامل<input required minLength={5} value={form.fullName} onChange={(event)=>setForm({...form,fullName:event.target.value})}/></label><div className="two-fields"><label>رقم الجوال<input required value={form.phone} onChange={(event)=>setForm({...form,phone:event.target.value})} dir="ltr"/></label><label>البريد الإلكتروني<input value={form.email} disabled dir="ltr"/></label></div><div className="two-fields"><label>الجامعة<SearchableSelect required value={form.universitySlug} onChange={(event)=>setForm({...form,universitySlug:event.target.value,specialty:""})}>{institutions.map((item)=><option key={item.slug} value={item.slug}>{item.name}</option>)}</SearchableSelect></label><label>التخصص<SearchableSelect required disabled={catalog.loading} value={form.specialty} onChange={(event)=>setForm({...form,specialty:event.target.value})}><option value="">{catalog.loading?"جارٍ التحميل...":"اختر تخصصك"}</option>{form.specialty&&!catalog.programs.some((item)=>item.name===form.specialty)&&<option value={form.specialty}>{form.specialty}</option>}{catalog.programs.map((item)=><option key={`${item.name}-${item.degree}`} value={item.name}>{item.name} — {item.degree}</option>)}</SearchableSelect></label></div><label>المستوى الدراسي<SearchableSelect value={form.academicLevel||""} onChange={(event)=>setForm({...form,academicLevel:event.target.value})}><option value="">اختر مستواك</option>{ACADEMIC_LEVELS.map((level)=><option key={level} value={level}>{level}</option>)}</SearchableSelect></label>{(message||catalog.error)&&<p className={message.startsWith("تم")?"auth-success":"form-error"}>{message||catalog.error}</p>}<button className="button button-primary" disabled={saving||catalog.loading}>{saving?"جارٍ الحفظ...":"حفظ التغييرات"}</button></form><SecurityCard /></div><AppearanceSettings /></>;
}

function SecurityCard() {
  const [devices,setDevices]=useState<Array<{id:number;deviceLabel:string;platform:string;firstSeenAt:string;lastSeenAt:string;current:boolean}>>([]);
  ...
  const [deleteMessage,setDeleteMessage]=useState(""); const [showDelete,setShowDelete]=useState(false);
  const loadSessions=useCallback(async()=>{ try{ const response=await fetch("/api/profile/sessions",{credentials:"same-origin",cache:"no-store"}); const data=await response.json() as {sessions?:SessionRow[];registeredDevices?:Array<{id:number;deviceLabel:string;platform:string;firstSeenAt:string;lastSeenAt:string;current:boolean}>;error?:string}; if(!response.ok)throw new Error(data.error||"تعذر تحميل الأجهزة"); setSessions(data.sessions||[]); setDevices(data.registeredDevices||[]); setSessionsError(""); }catch(caught){ setSessionsError(caught instanceof Error?caught.message:"تعذر تحميل الأجهزة"); setSessions([]); } },[]);
  ...
  return <section className="dashboard-panel security-card"><UserRound size={27}/><h2>الأمان والأجهزة</h2><p>تحكم في أمان حسابك، غيّر كلمة المرور، وسجّل خروج الأجهزة التي لم تعد تستخدمها.</p>
    <EmailPasswordChange onChanged={() => void loadSessions()} />
     <EmailChangeForm />
    <AccountMfaPanel /><div className="security-sessions"><h3><ShieldCheck size={16}/> الجهازان المعتمدان · {devices.length} / 2</h3><p className="security-hint">يرتبط حسابك بأول جهازين تسجل منهما الدخول. يبقيان معتمدين حتى بعد تسجيل الخروج. لاستبدال جهاز مفقود أو تالف تواصل مع الدعم.</p><ul>{devices.map(device=><li key={device.id}><i>{device.platform==="mobile"?<Smartphone size={15}/>:<Laptop size={15}/>}</i><span><strong>{device.deviceLabel}{device.current?" · هذا الجهاز":""}</strong><small>معتمد منذ {new Date(device.firstSeenAt).toLocaleDateString("ar-SA")}</small></span></li>)}</ul><h3><Laptop size={16}/> الجلسات المتصلة حاليًا</h3>{sessions===null?<small>جارٍ تحميل الأجهزة...</small>:sessions.length?<ul>{sessions.map((session)=><li key={session.id}><i>{session.platform==="mobile"?<Smartphone size={15}/>:<Laptop size={15}/>}</i><span><strong>{session.deviceLabel}{session.current?" · هذا الجهاز":""}</strong><small>{session.platform==="mobile"?"التطبيق":"الويب"} · آخر نشاط {new Date(session.lastSeenAt).toLocaleString("ar-SA")}</small></span>{!session.current&&<button type="button" disabled={Boolean(busy)} onClick={()=>void revoke(session)}>{busy===`session-${session.id}`?"...":"تسجيل الخروج"}</button>}</li>)}</ul>:<small>لا توجد أجهزة أخرى متصلة.</small>}{sessionsError&&<p className="form-error">{sessionsError}</p>}<small className="security-hint">تسجيل الخروج ينهي الجلسة فقط ولا يلغي اعتماد الجهاز أو يسمح لجهاز ثالث بالدخول.</small></div>
