    ...
    <div className={styles.shell}>
      ...
      {loading && !data ? <div className={styles.loading}><div><div className={styles.spinner} /><p>نحسب البيانات المالية من كامل السجلات…</p></div></div> : data ? <>
        <section className={styles.metrics} aria-label="المؤشرات المالية">
          ...
          <MetricCard label="اشتراكات أدوات مراس" value={money(data.metrics.aiNet || 0)} hint={`${(data.metrics.aiPaidOrders || 0).toLocaleString("ar-SA")} اشتراكًا مدفوعًا · غير مشمولة في صافي المواد`} tone="violet" icon={<BadgeDollarSign size={18} />} />
        </section>
