...
export function AdminDashboard({ adminName, initialView, initialQuery }: { adminName:string; initialView?:string; initialQuery?:string }) {
  ...
  const loadCenters=useCallback(async()=>{
    ...
    const [summary,referrals,security]=await Promise.all([
      can(["operations.manage"]) ? adminFetch("/api/admin/operations/summary",{credentials:"same-origin",signal}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{queues?:Record<string,number>;waitlist?:Record<string,number>}|null> : Promise.resolve(null),
      can(["referrals.manage"]) ? adminFetch("/api/admin/referrals?scope=stats",{credentials:"same-origin",signal}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{stats?:{pending:number};settings?:{enabled:boolean}}|null> : Promise.resolve(null),
