...
import {
  Activity, Bell, BookOpen, Bot, Building2, Check, ChevronLeft, CircleDollarSign, ClipboardList, CreditCard,
  FileVideo, Gift, GraduationCap, Headphones, LayoutDashboard, LoaderCircle, LockKeyhole, LogOut, Menu,
  PackageOpen, Paperclip, Plus, Receipt, RefreshCw, Search, Settings, ShieldCheck, Sparkles, Star, TicketPercent, Upload,
  Route, UserCog, UsersRound, X,
} from "lucide-react";
...
import { ADMIN_CENTERS } from "@/components/admin-center-nav";
...
export function AdminDashboard({ adminName, initialView, initialQuery }: { adminName:string; initialView?:string; initialQuery?:string }) {
  const [active,setActive]=useState<View>(isAdminView(initialView)?initialView:"overview"); const [sidebar,setSidebar]=useState(true); const [query,setQuery]=useState(initialQuery||"");
  ...
  const loadCenters=useCallback(async()=>{
    const [summary,referrals,security]=await Promise.all([
      fetch("/api/admin/operations/summary",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{queues?:Record<string,number>;waitlist?:Record<string,number>}|null>,
      fetch("/api/admin/referrals?scope=stats",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{stats?:{pending:number};settings?:{enabled:boolean}}|null>,
      fetch("/api/admin/security/mfa",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<MfaStatus|null>,
    ]);
  ...
  const load=useCallback(async()=>{const sequence=++requestSequence.current;setLoading(true);setError("");try{const response=await fetch(`/api/admin/console?${new URLSearchParams({view:active,q:query,page:String(page)})}`,{credentials:"same-origin"});const result=await response.json() as ConsoleData&{error?:string};if(!response.ok)throw new Error(result.error||"تعذر تحميل لوحة الإدارة");if(sequence===requestSequence.current)setData(result);void loadCenters();}catch(caught){const message=caught instanceof Error?caught.message:"تعذر التحميل";setError(message);setToast(`تعذر تحديث البيانات: ${message}`);setTimeout(()=>setToast(""),4200);}finally{if(sequence===requestSequence.current)setLoading(false);}},[loadCenters,active,query,page]);
  ...
  useEffect(()=>()=>{if(reloadTimer.current!==null)window.clearTimeout(reloadTimer.current);},[]);
  ...
  const render=()=>{if(!data)return null;const props={data,query,mutate,setDialog,onDelete:requestDelete};switch(active){case"overview":return <Overview data={data} go={setActive} queues={queues} mfa={mfa}/>;case"institutions":return <Institutions {...props}/>;case"specialties":return <Specialties {...props}/>;case"courses":return <Courses {...props}/>;case"content":return <Content data={data} mutate={mutate} refresh={load} onDelete={requestDelete}/>;case"students":return <Users data={data} query={query} mutate={mutate} setDialog={setDialog} role="student" onDelete={requestDelete}/>;case"subscriptions":return <Subscriptions data={data} query={query} mutate={mutate}/>;case"staff":return <Staff data={data} query={query} mutate={mutate} onDelete={requestDelete}/>;case"orders":return <Orders data={data} query={query}/>;case"requests":return <Requests data={data} query={query} mutate={mutate} onDelete={requestDelete}/>;case"support":return <Support data={data} query={query} mutate={mutate} refresh={load} onDelete={requestDelete}/>;case"reviews":return <Reviews data={data} query={query} mutate={mutate} onDelete={requestDelete}/>;case"notifications":return <Notifications data={data} mutate={mutate} onDelete={requestDelete}/>;case"coupons":return <Coupons data={data} mutate={mutate} onDelete={requestDelete}/>;case"settings":return <PlatformSettings data={data} mutate={mutate}/>;case"audit":return <Audit data={data} query={query}/>;}};
  return <main className={`admin-app ${sidebar?"":"admin-sidebar-closed"} ${mobileSidebarOpen?"admin-mobile-sidebar-open":""}`}>{mobileSidebarOpen&&<button type="button" className="admin-sidebar-backdrop" aria-label="إغلاق قائمة الإدارة" onClick={()=>setMobileSidebarOpen(false)}/>}<aside className="admin-sidebar" id="admin-navigation"><div className="admin-brand"><BrandLogo compact/><button onClick={()=>{setSidebar(false);setMobileSidebarOpen(false);}} aria-label="إغلاق القائمة"><X size={18}/></button></div><div className="admin-context"><div><ShieldCheck size={17}/></div><span><strong>إدارة مراس</strong><small>مساحة التحكم بالمنصة</small></span></div><label className="admin-navigation-search"><Search size={16}/><input value={navigationQuery} onChange={(event)=>setNavigationQuery(event.target.value)} placeholder="ابحث عن قسم..." aria-label="البحث في أقسام الإدارة"/></label><nav>{menuGroups.map((group)=>({...group,items:group.items.filter((item)=>includesQuery(navigationQuery,item.label,group.label))})).filter((group)=>group.items.length).map((group)=><section key={group.label}><span>{group.label}</span>{group.items.map((item)=>{const Icon=item.icon;return <button key={item.id} className={active===item.id?"active":""} onClick={()=>{setActive(item.id);setQuery("");setMobileSidebarOpen(false);}}><Icon size={17}/><b>{item.label}</b>{item.id==="requests"&&data?.metrics.openRequests?<i>{data.metrics.openRequests}</i>:item.id==="support"&&data?.metrics.openTickets?<i>{data.metrics.openTickets}</i>:item.id==="reviews"&&data?.metrics.pendingReviews?<i>{data.metrics.pendingReviews}</i>:null}</button>;})}</section>)}</nav><div className="admin-center-links"><span>مراكز الإدارة</span><Link href="/admin/finance"><CircleDollarSign size={17}/><b>المركز المالي</b></Link><Link href="/admin/operations"><Activity size={17}/><b>التشغيل والتحليلات</b></Link><Link href="/admin/bundles"><PackageOpen size={17}/><b>الباقات والعروض</b></Link><Link href="/admin/learning-tracks"><Route size={17}/><b>المسارات القادمة</b></Link><Link href="/admin/referrals"><Gift size={17}/><b>الإحالات والهدايا</b>{queues?.referralsPending?<i>{queues.referralsPending}</i>:null}</Link><Link href="/admin/course-resources"><FileVideo size={17}/><b>ملفات المواد</b></Link><Link href="/admin/partners"><Building2 size={17}/><b>الشركاء والاعتمادات</b></Link><Link href="/admin/ai"><Bot size={17}/><b>أدوات مراس</b></Link><Link href="/admin/files"><ShieldCheck size={17}/><b>فحص الملفات</b></Link><Link href="/admin/seo"><Search size={17}/><b>الظهور في البحث</b></Link><Link href="/admin/security"><LockKeyhole size={17}/><b>أمان الحساب</b></Link></div><button className="admin-logout" onClick={logout}><LogOut size={16}/> تسجيل الخروج</button></aside><section className="admin-main"><header className="admin-header"><div><button className="admin-menu-button" onClick={()=>{setSidebar(!sidebar);setMobileSidebarOpen(!mobileSidebarOpen);}} aria-label="القائمة" aria-expanded={mobileSidebarOpen} aria-controls="admin-navigation"><Menu size={19}/></button><label><Search size={17}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="ابحث في القسم الحالي..."/></label></div><div><ThemeToggle compact/><button className="icon-button" onClick={()=>void load()} aria-label="تحديث"><RefreshCw size={18}/></button><span className="admin-divider"/><span className="admin-user"><span>{adminName[0]}</span><div><strong>{adminName}</strong><small>مدير المنصة</small></div></span></div></header><div className="admin-content"><div className="admin-page-title"><div><p>لوحة الإدارة <ChevronLeft size={12}/> {titles[active]}</p><h1>{titles[active]}</h1></div>{canAdd&&<button className="button button-primary" onClick={openAdd}><Plus size={16}/> إضافة جديد</button>}</div>{mfaNotice&&<AdminMfaNotice/>}{!mfaNotice&&!mfaDismissed&&data&&(data.services.mfaConfigured===false||(mfa&&!mfa.enabled))&&<div className="admin-mfa-banner" role="status"><LockKeyhole size={17}/><p>{data.services.mfaConfigured===false?"مفتاح ADMIN_MFA_ENCRYPTION_KEY غير مضبوط على الخادم؛ لن تعمل العمليات الحساسة (الحذف النهائي، الإحالات، أدوات مراس، المالية) حتى ضبطه ثم تفعيل المصادقة الإضافية.":"فعّل المصادقة الإضافية (MFA) لتنفيذ العمليات الحساسة مثل الحذف النهائي وإدارة الإحالات وأدوات مراس والمالية."}</p><Link href="/admin/security">فتح أمان الحساب</Link><button type="button" onClick={()=>setMfaDismissed(true)} aria-label="إخفاء التنبيه"><X size={15}/></button></div>}{loading&&!data?<Loading/>:error&&!data?<ErrorState text={error} retry={load}/>:render()}{data?.pagination&&data.pagination.view===active&&<nav className="live-actions" aria-label="صفحات القسم"><button disabled={loading||page<=1} onClick={()=>setPageState({view:active,query,page:page-1})}>السابق</button><span>صفحة {page} من {Math.max(1,Math.ceil(data.pagination.total/data.pagination.pageSize))} · {data.pagination.total.toLocaleString("ar-SA")} سجل مطابق</span><button disabled={loading||page*data.pagination.pageSize>=data.pagination.total} onClick={()=>setPageState({view:active,query,page:page+1})}>التالي</button></nav>}</div></section>{dialog&&data&&<AdminDialog dialog={dialog} data={data} close={()=>setDialog(null)} mutate={mutate} refresh={load}/>} {deleteRequest&&<AdminConfirmDialog request={deleteRequest} close={()=>setDeleteRequest(null)} mutate={mutate}/>} {toast&&<div className="admin-toast"><Sparkles size={16}/>{toast}</div>}</main>;
}
...
function Overview({data,go,queues,mfa}:{data:ConsoleData;go:(view:View)=>void;queues:CenterQueues|null;mfa:MfaStatus|null}){const services=[{key:"payments",label:"مدفوعات Tap",copy:"مفتاح الدفع"},{key:"merasAi",label:"أدوات مراس",copy:"مفاتيح مزود الخدمة"},{key:"assistant",label:"المساعد المعرفي",copy:"محرك المعرفة والسياق"},{key:"videoSigning",label:"الفيديو الخاص",copy:"توقيع البث"},{key:"email",label:"استعادة الحساب",copy:"إرسال البريد"},{key:"mfaConfigured",label:"المصادقة الإضافية",copy:"مفتاح تشفير MFA"}] as const;const reviewOrders=data.metrics.reviewOrders??data.orders.filter((row)=>["verification_pending","payment_review"].includes(row.status)).length;const n=(value:number)=>value.toLocaleString("ar-SA");const centerCopy:Record<string,{count:number;copy:string}>={"/admin/finance":{count:(queues?.refundPending||0)+(queues?.settlementUnmatched||0)+reviewOrders,copy:queues?`${n(reviewOrders)} طلب قيد المراجعة · ${n(queues.refundPending)} استرداد معلق · ${n(queues.settlementUnmatched)} تسوية غير مطابقة`:"الإيراد والاستردادات والتسويات"},"/admin/operations":{count:(queues?.filesPendingScan||0)+(queues?.pushPending||0)+(queues?.abandonedCheckout||0),copy:queues?`${n(queues.filesPendingScan)} ملف بانتظار الفحص · ${n(queues.pushPending)} إشعار معلق · ${n(queues.abandonedCheckout)} سلة متروكة · ${n(queues.expiringAccess)} اشتراك ينتهي قريبًا`:"الطوابير والأتمتة والامتثال"},"/admin/bundles":{count:0,copy:"عروض مركبة بتسعير محكوم وسجل تدقيق"},"/admin/learning-tracks":{count:queues?.waitlistActive||0,copy:queues?`${n(queues.waitlistActive)} طالب في قوائم الانتظار`:"خارطة المحتوى وتسجيل الاهتمام"},"/admin/referrals":{count:queues?.referralsPending||0,copy:queues?.referralsEnabled===false?"البرنامج متوقف حاليًا":queues?`${n(queues.referralsPending)} إحالة تحتاج مراجعة`:"المستويات والكوبونات والمراجعة"},"/admin/ai":{count:0,copy:data.services.merasAi?"مفاتيح مزود الخدمة جاهزة":"لا توجد مفاتيح مزود الخدمة نشطة"},"/admin/security":{count:mfa&&!mfa.enabled?1:0,copy:data.services.mfaConfigured===false?"مفتاح MFA غير مضبوط على الخادم":mfa?mfa.enabled?"المصادقة الإضافية مفعلة":"المصادقة الإضافية غير مفعلة بعد":"المصادقة الإضافية والتحقق الإداري"}};return <><div className="admin-metrics"><Metric icon={CircleDollarSign} label="إجمالي الإيراد المؤكد" value={`${data.metrics.revenue.toLocaleString("ar-SA")} ر.س`} copy={`${data.metrics.paidOrders} طلب مدفوع`} color="blue"/><Metric icon={UsersRound} label="الطلاب" value={data.metrics.students.toLocaleString("ar-SA")} copy={`${data.metrics.activeStudents} حساب نشط`} color="violet"/><Metric icon={Building2} label="الجهات التعليمية" value={String(data.metrics.institutions)} copy="قابلة للتعديل والشعار" color="green"/><Metric icon={BookOpen} label="المواد" value={String(data.courses.length)} copy={`${data.metrics.publishedCourses} بمحتوى دروس`} color="orange"/></div><section className="admin-panel admin-centers"><div className="admin-panel-head"><div><h2>مراكز الإدارة</h2><p>كل مركز يفتح صفحته المستقلة بأرقام حية</p></div></div><div className="admin-centers-grid">{ADMIN_CENTERS.filter((center)=>center.href!=="/admin").map((center)=>{const Icon=center.icon;const meta=centerCopy[center.href];return <Link key={center.href} href={center.href} className="admin-center-card"><i><Icon size={19}/></i><span><strong>{center.label}</strong><small>{meta?.copy||center.description}</small></span>{meta?.count?<b>{n(meta.count)}</b>:<ChevronLeft size={16}/>}</Link>;})}</div></section><div className="admin-dashboard-grid"><section className="admin-panel live-queue"><div className="admin-panel-head"><div><h2>طابور العمل</h2><p>أرقام فعلية من قاعدة البيانات</p></div></div>{[{label:"طلبات مواد مفتوحة",value:data.metrics.openRequests,view:"requests" as View},{label:"تذاكر دعم مفتوحة",value:data.metrics.openTickets,view:"support" as View},{label:"تقييمات تنتظر المراجعة",value:data.metrics.pendingReviews,view:"reviews" as View},{label:"طلبات دفع قيد المراجعة",value:reviewOrders,view:"orders" as View},{label:"عمليات دفع غير مكتملة",value:data.orders.filter((row)=>!["paid","refunded","verification_pending","payment_review"].includes(row.status)).length,view:"orders" as View}].map((item)=><button key={item.label} onClick={()=>go(item.view)}><span>{item.label}</span><strong>{item.value}</strong><ChevronLeft size={16}/></button>)}</section><section className="admin-panel live-services"><div className="admin-panel-head"><div><h2>جاهزية الخدمات</h2><p>لا تُعرض المفاتيح السرية</p></div></div>{services.map((service)=>{const ready=service.key==="mfaConfigured"?data.services.mfaConfigured!==false:Boolean(data.services[service.key]);return <div key={service.key}><i className={ready?"ok":"warn"}/><span><strong>{service.label}</strong><small>{ready?`${service.copy} مضبوط`: `${service.copy} يحتاج متغير بيئة`}</small></span><em>{ready?"جاهز":"إجراء مطلوب"}</em></div>;})}</section></div><section className="admin-panel live-recent"><div className="admin-panel-head"><div><h2>آخر العمليات</h2><p>آخر تحديث {new Date(data.generatedAt).toLocaleString("ar-SA")}</p></div><button onClick={()=>go("orders")}>عرض الكل <ChevronLeft size={14}/></button></div><Table headers={["الطلب","الطالب","المادة","المبلغ","الحالة"]}>{data.orders.slice(0,8).map((row)=><div className="live-table-row" key={row.orderNumber}><b dir="ltr">{row.orderNumber}</b><span>{row.customerName}<small>{row.customerEmail}</small></span><span>{data.courses.find((course)=>course.slug===row.courseSlug)?.title||row.courseSlug}</span><strong>{row.total} {row.currency}</strong><em className={`status ${row.status==="paid"?"published":"draft"}`}>{statusLabel(row.status)}</em></div>)}</Table></section></>}
...
function Courses({data,query,setDialog,onDelete}:{data:ConsoleData;query:string;mutate:(p:Record<string,unknown>,s?:string)=>Promise<boolean>;setDialog:(d:Dialog)=>void;onDelete:(entityType:string,entityId:string|number,label:string,impact:string)=>void}){const rows=data.courses.filter((row)=>includesQuery(query,row.title,row.university,row.specialty,row.code));return <section className="admin-panel data-panel"><Table headers={["المادة","الجامعة والتخصص","السعر","الدروس","الطلب والوصول","إجراء"]}>{rows.map((row)=><div className="live-table-row" key={row.slug}><span className="live-entity"><i className={`bg-gradient-to-br ${row.color}`}>{row.coverImage?<img src={row.coverImage} alt="" loading="lazy"/>:row.icon}</i><b>{row.title}<small>{row.code||row.slug}</small></b></span><span>{row.university}<small>{row.specialty}</small></span><strong>{row.price} ر.س</strong><span>{row.lessons}<small>{statusLabel(row.status||"published")}</small></span><span>{row.access}<small>{row.waitlistCount?`${row.waitlistCount} في قائمة الانتظار`:"لا يوجد انتظار"}</small></span><span className="live-actions"><Link href={`/admin/courses/${encodeURIComponent(row.slug)}`}>المشتركون والمنتظرون</Link><button onClick={()=>setDialog({kind:"course",item:row})}>تعديل المادة</button><button className="danger-button" onClick={()=>onDelete("course",row.slug,row.title,"سيُحذف الوحدات والدروس والفيديوهات والملفات والتقدم والمفضلة والسلة. لن يُحذف إذا وُجد طلب مدفوع أو فاتورة أو وصول فعال.")}>حذف نهائي</button></span></div>)}</Table></section>}
...
async function readBrowserVideoDuration(file: File) {
  if (typeof window === "undefined" || typeof document === "undefined" || !file.type.startsWith("video/")) return 0;
  ...
  return await new Promise<number>((resolve) => {
    ...
    const done = (value: number) => {
      cleanup();
      resolve(Number.isFinite(value) && value > 0 && value <= 7 * 24 * 60 * 60 ? Math.round(value) : 0);
    };
...
function VideoUpload({
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const browserDurationSeconds = await readBrowserVideoDuration(file);
      ...
      const completeResponse = await fetch("/api/admin/videos/direct", {
        ...
        body: JSON.stringify({
          ...
          contentType,
          sizeBytes: file.size,
          durationSeconds: browserDurationSeconds || 0,
        }),
      });
...
<button type="button" disabled={!selected} onClick={()=>void mutate({action:"prepareRequest",id:row.id,courseSlug:selected},"تم تجهيز الطلب وإرسال الإشعار")}>تم تجهيز الطلب</button><button type="button" className="danger-button" onClick={()=>onDelete("course_request",row.id,row.courseName,"سيُحذف الطلب وجميع ملفاته من التخزين نهائيًا، ولن يؤثر على الطلبات المدفوعة.")}>حذف الطلب</button></div></div>})}</Table>{rows.length===0&&<p className="live-empty">لا توجد طلبات مطابقة للبحث.</p>}</section>}
...
</label><button className="danger-button" onClick={()=>onDelete("support_ticket",selected.id,selected.title,"سيُحذف عنوان التذكرة والمحادثة وجميع المرفقات من قاعدة البيانات والتخزين نهائيًا.")}>حذف نهائي</button></div>
  </section>;
}
...
<button className="danger-button" onClick={()=>onDelete("review",row.id,`تقييم ${row.courseSlug}`,"سيُحذف التقييم فقط مع إبقاء المادة والحساب وسجل التدقيق.")}>حذف التقييم</button></span></div>)}</Table></section>}
...
function AdminDialog({dialog,data,close,mutate,refresh}:{dialog:Exclude<Dialog,null>;data:ConsoleData;close:()=>void;mutate:(p:Record<string,unknown>,s?:string)=>Promise<boolean>;refresh:()=>Promise<void>}){const[busy,setBusy]=useState(false);const[error,setError]=useState("");const[operationKey]=useState(()=>crypto.randomUUID());const submit=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();setBusy(true);setError("");const form=new FormData(event.currentTarget);try{if(dialog.kind==="institution"){const institutionKey=String(form.get("slug")||makeInstitutionSlug(String(form.get("name")||"")));if(!await mutate({action:"saveInstitution",slug:institutionKey,name:form.get("name"),nameEn:form.get("nameEn"),region:form.get("region"),type:form.get("type"),domain:form.get("domain"),directorySourceUrl:form.get("directorySourceUrl"),verificationStatus:form.get("verificationStatus"),aliases:String(form.get("aliases")||"").split("\n").map((value)=>value.trim()).filter(Boolean),logoUrl:form.get("logoUrl"),status:form.get("status"),featured:form.get("featured")==="on"}))throw new Error("تعذر حفظ بيانات الجهة");const file=form.get("logoFile");if(file instanceof File&&file.size){const upload=new FormData();upload.set("slug",institutionKey);upload.set("file",file);const response=await fetch("/api/admin/logos",{method:"POST",credentials:"same-origin",body:upload});const result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||"تعذر رفع الشعار");await refresh();}}else if(dialog.kind==="specialty"){const specialtyKey=String(form.get("slug")||"");if(!await mutate({action:"saveSpecialty",slug:specialtyKey,name:form.get("name"),description:form.get("description"),sourceUrl:form.get("sourceUrl"),verifiedAt:form.get("verifiedAt"),verificationStatus:form.get("verificationStatus"),faculty:form.get("faculty"),degree:form.get("degree"),status:form.get("status"),institutionSlug:form.get("institutionSlug")}))throw new Error("تعذر حفظ التخصص");}else if(dialog.kind==="course"){const courseKey=String(form.get("slug")||makeCourseSlug(String(form.get("institutionSlug")||""),String(form.get("specialtySlug")||""),String(form.get("title")||"")));if(!await mutate({action:"saveCourse",slug:courseKey,title:form.get("title"),titleEn:form.get("titleEn"),code:form.get("code"),description:form.get("description"),coverImageUrl:form.get("coverImageUrl"),institutionSlug:form.get("institutionSlug"),specialtySlug:form.get("specialtySlug"),audienceScope:form.get("audienceScope"),price:Number(form.get("price")),oldPrice:Number(form.get("oldPrice")),accessLabel:form.get("accessLabel"),sourceUrl:form.get("sourceUrl"),verifiedAt:form.get("verifiedAt"),status:form.get("status"),featured:form.get("featured")==="on",coverTheme:form.get("coverTheme")}))throw new Error("تعذر حفظ المادة");const coverFile=form.get("coverFile");if(coverFile instanceof File&&coverFile.size){const upload=new FormData();upload.set("courseSlug",courseKey);upload.set("file",coverFile);const response=await fetch("/api/admin/covers",{method:"POST",credentials:"same-origin",body:upload});const result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||"تعذر رفع غلاف المادة");await refresh();}}else if(dialog.kind==="staff"){const response=await fetch("/api/admin/staff",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify({email:form.get("email"),fullName:form.get("fullName"),phone:form.get("phone"),password:form.get("password"),role:form.get("role"),universitySlug:form.get("universitySlug"),specialty:form.get("specialty")})});const result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||"تعذر حفظ الموظف");await refresh();}else if(dialog.kind==="access"){if(!await mutate({action:"grantAccess",operationKey,userEmail:dialog.user.email,courseSlug:form.get("courseSlug"),grantType:form.get("grantType"),price:Number(form.get("price")),expiresAt:form.get("expiresAt")},"تم منح صلاحية المادة"))throw new Error("تعذر منح صلاحية المادة");}close();}catch(caught){setError(caught instanceof Error?caught.message:"تعذر الحفظ");setBusy(false);}};const item=dialog.kind==="institution"?dialog.item:dialog.kind==="specialty"?dialog.item:dialog.kind==="course"?dialog.item:undefined;return <div className="admin-modal-overlay"><button onClick={close} aria-label="إغلاق"/><form className="admin-modal live-modal" onSubmit={submit}><div><span><Settings size={19}/></span><div><h2>{dialog.kind==="access"?`منح مادة إلى ${dialog.user.fullName}`:item?"تعديل السجل":"إضافة سجل جديد"}</h2><p>يحفظ التغيير في قاعدة البيانات ويسجل في سجل النشاط.</p></div><button type="button" onClick={close}><X size={19}/></button></div>{dialog.kind==="institution"&&<><label>المعرّف (اختياري؛ يُنشأ تلقائيًا عند تركه فارغًا)<input name="slug" readOnly={Boolean(dialog.item)} placeholder={dialog.item?.slug||"مثال: king-saud-university"} defaultValue={dialog.item?.slug} pattern="[A-Za-z0-9._-]{2,80}" dir="ltr"/></label><label>الاسم العربي<input name="name" required defaultValue={dialog.item?.name}/></label><label>الاسم الإنجليزي<input name="nameEn" defaultValue={dialog.item?.nameEn} dir="ltr"/></label><div className="two-fields"><label>المنطقة<input name="region" required defaultValue={dialog.item?.region}/></label><label>النوع
