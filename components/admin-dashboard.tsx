...
import {
  ...
  FileVideo, GraduationCap, Headphones, LayoutDashboard, LoaderCircle, LockKeyhole, LogOut, Menu,
...
export function AdminDashboard({ adminName, initialView, initialQuery }: { adminName:string; initialView?:string; initialQuery?:string }) {
  const { can, canVisit, owner } = useAdminAccess();
  const [active,setActive]=useState<View>(isAdminView(initialView) && can(CONSOLE_VIEWS[initialView]) ? initialView : "overview"); const [sidebar,setSidebar]=useState(true); const [query,setQuery]=useState(initialQuery||"");
  ...
  const loadCenters=useCallback(async()=>{
    const [summary,referrals,security]=await Promise.all([
      adminFetch("/api/admin/operations/summary",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{queues?:Record<string,number>;waitlist?:Record<string,number>}|null>,
      adminFetch("/api/admin/referrals?scope=stats",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<{stats?:{pending:number};settings?:{enabled:boolean}}|null>,
      adminFetch("/api/admin/security/mfa",{credentials:"same-origin"}).then((response)=>response.ok?response.json():null).catch(()=>null) as Promise<MfaStatus|null>,
    ]);
  ...
  const canAdd=can(["catalog.manage"]) && ["institutions","specialties","courses"].includes(active);
