...
type Provider = { decryptable?: boolean; decryptionError?: string; id: number; label: string; projectLabel: string | null; tier: "free" | "paid"; maskedKey: string; fingerprint: string; priority: number; status: string; cooldownUntil: string | null; consecutiveFailures: number; lastUsedAt: string | null; lastSuccessAt: string | null; lastErrorCode: string | null; lastErrorMessage?: string };
...
export function AdminAiCenter({ adminName }: { adminName: string }) {
  ...
  return <main className={styles.page} dir="rtl"><div className={styles.shell}>
    ...
    {data&&tab==="providers" ? <section className={styles.providers}>
      ...
      <div className={styles.providerList}><header><div><small>تدوير تلقائي وفترة تهدئة</small><h2>المزودون المحفوظون</h2></div><span>{data.environmentKeyCount} مفاتيح من بيئة الخادم</span></header>{data.keys.map((key)=><article key={key.id}><div className={styles.keyIcon}><KeyRound size={19}/></div><div className={styles.keyMeta}><div><h3>{key.label}</h3><span className={key.status==="active"?styles.activeStatus:styles.disabledStatus}>{key.status==="active"?"نشط":key.status==="error"?"يحتاج مراجعة":"متوقف"}</span></div><p><strong>{key.tier === "paid" ? "مدفوع" : "مجاني"}</strong> · {key.decryptionError ? <strong role="alert">{key.decryptionError} </strong> : null}{key.projectLabel||"لم يحدد المشروع"} · <span dir="ltr">{key.maskedKey}</span></p><small>آخر نجاح: {key.lastSuccessAt?new Date(key.lastSuccessAt).toLocaleString("ar-SA"):"لا يوجد"} · آخر استخدام: {key.lastUsedAt?new Date(key.lastUsedAt).toLocaleString("ar-SA"):"لم يُستخدم"}{key.consecutiveFailures?` · ${key.consecutiveFailures} إخفاقات متتالية`:""}{key.cooldownUntil?` · آخر فترة تهدئة حتى ${new Date(key.cooldownUntil).toLocaleTimeString("ar-SA")}`:""}{key.lastErrorCode?` · ${key.lastErrorMessage || key.lastErrorCode}`:""}</small></div><label>الأولوية<input type="number" min="1" value={key.priority} onChange={(event)=>setData((current)=>current?{...current,keys:current.keys.map((item)=>item.id===key.id?{...item,priority:Number(event.target.value)}:item)}:current)}/></label><button disabled={Boolean(busy)} onClick={()=>void mutate({action:"updateKey",id:key.id,label:key.label,projectLabel:key.projectLabel,priority:key.priority,status:key.status==="active"?"disabled":"active"},key.status==="active"?"تم تعطيل المفتاح.":"تم تفعيل المفتاح وإعادته للتدوير.")}>{key.status==="active"?"تعطيل":"تفعيل"}</button></article>)}{!data.keys.length?<div className={styles.empty}><KeyRound size={27}/><p>لا توجد مفاتيح محفوظة في قاعدة البيانات. يمكن أن تعمل مفاتيح بيئة الخادم، لكنها لا تظهر هنا حفاظًا على سريتها.</p></div>:null}</div>
    </section> : null}
