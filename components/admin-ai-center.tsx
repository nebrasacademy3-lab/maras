...
type Provider = { decryptable?: boolean; decryptionError?: string; id: number; label: string; projectLabel: string | null; tier: "free" | "paid" | "unverified"; projectNumber?: string | null; proofValidUntil?: string | null; maskedKey: string; fingerprint: string; priority: number; status: string; cooldownUntil: string | null; consecutiveFailures: number; lastUsedAt: string | null; lastSuccessAt: string | null; lastErrorCode: string | null; lastErrorMessage?: string };
...
type PaidFallbackPolicy = { enabled: boolean; dailyCapSar: number; monthlyCapSar: number; perRequestCapSar: number; estimatedRequestSar: number; updatedAt: string | null; updatedBy: string | null };
...
type Dashboard = { monthlyPrice: number; currency: string; paidFallback: PaidFallbackPolicy; settings: ServiceSetting[]; keys: Provider[]; environmentKeyCount: number; entitlements: Entitlement[]; usage: Usage[]; subscriptionOrders?: SubscriptionOrder[]; subscriptionSummary?: Array<{ status: string; total: number; amount: number }> };
...
export function AdminAiCenter({ adminName }: { adminName: string }) {
  ...
  const dirtyServices = useRef(new Set<AiService>());
  const priceDirty = useRef(false);
  const [models, setModels] = useState<GeminiModelOption[]>([]);
  const [paidFallbackDraft, setPaidFallbackDraft] = useState<PaidFallbackPolicy>({ enabled: false, dailyCapSar: 0, monthlyCapSar: 0, perRequestCapSar: 0, estimatedRequestSar: 0, updatedAt: null, updatedBy: null });
  const paidFallbackDirty = useRef(false);
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!paidFallbackDirty.current && result.paidFallback) setPaidFallbackDraft(result.paidFallback);
  ...
  const mutate = async (body: Record<string, unknown>, success: string, key?: string) => {
    ...
    try {
      ...
      if (body.action === "setPaidFallback") paidFallbackDirty.current = false;
  ...
  const updateService = (service: AiService, patch: Partial<ServiceSetting>) => { dirtyServices.current.add(service); setData((current) => current ? { ...current, settings: current.settings.map((setting) => setting.service === service ? { ...setting, ...patch } : setting) } : current); };
  const updatePaidFallback = (patch: Partial<PaidFallbackPolicy>) => { paidFallbackDirty.current = true; setPaidFallbackDraft((current) => ({ ...current, ...patch })); };
  ...
  return <main className={styles.page} dir="rtl"><div className={styles.shell}>
    ...
    {notice && notice.tone==="error" && isAdminStepUpMessage(notice.text) ? <AdminMfaNotice /> : notice ? <div className={`${styles.notice} ${styles[notice.tone]}`}>{notice.tone==="ok"?<Check size={17}/>:<CircleAlert size={17}/>}<span>{notice.text}</span><button aria-label="إغلاق الرسالة" onClick={()=>setNotice(null)}><X size={16}/></button></div> : null}
    ...
    <section className={styles.metrics}>
      ...
      <article><span className={styles.blue}><KeyRound size={20}/></span><div><small>مفاتيح مفعّلة</small><b>{activeProviders.toLocaleString("ar-SA")}</b><em>{data?.keys.length || 0} محفوظة بأمان</em></div></article>
    ...
    {data&&tab==="overview" ? <section className={styles.overview}>
      ...
      <div className={styles.paidFallbackCard}><header><div><small>بوابة الفوترة الآمنة</small><h2>احتياط Gemini المدفوع</h2></div><span className={paidFallbackDraft.enabled ? styles.paidOn : styles.paidOff}>{paidFallbackDraft.enabled ? "مسموح بسقوف" : "مغلق افتراضيًا"}</span></header><p>لا يُستخدم المدفوع إلا بعد محاولة كل المفاتيح المجانية المؤهلة وفشلها بحصتها. الحجز ذري، والسقوف توقف المسار فورًا ولا تُنفّذ دفعة تلقائية خارج هذه السياسة.</p><div className={styles.paidFallbackGrid}><label className={styles.paidToggle}><input type="checkbox" checked={paidFallbackDraft.enabled} onChange={(event)=>updatePaidFallback({enabled:event.target.checked})}/><span>السماح بالاحتياط بعد نفاد المجاني</span></label><label>السقف اليومي (ر.س)<input type="number" min="0" step="0.01" value={paidFallbackDraft.dailyCapSar} onChange={(event)=>updatePaidFallback({dailyCapSar:Number(event.target.value)})}/></label><label>السقف الشهري (ر.س)<input type="number" min="0" step="0.01" value={paidFallbackDraft.monthlyCapSar} onChange={(event)=>updatePaidFallback({monthlyCapSar:Number(event.target.value)})}/></label><label>سقف الطلب (ر.س)<input type="number" min="0" step="0.01" value={paidFallbackDraft.perRequestCapSar} onChange={(event)=>updatePaidFallback({perRequestCapSar:Number(event.target.value)})}/></label><label>التكلفة التقديرية للطلب<input type="number" min="0" step="0.01" value={paidFallbackDraft.estimatedRequestSar} onChange={(event)=>updatePaidFallback({estimatedRequestSar:Number(event.target.value)})}/></label></div><button className={styles.saveButton} disabled={Boolean(busy)} onClick={()=>void mutate({action:"setPaidFallback",...paidFallbackDraft}, "تم حفظ سياسة الاحتياط المدفوع؛ تبقى مغلقة ما لم تُفعّل بسقوف صالحة.", "setPaidFallback")}><Save size={16}/> حفظ سياسة الاحتياط</button></div>
    </section> : null}

    {data && (tab === "services" || tab === "providers") && <AdminGeminiConnection providers={data.keys} environmentKeyCount={data.environmentKeyCount} draftApiKey={keyForm.apiKey} defaultModel={data.settings[0]?.model || ""} onModels={setModels} onTested={() => void load(true)} />}
    <datalist id="admin-gemini-models">{models.map(model => <option key={model.id} value={model.id}>{model.displayName}</option>)}</datalist>

    {data&&tab==="services" ? <section className={styles.services}>
      <div className={styles.sectionTitle}><div><small>سيطرة مستقلة لكل أداة</small><h2>الخدمات والحدود الشهرية</h2><p>اجلب قائمة النماذج من فحص الاتصال أعلاه، أو الصق معرّف النموذج بما فيه بادئة models/. الصفر في الحصة يعني أن الخدمة غير متاحة لتلك الخطة. تبقى تعديلاتك محفوظة في النموذج عند تحديث الحالة.</p></div></div>
    ...
    {data&&tab==="providers" ? <section className={styles.providers}>
      <div className={styles.providerForm}><div className={styles.sectionTitle}><div><small>مفتاح من مشروع مستقل عند الحاجة</small><h2>إضافة مزود الخدمة</h2><p>يُشفّر المفتاح بـ AES-256-GCM ولا يظهر مرة أخرى. تُقبل مفاتيح المصادقة الحديثة التي تبدأ بـ AQ.؛ حفظ المفتاح لا يعني أن الاتصال اختُبر. تعدد المفاتيح يفيد التوفر عندما تنتمي إلى حصص أو مشاريع مختلفة.</p></div></div><div className={styles.keyFields}><label>طريقة الحفظ<SearchableSelect value={String(replacementId)} onChange={event => setReplacementId(Number(event.target.value))}><option value="0">إضافة مفتاح جديد</option>{data.keys.map(key => <option key={key.id} value={String(key.id)}>إعادة حفظ: {key.label}</option>)}</SearchableSelect></label><label>اسم واضح<input value={keyForm.label} onChange={(event)=>setKeyForm({...keyForm,label:event.target.value})} placeholder="مزود رئيسي"/></label><label>وصف المشروع (لا يثبت الهوية أو الفوترة)<input value={keyForm.projectLabel} onChange={(event)=>setKeyForm({...keyForm,projectLabel:event.target.value})} placeholder="لوحة مزود الخدمة · Project A"/></label><label>الأولوية<input type="number" min="1" value={keyForm.priority} onChange={(event)=>setKeyForm({...keyForm,priority:Number(event.target.value)})}/></label><label className={styles.keyInput}>مفتاح API<input dir="ltr" type="password" autoComplete="new-password" spellCheck={false} maxLength={4096} value={keyForm.apiKey} onChange={(event)=>setKeyForm({...keyForm,apiKey:event.target.value})} placeholder="AQ.… أو مفتاح Google AI Studio"/></label><button disabled={Boolean(busy)} onClick={async()=>{if(await mutate({action:replacementId?"replaceKey":"addKey",id:replacementId,...keyForm},"تم تشفير المفتاح وحفظه. اختبر الأداة للتأكد من الاتصال."))setKeyForm({label:"",projectLabel:"",apiKey:"",priority:100});}}>{busy==="addKey"?<LoaderCircle className={styles.spin} size={17}/>:<Plus size={17}/>} {replacementId?"إعادة تشفير وحفظ المفتاح":"إضافة المفتاح"}</button></div></div>
      <div className={styles.providerList}><header><div><small>تدوير تلقائي وفترة تهدئة</small><h2>المزودون المحفوظون</h2></div><span>{data.environmentKeyCount} مفاتيح من بيئة الخادم</span></header>{data.keys.map((key)=><article key={key.id}><div className={styles.keyIcon}><KeyRound size={19}/></div><div className={styles.keyMeta}><div><h3>{key.label}</h3><span className={key.status==="active"?styles.activeStatus:styles.disabledStatus}>{key.status==="active"?"نشط":key.status==="error"?"يحتاج مراجعة":"متوقف"}</span></div><p><strong>{key.tier === "paid" ? "مدفوع — محجوب" : key.tier === "free" ? "غير مرتبط بالفوترة" : "غير مثبت — محجوب"}</strong> · {key.decryptionError ? <strong role="alert">{key.decryptionError} </strong> : null}{key.projectLabel||"لم يحدد المشروع"} · <span dir="ltr">{key.maskedKey}</span></p><small>آخر نجاح: {key.lastSuccessAt?new Date(key.lastSuccessAt).toLocaleString("ar-SA"):"لا يوجد"} · آخر استخدام: {key.lastUsedAt?new Date(key.lastUsedAt).toLocaleString("ar-SA"):"لم يُستخدم"}{key.consecutiveFailures?` · ${key.consecutiveFailures} إخفاقات متتالية`:""}{key.cooldownUntil?` · آخر فترة تهدئة حتى ${new Date(key.cooldownUntil).toLocaleTimeString("ar-SA")}`:""}{key.lastErrorCode?` · ${key.lastErrorMessage || key.lastErrorCode}`:""}</small></div><label>الأولوية<input type="number" min="1" value={key.priority} onChange={(event)=>setData((current)=>current?{...current,keys:current.keys.map((item)=>item.id===key.id?{...item,priority:Number(event.target.value)}:item)}:current)}/></label><button disabled={Boolean(busy)} onClick={()=>void mutate({action:"updateKey",id:key.id,label:key.label,projectLabel:key.projectLabel,priority:key.priority,status:key.status==="active"?"disabled":"active"},key.status==="active"?"تم تعطيل المفتاح.":"تم تفعيل المفتاح وإعادته للتدوير.")}>{key.status==="active"?"تعطيل":"تفعيل"}</button></article>)}{!data.keys.length?<div className={styles.empty}><KeyRound size={27}/><p>لا توجد مفاتيح محفوظة في قاعدة البيانات. يمكن أن تعمل مفاتيح بيئة الخادم، لكنها لا تظهر هنا حفاظًا على سريتها.</p></div>:null}</div>
    </section> : null}

    {data&&tab==="subscriptions" ? <section className={styles.subscriptions}>
      <div className={styles.subscriptionControls}><div><small>السعر العام</small><h2>اشتراك أدوات مراس</h2><p>يظهر للطلاب غير المشتركين. يحصل مشترك المادة على الخطة تلقائيًا دون خصم هذا السعر.</p><label>السعر الشهري<div><input type="number" min="1" value={price} onChange={(event)=>{priceDirty.current = true; setPrice(event.target.value);}}/><span>ريال</span><button onClick={()=>void mutate({action:"setSubscription",monthlyPrice:Number(price)},"تم تحديث سعر الاشتراك.")}><Save size={15}/> حفظ</button></div></label></div><div><small>منحة مخصصة</small><h2>امنح طالبًا اشتراكًا</h2><p>يمكن استخدامها لهدية أو إحالة أو معالجة اشتراك مدفوع يدويًا.</p><div className={styles.grantForm}><input type="email" dir="ltr" placeholder="student@example.com" value={grant.email} onChange={(event)=>setGrant({...grant,email:event.target.value})}/><SearchableSelect value={grant.source} onChange={(event)=>setGrant({...grant,source:event.target.value})}><option value="admin">منحة إدارية</option><option value="paid">اشتراك مدفوع</option><option value="gift">هدية</option><option value="referral">إحالة</option></SearchableSelect><input type="number" min="1" max="36" value={grant.months} onChange={(event)=>setGrant({...grant,months:Number(event.target.value)})}/><button onClick={async()=>{if(await mutate({action:"grantEntitlement",...grant},"تم منح الاشتراك وإرسال إشعار للطالب."))setGrant({...grant,email:""});}}><Gift size={15}/> منح</button></div></div></div>
