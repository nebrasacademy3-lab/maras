...
export function AdminAiCenter({ adminName }: { adminName: string }) {
  ...
  return <main className={styles.page} dir="rtl"><div className={styles.shell}>
    ...
    <nav className={styles.tabs}>{([ ["overview","نظرة عامة",Gauge], ["services","الخدمات والحدود",SlidersHorizontal], ["providers","مفاتيح Gemini",KeyRound], ["subscriptions","الاشتراكات والمنح",Gift], ["orders","الاشتراكات المدفوعة",CircleDollarSign] ] as const).map(([value,label,Icon])=><button key={value} className={tab===value?styles.activeTab:""} onClick={()=>setTab(value)}><Icon size={16}/>{label}</button>)}</nav>
    ...
    {!data&&busy!=="load" ? <div className={styles.empty}><CircleAlert size={27}/><p>{notice?.text || "تعذر تحميل مركز مراس AI."}</p><button className={styles.saveButton} onClick={()=>void load()}><RefreshCw size={16}/> إعادة المحاولة</button></div> : null}
