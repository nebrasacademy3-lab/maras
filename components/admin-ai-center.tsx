...
import {
  Activity, ArrowLeft, Bot, Check, CircleAlert, Coins, Gauge, Gift,
...
type Dashboard = { monthlyPrice: number; currency: string; settings: ServiceSetting[]; keys: Provider[]; environmentKeyCount: number; entitlements: Entitlement[]; usage: Usage[] };
type Tab = "overview" | "services" | "providers" | "subscriptions";
...
export function AdminAiCenter({ adminName }: { adminName: string }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (error) { setNotice({ tone: "error", text: error instanceof Error ? error.message : "تعذر تحميل مركز AI" }); }
    finally { setBusy(""); }
  }, []);
  ...
  return <main className={styles.page} dir="rtl"><div className={styles.shell}>
    <header className={styles.header}>
      <div className={styles.heading}><span><Sparkles size={24}/></span><div><small>مركز الذكاء الاصطناعي</small><h1>إدارة مراس AI</h1><p>{adminName} · الخدمات والمزودون والحدود والاشتراكات من مكان واحد.</p></div></div>
      <div className={styles.headerActions}><Link href="/admin/security"><ShieldCheck size={16}/> التحقق الإداري</Link><button onClick={()=>void load()} disabled={busy==="load"}><RefreshCw className={busy==="load"?styles.spin:""} size={16}/> تحديث</button><Link href="/admin"><ArrowLeft size={16}/> لوحة الإدارة</Link></div>
    </header>

    {notice ? <div className={`${styles.notice} ${styles[notice.tone]}`}>{notice.tone==="ok"?<Check size={17}/>:<CircleAlert size={17}/>}<span>{notice.text}</span><button onClick={()=>setNotice(null)}><X size={16}/></button></div> : null}
    ...
    <section className={styles.metrics}>
      ...
      <article><span className={styles.gold}><Coins size={20}/></span><div><small>السعر الشهري</small><b>{data?.monthlyPrice || 30} ر.س</b><em>اشتراك المادة يشمله مجانًا</em></div></article>
    </section>

    <nav className={styles.tabs}>{([ ["overview","نظرة عامة",Gauge], ["services","الخدمات والحدود",SlidersHorizontal], ["providers","مفاتيح Gemini",KeyRound], ["subscriptions","الاشتراكات والمنح",Gift] ] as const).map(([value,label,Icon])=><button key={value} className={tab===value?styles.activeTab:""} onClick={()=>setTab(value)}><Icon size={16}/>{label}</button>)}</nav>

    {busy==="load"&&!data ? <div className={styles.loading}><LoaderCircle className={styles.spin} size={28}/><span>يجري تحميل مركز مراس AI…</span></div> : null}
    ...
    {data&&tab==="overview" ? <section className={styles.overview}>
      ...
      <div className={styles.policyCard}><Bot size={25}/><h2>سياسة الاستحقاق</h2><p>يحصل مشتركو أي مادة نشطة على خطة AI للمشترك تلقائيًا. ويظل بإمكان الإدارة منح اشتراك مستقل كهدية أو إحالة أو اشتراك مدفوع.</p><button onClick={()=>setTab("subscriptions")}>إدارة المنح والاشتراكات</button></div>
    </section> : null}
