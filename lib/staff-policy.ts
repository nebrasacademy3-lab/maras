...
export function requiredRoutePermissions(path: string, method = "GET"): string[] | null {
  ...
  if (/^\/api\/admin\/(course-resources|covers|logos|videos)(\/|$)/.test(path)) return [read ? "catalog.view" : "catalog.manage", ...(method === "DELETE" ? ["records.delete"] : [])];
  if (path === "/api/admin/bundles") return [read ? "catalog.view" : "catalog.manage", "data.all", ...(method === "DELETE" ? ["records.delete"] : [])];
  ...
  if (path === "/api/admin/ai") return ["ai.manage", ...global];
  if (path === "/api/admin/referrals") return ["referrals.manage", ...global];
  if (path === "/api/admin/learning-tracks") return ["roadmap.manage", ...global];
  ...
  if (path === "/api/admin/compliance") return [read ? "compliance.view" : "compliance.manage", ...global];
  if (path === "/api/admin/notifications/dispatch" || path === "/api/admin/lifecycle/dispatch") return ["notifications.dispatch", ...global];
  if (/^\/api\/admin\/(operations|files)(\/|$)/.test(path)) return ["operations.manage", ...global];
  return null;
}
export const CONSOLE_VIEWS: Record<string, string[]> = {
  ...
  subscriptions: ["subscriptions.manage"], reviews: ["catalog.manage"], notifications: ["notifications.manage", "data.all"], coupons: ["finance.manage"], settings: ["settings.manage"], audit: ["audit.view"],
};
...
const GLOBAL_CONSOLE_ACTIONS = new Set(["syncCatalogTemplates", "syncOfficialPrograms", "saveInstitution", "saveSpecialty", "createNotification", "dispatchNotifications"]);
...
export function consoleActionPermissions(action: string, entityType?: unknown) {
  if (action === "deleteEntity") return typeof entityType === "string" && DELETE_AREA[entityType] ? ["records.delete", DELETE_AREA[entityType], ...(["institution", "specialty", "notification"].includes(entityType) ? ["data.all"] : [])] : null;
  const required = CONSOLE_ACTIONS[action];
  return required ? [...required, ...(GLOBAL_CONSOLE_ACTIONS.has(action) ? ["data.all"] : [])] : null;
}
