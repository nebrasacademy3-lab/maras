...
export function uploadWithProgress<T>({
  ...
  return new Promise<T>((resolve, reject) => {
    if (signal?.aborted) {
      reject(new UploadError("تم إلغاء الرفع.", 499));
      return;
    }
    ...
    xhr.upload.onprogress = (event) => {
      ...
      const remainingSeconds =
        total > event.loaded && bytesPerSecond > 0
          ? Math.ceil((total - event.loaded) / bytesPerSecond)
          : null;
    ...
    xhr.onerror = () => {
      reject(new UploadError("تعذر الاتصال بالخادم أثناء الرفع."));
    };

    xhr.ontimeout = () => {
      reject(new UploadError("استغرق الرفع وقتًا أطول من المتوقع.", 408));
    };
...
export function uploadProgressLabel(progress: UploadProgress) {
  const speed = progress.bytesPerSecond > 0
    ? `${formatBytes(progress.bytesPerSecond)}/ث`
    : "جارٍ حساب السرعة";

  const remaining = progress.remainingSeconds == null
    ? ""
    : progress.remainingSeconds < 60
      ? ` · متبقٍ نحو ${progress.remainingSeconds} ث`
      : ` · متبقٍ نحو ${Math.ceil(progress.remainingSeconds / 60)} د`;

  return `${progress.percent}% · ${formatBytes(progress.loaded)}${
