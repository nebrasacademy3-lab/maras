...
export async function manageRegisteredDeviceTx(tx: Tx, input: DeviceCommand & { userId: number; actorEmail: string; ipAddress: string; now: string }) {
  ...
  await tx.insert(auditLogs).values({ actorEmail: input.actorEmail, action: input.action === "revoke_block" ? "revoke_registered_device" : `device.${input.action}`, entityType: "auth_device", entityId: String(device.id), beforeJson: JSON.stringify({ userId: input.userId, revokedAt: device.revokedAt, returnPolicy: device.returnPolicy || "blocked", blockedUntil: device.blockedUntil, revision: version }), afterJson: JSON.stringify({ userId: input.userId, reason: input.reason, returnPolicy: policy?.returnPolicy || device.returnPolicy || "blocked", blockedUntil: policy ? policy.blockedUntil : device.blockedUntil || null, revision: version + 1, enrollmentRestored: false, sessionsRevoked: sessions.length }), ipAddress: input.ipAddress, createdAt: input.now });
