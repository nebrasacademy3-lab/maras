import { studyTableCells, studyMarkdownLines } from "./study-markup.mjs";
import { createHash } from "node:crypto";
import katex from "katex";
import "katex/contrib/mhchem";

export const STUDY_PDF_VERSION = "maras-study-pdf-v2";
...
export function validatePdfInput(input) {
  ...
  if (input.recipient !== undefined && (typeof input.recipient !== "string" || !input.recipient.trim() || input.recipient.length > 200)) throw new StudyPdfError("PDF_INPUT_INVALID");
  const brand = input.branding;
  if (!brand || typeof brand !== "object" || typeof brand.description !== "string" || brand.description.length > 2000 || typeof brand.siteUrl !== "string" || typeof brand.whatsapp !== "string" || !Array.isArray(brand.links) || brand.links.length > 14) throw new StudyPdfError("PDF_BRANDING_INVALID");
  const safeUrl = (value) => {
...
export function studyMarkdownHtml(markdown) {
  ...
  const inline = (value, depth = 0) => {
    if (depth > 8) return escapePdfHtml(value);
    ...
    for (const token of value.matchAll(tokens)) {
      ...
      else if (token[6]) result += `<strong dir="auto">${inline(token[6], depth + 1)}</strong>`;
  ...
  const lines = studyMarkdownLines(markdown);
  const output = [];
  const cells = studyTableCells;
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    if (!line) continue;
    if (/^\\begin\{(?:align\*?|aligned|equation\*?|gather\*?|gathered)\}/.test(line)) throw new StudyPdfError("PDF_MARKUP_INCOMPLETE");
    if (/^```/.test(line)) {
      const code = [];
