...
async function handleAiSubscriptionCharge(verified: TapCharge, chargeId: string, status: string, orderNumber: string, reconciliation = false) {
  const db = getDb();
  ...
  await db.transaction(async (tx) => {
    ...
    if (reconciliation && current.status === "payment_review") { effectiveStatus = current.status; return; }
...
export async function POST(request: Request) {
  ...
  return retrieveAndApplyTapCharge(chargeId, tapSecretKey, true);
}

/** Server-only lookup shared by signed callbacks and reconciliation. This never
 * initiates a charge; scheduled lookups are not recorded as signed callbacks. */
export async function retrieveAndApplyTapCharge(chargeId: string, tapSecretKey: string, signatureVerified = false, expected?: { kind: "course" | "ai"; orderNumber: string }) {
  if (!/^chg_[A-Za-z0-9_-]{1,150}$/.test(chargeId)) return jsonError("معرّف العملية غير صالح");
  ...
  if (!verified || typeof verified !== "object" || Array.isArray(verified) || verified.id !== chargeId) return jsonError("تعذر مطابقة العملية", 409);

  const orderNumber = cleanText(verified.metadata?.order_number || verified.reference?.order || verified.reference?.transaction, 160);
  if (expected) {
    const kind = cleanText(verified.metadata?.product, 40) === "meras-ai" ? "ai" : "course";
    const providerOrderNumber = kind === "ai" ? cleanText(verified.metadata?.ai_order_number || orderNumber, 160) : orderNumber;
    if (kind !== expected.kind || providerOrderNumber !== expected.orderNumber) return jsonError("لا تطابق عملية Tap الطلب الجاري التحقق منه", 409);
  }
  const status = cleanText(verified.status, 60).toUpperCase();
  const db = getDb();
  ...
  await db.insert(paymentEvents).values({
    ...
    eventType: signatureVerified ? "charge_status" : "charge_reconciliation",
  ...
  if (cleanText(verified.metadata?.product, 40) === "meras-ai") {
    ...
    return handleAiSubscriptionCharge(verified, chargeId, status, aiOrderNumber, !signatureVerified);
  }
  ...
  await db.transaction(async (tx) => {
    ...
    if (!signatureVerified && current.status === "payment_review") { effectiveStatus = current.status; return; }
