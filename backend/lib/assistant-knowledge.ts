...
import { PUBLIC_SETTING_DEFAULTS, settingEnabled, type PublicSettings, whatsappHref } from "@/lib/platform-settings";
...
function intentFallback(intent: AssistantIntent, user: SessionUser | null, settings: PublicSettings, institutions: Institution[], courses: Course[]): AssistantReply | null {
  ...
  switch (intent) {
    case "registration": return registrationEnabled ? reply("لإنشاء حساب جديد: 1) افتح صفحة التسجيل. 2) أدخل الاسم والبريد والجوال وكلمة مرور قوية. 3) اختر الجامعة أو الكلية، ثم اختر تخصصًا تابعًا لها. 4) حدّد مستواك الدراسي ووافق على الشروط. بعد النجاح أكمل الجولة التعريفية. لا تستخدم بريدًا أو رقمًا مرتبطًا بحساب سابق، ولا تشارك كلمة المرور مع أي شخص.", [action("إنشاء حساب", "/register"), action("لدي حساب", "/login")], ["ما شروط كلمة المرور؟", "لماذا لا يقبل التسجيل؟"]) : reply("إنشاء الحسابات الجديدة متوقف مؤقتًا من إدارة مراس. الحسابات الحالية تعمل بشكل طبيعي، ويمكنك تسجيل الدخول أو متابعة الواجهة العامة إذا كان التصفح كضيف مفعّلًا. سيظهر خيار إنشاء الحساب تلقائيًا عند إعادة فتح التسجيل.", [action("تسجيل الدخول", "/login"), action("الرئيسية", "/")], ["كيف أسجل الدخول؟", "متى يفتح التسجيل؟"]);
    ...
    case "course_request": return requestsEnabled ? reply("إذا لم تجد المادة: 1) افتح «طلب مادة». 2) اكتب الاسم والرمز إن وجد. 3) أضف ملاحظات عن المطلوب. 4) ارفع السلايدات أو توصيف المقرر المسموح بمشاركته. 5) أرسل الطلب وتابع حالته من لوحة الطالب. يُربط الطلب بجامعتك وتخصصك ليصل إلى المشرف الصحيح، وعند تجهيز المحتوى يصلك إشعار ورابط المادة.", [action("طلب مادة الآن", "/request-course"), action("متابعة الطلبات", "/dashboard?view=requests")], ["ما الملفات المقبولة؟", "كم يستغرق الطلب؟"]) : reply("خدمة طلبات المواد متوقفة مؤقتًا من إدارة مراس، لذلك لن يظهر نموذج إرسال طلب جديد الآن. يمكنك البحث في الكتالوج أو التواصل مع الدعم للاستفسار، وستعود الخدمة تلقائيًا عند إعادة تفعيلها.", [action("استكشاف المواد", "/courses"), action("الدعم", "/support")], ["كيف أبحث عن مادتي؟", "كيف أتواصل مع الدعم؟"]);
    case "payment": return paymentsEnabled ? reply("لشراء مادة: 1) سجّل الدخول وأكمل ملفك الجامعي. 2) افتح المادة وراجع السعر والوصف والمعاينة إن كانت متوفرة. 3) أضفها إلى السلة، وراجع المواد والإجمالي. 4) أدخل كوبونًا نشطًا إن وجد. 5) تابع الدفع عبر Tap. لا تعتبر المادة مفعّلة بمجرد العودة من صفحة الدفع؛ الخادم يؤكد العملية أولًا، ثم تظهر المادة في «موادي» وتصدر الفاتورة. عند مشكلة، أرسل رقم الطلب فقط ولا ترسل بيانات البطاقة.", [action("استكشاف المواد", "/courses"), action("السلة", "/cart"), action("الطلبات والفواتير", "/dashboard?view=orders")], ["لم تتفعل المادة", "أريد استرداد المبلغ"]) : reply("بدء الاشتراكات وعمليات الدفع الجديدة متوقف مؤقتًا من إدارة مراس. يمكنك الاستمرار في تصفح المواد ومشاهدة محتواك المفعّل سابقًا، وستبقى سلتك محفوظة. لا تحاول إجراء دفع خارجي أو تحويل مباشر؛ انتظر إعادة التفعيل من المنصة.", [action("استكشاف المواد", "/courses"), action("موادي", "/dashboard?view=courses")], ["هل موادي الحالية تعمل؟", "كيف أعرف أن الدفع عاد؟"]);
    ...
    case "profile": return reply(requestsEnabled ? "لإكمال ملفك: افتح «حسابي»، اختر الجامعة أولًا، ثم اختر تخصصًا تابعًا لها، وبعد ذلك حدّد المستوى الدراسي مثل الأول أو الثاني أو خريج. حفظ هذه البيانات يجعل التوصيات وطلبات المواد تصل إلى النطاق الصحيح. إذا لم يظهر تخصصك، اختر الجهة الصحيحة أو استخدم طلب المادة." : "لإكمال ملفك: افتح «حسابي»، اختر الجامعة أولًا، ثم اختر تخصصًا تابعًا لها، وبعد ذلك حدّد المستوى الدراسي مثل الأول أو الثاني أو خريج. حفظ هذه البيانات يجعل التوصيات والمحتوى المقترح أكثر دقة. إذا لم يظهر تخصصك، تأكد من الجهة أو تواصل مع الدعم.", [action("إعدادات الحساب", "/dashboard?view=account"), ...(requestsEnabled ? [action("طلب مادة", "/request-course")] : [action("الدعم", "/support")])], ["كيف أغيّر جامعتي؟", "ما المستويات المتاحة؟"]);
    ...
    case "university": return reply(`يعرض دليل مراس ${institutions.length} جهة تعليمية منشورة. يمكنك البحث بالاسم أو المنطقة أو نوع الجهة، ثم فتح صفحة الجهة لرؤية التخصصات والمواد المرتبطة بها.${requestsEnabled ? " إذا لم تجد جامعتك أو كلية معينة، اكتب اسمها كاملًا أو أرسل طلب مادة/جهة لتراجعها الإدارة." : " إذا لم تجد جامعتك أو كلية معينة، اكتب اسمها كاملًا أو تواصل مع الدعم لتراجعها الإدارة."}`, [action("فتح دليل الجامعات", "/universities"), action("استكشاف المواد", "/courses"), ...(requestsEnabled ? [action("طلب مادة أو جهة", "/request-course")] : [action("الدعم", "/support")])], ["أريد تخصصات جامعة معينة", "كيف أختار التخصص؟"]);
    case "specialty": return reply(`التخصصات تُعرض بحسب الجامعة المختارة حتى لا تختلط الخطط بين الجهات. افتح دليل الجامعات لاختيار الجهة، أو افتح المواد وحدد الجامعة ثم التخصص. قد يختلف اسم التخصص أو الخطة من جامعة إلى أخرى؛ لذلك راجع رمز المادة ووصفها.${requestsEnabled ? " إذا كان تخصصك غير موجود، أكمل بيانات الحساب أولًا ثم أرسل طلب مادة مع اسم التخصص والجامعة." : " إذا كان تخصصك غير موجود، أكمل بيانات الحساب ثم تواصل مع الدعم ليتحقق الفريق من الجهة والتخصص."}`, [action("دليل الجامعات", "/universities"), action("بحث المواد", "/courses"), action("إكمال الحساب", "/dashboard?view=account"), ...(!requestsEnabled ? [action("الدعم", "/support")] : [])], ["ما مواد تخصصي؟", "تخصصي غير موجود"]);
    ...
    case "files": return reply(requestsEnabled ? "الملف المقبول يعتمد على الخدمة: طلبات المواد تقبل PDF وPPT/PPTX وDOC/DOCX وPNG/JPG بإجمالي يصل إلى 100 ميجابايت، بينما مرفقات الدعم تقبل الأنواع الآمنة التي تظهر في منتقي الملفات. فيديوهات الدروس يرفعها المدير أو المشرف المصرح من مساحة المحتوى، وتُفحص بصمة الحاوية وتخزن في مساحة خاصة. استخدم ملفات تملك حق مشاركتها، وانتظر رسالة النجاح قبل إغلاق الصفحة." : "مرفقات الدعم تقبل الأنواع الآمنة التي تظهر في منتقي الملفات. خدمة طلبات المواد متوقفة مؤقتًا من الإدارة، لذلك لن يظهر رفع مرفقات الطلب الآن. فيديوهات الدروس يرفعها المدير أو المشرف المصرح من مساحة المحتوى وتخزن في مساحة خاصة.", [...(requestsEnabled ? [action("طلب مادة ورفع ملفات", "/request-course")] : []), action("الدعم", "/support"), ...(user?.role === "admin" ? [action("لوحة الإدارة", "/admin")] : user?.role === "supervisor" ? [action("لوحة المشرف", "/supervisor")] : [])], ["كم حجم المرفق؟", "كيف أرفع فيديو؟"]);
    default: return null;
  }
}
...
export function answerAssistant(rawQuestion: string, user: SessionUser | null, publicSettings: PublicSettings = { ...PUBLIC_SETTING_DEFAULTS }, liveCatalog?: LiveAssistantCatalog): AssistantReply {
  ...
  const firstName = user?.fullName?.split(" ")[0];
  const registrationEnabled = settingEnabled(publicSettings.student_registration_enabled);
  const requestsEnabled = settingEnabled(publicSettings.course_requests_enabled);
  const paymentsEnabled = settingEnabled(publicSettings.payments_enabled);

  if (has(text, ["السلام", "مرحبا", "هلا", "اهلا", "صباح الخير", "مساء الخير"])) return {
    answer: `${firstName ? `أهلًا ${firstName}،` : "أهلًا بك،"} أنا مساعد مراس الذكي. أقدر أرشدك للجامعات والتخصصات والمواد والحساب والتعلم والدعم، وأراعي حالة الخصائص التي تتحكم بها الإدارة الآن. اكتب سؤالك بطريقتك.`,
    actions: user ? [action("فتح لوحتي", "/dashboard")] : registrationEnabled ? [action("إنشاء حساب", "/register")] : [action("تسجيل الدخول", "/login")],
    suggestions: ["ما لقيت مادتي", "كيف أجرب درسًا؟", "كيف أحمي حسابي؟"],
  };

  if (matchedCourse) { const hasPreview = matchedCourse.units.flatMap((unit) => unit.lessons).some((lesson) => lesson.free && lesson.ready); return {
    answer: `${matchedCourse.title} مدرجة لطلاب ${matchedCourse.university} ضمن ${matchedCourse.specialty}. تحتوي ${matchedCourse.lessons} درسًا لمدة ${matchedCourse.duration}، وسعرها ${matchedCourse.price} ر.س.${hasPreview ? " توجد معاينة مجانية جاهزة الآن." : " يمكنك مراجعة وصف وهيكل المادة قبل الاشتراك."}${paymentsEnabled ? " بعد الدفع المؤكد من الخادم تظهر في «موادي»." : " الاشتراكات الجديدة متوقفة مؤقتًا من الإدارة حاليًا."}`,
    ...
    suggestions: [hasPreview ? "كيف أشاهد المعاينة؟" : "ما محتوى المادة؟", "كيف أضيفها للمفضلة؟", paymentsEnabled ? "كيف أشتريها؟" : "متى يعود الدفع؟"],
  }; }
  ...
  if (institution) {
    ...
    return {
      ...
      actions: [action(`صفحة ${institution.name}`, `/universities/${institution.slug}`), ...(requestsEnabled ? [action("طلب مادة لهذه الجهة", "/request-course")] : [])],
  ...
  if (program) {
    ...
    return {
      ...
      actions: [action("البحث عن المواد", `/courses?q=${encodeURIComponent(program.name)}`), ...(requestsEnabled ? [action("طلب مادة", "/request-course")] : [])],
    };
  }
  ...
  if (has(text, ["اشتري", "ابغى اشتري", "ابي اشتري", "كيف اشتري", "شراء", "ادفع", "الدفع", "تاب", "بطاقه", "مدى", "فيزا", "ماستر", "السعر", "بكم"])) return paymentsEnabled ? {
    answer: "يلزم تسجيل الدخول وملف مكتمل قبل الشراء. اختر المادة وراجع وصفها والمعاينة إن كانت متوفرة ثم تابع الدفع الآمن عبر Tap. لا تتفعّل المادة بمجرد الرجوع من صفحة الدفع؛ التفعيل يتم فقط بعد تأكيد العملية من الخادم، ثم تظهر في «موادي» وتصدر الفاتورة.",
    actions: [action("تصفح المواد", "/courses"), action("الطلبات والفواتير", "/dashboard?view=orders")],
  } : { answer: "الاشتراكات الجديدة متوقفة مؤقتًا من إدارة مراس. يمكنك تصفح المواد ومتابعة موادك الحالية، وستظهر خيارات الدفع تلقائيًا عند إعادة التفعيل.", actions: [action("تصفح المواد", "/courses"), action("موادي", "/dashboard?view=courses")] };
  ...
  if (has(text, ["درس مجاني", "تجربه", "اجرب الشرح", "قبل الاشتراك"])) return {
    answer: "بعض المواد توفر معاينة مجانية عندما يكون درس تجريبي مرفوعًا وجاهزًا. إذا ظهرت شارة المعاينة في صفحة المادة يمكنك تشغيلها مباشرة قبل الاشتراك؛ وإذا لم تظهر فراجع وصف المادة ومحتوياتها المنشورة.",
  ...
  if (has(text, ["الملفات", "ارفع", "مرفقات", "pdf", "ppt", "بوربوينت"])) return {
    ...
    actions: requestsEnabled ? [action("رفع ملفات مع طلب مادة", "/request-course")] : [action("الدعم", "/support")],
  };
  ...
  if (has(text, ["اشعار", "التنبيهات", "وصلني", "حاله الطلب", "متابعه الطلب"])) return {
    ...
    actions: [action("الإشعارات", "/dashboard?view=notifications"), ...(requestsEnabled ? [action("طلبات المواد", "/dashboard?view=requests")] : [])],
  };
  ...
  return {
    ...
    actions: [action("مركز المساعدة", "/support"), action("استكشاف المواد", "/courses"), ...(requestsEnabled ? [action("طلب مادة", "/request-course")] : [])],
