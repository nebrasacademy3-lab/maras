...
export async function GET(request: Request) {
  ...
  const selected = await getDb().select({ notification: notificationsDb, readAt: notificationReads.readAt })
    ...
    .where(visibleFor(user.email, user.role)).orderBy(desc(notificationsDb.createdAt)).limit(100);
...
export async function PATCH(request: Request) {
  ...
  const visibleRows = await db.select({ id: notificationsDb.id }).from(notificationsDb)
    ...
    .limit(payload.all === true ? 100 : 1);
  if (!visibleRows.length) return payload.all === true ? Response.json({ ok: true, readAt: now }, { headers: mobileNoStoreHeaders }) : jsonError("الإشعار غير موجود", 404);
