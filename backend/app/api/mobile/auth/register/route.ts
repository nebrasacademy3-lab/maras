...
import { checkRateLimit, clientIp, createSession, hashPassword, sessionUserFromRow, validEmail, validPassword, validSaudiPhone } from "@/lib/auth";
...
export async function POST(request: Request) {
  ...
  const [created] = await db.insert(users).values({
    email, phone, fullName, passwordHash: await hashPassword(password), role: "student", universitySlug, specialty,
