...
export async function GET(request: Request) {
  ...
  const [accessRows, progressRows, orderRows, invoiceRows, requestRows, noticeRows, noticeReadRows, ticketRows] = await Promise.all([
    ...
    db.select().from(notificationsDb).where(and(or(eq(notificationsDb.userEmail, user.email), and(isNull(notificationsDb.userEmail), or(eq(notificationsDb.audience, user.role), eq(notificationsDb.audience, "public")))), or(isNull(notificationsDb.startsAt), lte(notificationsDb.startsAt, now)), or(isNull(notificationsDb.expiresAt), gt(notificationsDb.expiresAt, now)))).orderBy(desc(notificationsDb.createdAt)).limit(50),
  ...
  return Response.json({
    ...
    notifications: noticeRows.map((row) => ({ ...row, readAt: noticeReadMap.get(row.id) || null })),
