...
export async function POST(request:Request){
  ...
  if(files.length&&bucket){for(const file of files){const safe=file.name.replace(/[^a-zA-Z0-9._-]+/g,"-").slice(-120)||"attachment";const objectKey=`course-requests/${user.id}/${row.id}/${crypto.randomUUID()}-${safe}`;await bucket.put(objectKey,file.stream(),{httpMetadata:{contentType:file.type},customMetadata:{requestId:String(row.id),userId:String(user.id),visibility:"private"}});await db.insert(courseRequestFiles).values({requestId:row.id,userId:user.id,objectKey,originalName:file.name.slice(0,180),contentType:file.type,sizeBytes:file.size});}await db.update(courseRequests).set({attachmentsCount:files.length,updatedAt:new Date().toISOString()}).where(eq(courseRequests.id,row.id));}
