...
export async function PATCH(request:Request){
  if(!sameOriginRequest(request))return jsonError("تعذر التحقق من مصدر الطلب",403);const user=await getSessionUser(request);if(!roleAllowed(user,["supervisor","admin"]))return jsonError("غير مصرح",403);
  ...
  if(row.userId){const [student]=await db.select({email:users.email}).from(users).where(eq(users.id,row.userId)).limit(1);if(student){const title="تحديث طلب المادة";const body=`أصبحت حالة «${row.courseName}»: ${statusLabel(status)}.`;await db.insert(notificationsDb).values({userEmail:student.email,audience:"student",title,body,actionUrl:"/dashboard?view=requests"});await sendPushNotification({userEmail:student.email},title,body,{route:"/requests"});}}
