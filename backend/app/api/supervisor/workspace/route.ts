...
export async function POST(request: Request) {
  ...
  if(action==="saveUnit"){
    const title=cleanText(payload.title,160);if(title.length<2)return jsonError("اسم الوحدة قصير");
    const [created]=await db.insert(courseUnitsDb).values({courseSlug,title,position:Math.max(0,Math.floor(Number(payload.position)||0)),status:"published",createdAt:now,updatedAt:now}).returning({id:courseUnitsDb.id});
    await db.insert(auditLogs).values({actorEmail:scope.user.email,action:"create",entityType:"unit",entityId:String(created.id),afterJson:JSON.stringify({courseSlug,title})});return Response.json({ok:true,id:created.id},{status:201});
  }
  if(action==="saveLesson"){
    const id=cleanText(payload.id,100);const unitId=Math.floor(Number(payload.unitId));const title=cleanText(payload.title,160);if(!/^[a-z0-9._-]{2,100}$/i.test(id)||!unitId||title.length<2)return jsonError("تحقق من بيانات الدرس");
    ...
    const values={id,courseSlug,unitId,title,position:Math.max(0,Math.floor(Number(payload.position)||0)),durationSeconds:Math.max(0,Math.floor(Number(payload.durationSeconds)||0)),freePreview:payload.freePreview===true,status:"published",videoAssetId:existing?.videoAssetId||null,updatedAt:now};
    await db.insert(lessonsDb).values({...values,createdAt:existing?.createdAt||now}).onConflictDoUpdate({target:lessonsDb.id,set:values});await db.insert(auditLogs).values({actorEmail:scope.user.email,action:existing?"update":"create",entityType:"lesson",entityId:id,afterJson:JSON.stringify(values)});return Response.json({ok:true});
  }
