...
export async function POST(request: Request) {
  ...
  if (action === "createNotification") {
    ...
    const [created] = await db.insert(notificationsDb).values({ audience, title, body, userEmail, actionUrl, actionLabel, presentation, pushEnabled, pushDeliveredAt: pushScheduled ? null : now, startsAt, expiresAt, dismissible, createdAt: now }).returning({ id: notificationsDb.id });
    const push = pushScheduled
      ? { scheduled: true, attempted: 0, accepted: 0, rejected: 0, invalidated: 0, providerErrors: [] as string[] }
      : pushEnabled
        ? { scheduled: false, ...await sendPushNotification({ userEmail, audience }, title, body, { route: actionUrl || "/notifications", notificationId: created.id }) }
        : { scheduled: false, attempted: 0, accepted: 0, rejected: 0, invalidated: 0, providerErrors: [] as string[] };
    await audit(request, authorization.actor, "create", "notification", String(created.id), null, { audience, title, userEmail, push });
