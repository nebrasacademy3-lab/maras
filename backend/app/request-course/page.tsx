...
import { ChevronLeft, Sparkles, TrendingUp, Wrench } from "lucide-react";
...
export default async function RequestCoursePage() {
  ...
  const [institution, settings] = await Promise.all([getInstitutionCatalog(user.universitySlug || ""), getPublicSettings()]);

  if (!settingEnabled(settings.course_requests_enabled)) {
    return <main><SiteHeader appMode userName={user.fullName}/><section className="page-hero request-page-hero"><div className="container"><div className="breadcrumbs"><Link href="/dashboard">لوحتي</Link><ChevronLeft size={13}/><span>طلب مادة</span></div><span className="section-kicker"><Wrench size={14}/> الخدمة تحت تحكم الإدارة</span><h1>طلبات المواد متوقفة مؤقتًا</h1><p>أوقفت الإدارة استقبال الطلبات الجديدة حاليًا. طلباتك السابقة تبقى محفوظة في حسابك ويمكنك متابعة الإشعارات والدعم.</p><Link href="/dashboard" className="button button-primary">العودة إلى لوحتي</Link></div></section><SiteFooter/></main>;
  }
