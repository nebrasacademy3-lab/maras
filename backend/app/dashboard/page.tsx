...
export default async function DashboardPage({ searchParams }:{ searchParams:Promise<{view?:string}> }) {
  ...
  const [accessRows, progressRows, orderRows, requestRows, noticeRows, noticeReadRows, ticketRows, replyRows, catalogCourses, institutions, recommendedRows, settings] = await Promise.all([
    ...
    db.select().from(notificationsDb).where(and(or(eq(notificationsDb.userEmail,user.email),and(isNull(notificationsDb.userEmail),or(eq(notificationsDb.audience,"student"),eq(notificationsDb.audience,"public")))),or(isNull(notificationsDb.startsAt),lte(notificationsDb.startsAt,now)),or(isNull(notificationsDb.expiresAt),gt(notificationsDb.expiresAt,now)))).orderBy(desc(notificationsDb.createdAt)).limit(30),
    db.select({notificationId:notificationReads.notificationId,readAt:notificationReads.readAt}).from(notificationReads).where(eq(notificationReads.userEmail,user.email)).limit(500),
  ...
  const noticeReadMap=new Map(noticeReadRows.map((row)=>[row.notificationId,row.readAt]));
  const notices:DashboardNotice[]=noticeRows.map((row)=>({id:row.id,title:row.title,body:row.body,actionUrl:row.actionUrl,actionLabel:row.actionLabel,presentation:row.presentation,createdAt:row.createdAt,read:Boolean(noticeReadMap.get(row.id))}));
