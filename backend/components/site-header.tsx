    ...
    <header className={`site-header ${signedIn ? "site-header-app" : ""}`}>
      <div className="container header-inner">
        ...
        <div className="header-actions">
          ...
          {!signedIn ? <><Link href="/login" className="button button-ghost desktop-only">تسجيل الدخول</Link><Link href="/register" className="button button-primary desktop-only">إنشاء حساب</Link></> : <Link href="/dashboard?view=account" className="user-chip" aria-label={`حساب ${displayName}`}><span>{displayName.split(" ")[0]}</span><i>{displayName[0] || <UserRound size={16} />}</i></Link>}
