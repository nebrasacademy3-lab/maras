...
import { Bell, BookOpen, CircleHelp, GraduationCap, Heart, House, LayoutDashboard, LifeBuoy, LogOut, Menu, Search, ShoppingBag, UserRound, X } from "lucide-react";
...
export function SiteHeader({ appMode = false, userName = "طالب مراس" }: { appMode?: boolean; userName?: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/public/settings", { cache: "no-store", signal: controller.signal })
      .then(async (response) => response.ok ? await response.json() as { settings?: { student_registration_enabled?: string } } : null)
      .then((payload) => { if (!controller.signal.aborted) setRegistrationEnabled(payload?.settings?.student_registration_enabled !== "false"); })
      .catch(() => undefined);
    return () => controller.abort();
  }, []);
  ...
  useRealtimeSync((payload) => {
    ...
    if (!changed.length || changed.includes("settings")) fetch("/api/public/settings", { cache: "no-store" }).then(async (response) => response.ok ? await response.json() as { settings?: { student_registration_enabled?: string } } : null).then((payload) => setRegistrationEnabled(payload?.settings?.student_registration_enabled !== "false")).catch(() => undefined);
  });
    ...
    <header className={`site-header ${signedIn ? "site-header-app" : ""}`}>
      <div className="container header-inner">
        ...
        <div className="header-actions">
          ...
          {!signedIn ? <><Link href="/login" className="button button-ghost desktop-only">تسجيل الدخول</Link>{registrationEnabled&&<Link href="/register" className="button button-primary desktop-only">إنشاء حساب</Link>}</> : <Link href="/dashboard?view=account" className="user-chip" aria-label={`حساب ${displayName}`}><span>{displayName.split(" ")[0]}</span><i>{displayName[0] || <UserRound size={16} />}</i></Link>}
      ...
      {menuOpen && <div className={`mobile-nav container ${signedIn ? "mobile-nav-account" : ""}`}>
        ...
        <div className="mobile-nav-links">{activeLinks.map((link) => <Link key={link.href} href={link.href} className={`mobile-nav-link ${pathname === link.href ? "active" : ""}`} onClick={() => setMenuOpen(false)}><span className="mobile-nav-icon"><link.icon size={17} /></span><span>{link.label}</span>{link.href === "/cart" && cartSlugs.length > 0 && <b>{cartSlugs.length > 99 ? "99+" : cartSlugs.length}</b>}{link.href === "/favorites" && favoriteSlugs.length > 0 && <b>{favoriteSlugs.length > 99 ? "99+" : favoriteSlugs.length}</b>}<i>‹</i></Link>)}</div>
        {!signedIn && <div className="mobile-auth"><Link href="/login" className="button button-ghost" onClick={() => setMenuOpen(false)}>تسجيل الدخول</Link>{registrationEnabled&&<Link href="/register" className="button button-primary" onClick={() => setMenuOpen(false)}>إنشاء حساب</Link>}</div>}
