...
export function ForgotPasswordForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try{const response=await fetch("/api/auth/forgot-password",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:form.get("email")})});const data=await response.json() as {error?:string};if(!response.ok)throw new Error(data.error||"تعذر إرسال الطلب");setSent(true);}catch(caught){setError(caught instanceof Error?caught.message:"تعذر إرسال الطلب");}finally{setLoading(false);}
  };
  if (sent) return <div className="welcome-step"><div className="welcome-check"><CheckCircle2 size={38} /></div><span>تحقق من بريدك</span><h1>أرسلنا رابط الاستعادة</h1><p>إذا كان البريد مسجلًا فستصلك رسالة صالحة لمدة 15 دقيقة. افحص مجلد الرسائل غير المرغوب فيها أيضًا.</p><Link href="/login" className="button button-primary">العودة لتسجيل الدخول <ArrowLeft size={17} /></Link></div>;
