...
export function InstitutionPrograms({ programs, institutionName }: { programs: AcademicProgram[]; institutionName: string }) {
  ...
  return <div className="program-browser">
    ...
    <div className="program-results-note"><Sparkles size={15} /><span>نعرض {filtered.length} من {programs.length} برنامجًا، ومع كل برنامج مواد تأسيسية ومقترحة لتسهيل طلب الشرح.</span></div>
    {filtered.length ? <div className="program-grid">{filtered.map((program) => {
      ...
      return <details className="program-card" key={`${program.name}-${program.degree}`}>
        <summary><i><GraduationCap size={20} /></i><span><small>{program.area} · {program.degree}</small><strong>{program.name}</strong><em>{courseNames.length} مواد <ArrowLeft size={14} /></em></span></summary>
        <div className="program-card-body"><div className="program-courses-title"><BookOpen size={16} /><strong>مواد مرتبطة بالتخصص</strong></div><div className="program-course-chips">{courseNames.map((course) => <Link key={course} href={`/courses?q=${encodeURIComponent(course)}`}><BookMarked size={13} />{course}</Link>)}</div><footer><p>لا تجد المادة التي تحتاجها؟ أرسل السلايدات أو توصيف المقرر ليصل الطلب إلى المشرف.</p><Link className="button button-soft" href="/request-course">اطلب شرح مادة <ArrowLeft size={15} /></Link></footer></div>
      </details>;
