...
export function InstitutionPrograms({ programs, institutionName, requestsEnabled = true }: { programs: AcademicProgram[]; institutionName: string; requestsEnabled?: boolean }) {
  ...
  return <div className="program-browser">
    ...
    {filtered.length ? <div className="program-grid">{filtered.map((program) => {
      ...
      return <details className="program-card" key={`${program.name}-${program.degree}`}>
        ...
        <div className="program-card-body"><div className="program-courses-title"><BookOpen size={16} /><strong>مواد تأسيسية ومقترحة</strong></div><p className="program-disclaimer">هذه أسماء إرشادية للمساعدة في طلب المادة وليست خطة دراسية رسمية أو دورة منشورة. المواد المتاحة فعلًا تظهر في فهرس المواد بعد اعتماد الوحدات والدروس.</p>{program.sourceUrl&&program.verificationStatus==="official-program"?<a className="catalog-source" href={program.sourceUrl} target="_blank" rel="noreferrer">فتح مصدر البرنامج الرسمي <ArrowLeft size={14}/></a>:null}<div className="program-course-chips">{courseNames.map((course) => <Link key={course} href={`/courses?q=${encodeURIComponent(course)}`}><BookMarked size={13} />{course}</Link>)}</div><footer><p>{requestsEnabled?"لا تجد المادة التي تحتاجها؟ أرسل السلايدات أو توصيف المقرر ليصل الطلب إلى المشرف.":"المواد المنشورة فعليًا تظهر في الفهرس، ويمكنك التواصل مع الدعم للاستفسار عن مادة غير موجودة."}</p>{requestsEnabled?<Link className="button button-soft" href="/request-course">اطلب شرح مادة <ArrowLeft size={15} /></Link>:<Link className="button button-soft" href="/support">الدعم الفني <ArrowLeft size={15} /></Link>}</footer></div>
      </details>;
    })}</div> : <div className="program-empty"><Search size={27} /><h3>لا توجد نتيجة مطابقة</h3><p>جرّب اسمًا أقصر أو اختر «الكل».</p></div>}
  </div>;
}
