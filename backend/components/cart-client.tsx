...
type CartPayload = { items: Course[]; subtotal: number; count: number };
...
export function CartClient() {
  ...
  useEffect(() => {
    ...
    fetch("/api/cart", { credentials: "same-origin", cache: "no-store", signal: controller.signal })
      ...
      .then((data) => { if (!controller.signal.aborted) { setCart({ items: data.items || [], subtotal: data.subtotal || 0, count: data.count || 0 }); setLoading(false); } })
