...
export function FavoritesClient({ courses }: { courses: Course[] }) {
  ...
  useEffect(() => { fetch("/api/favorites", { credentials: "same-origin", cache: "no-store" }).then(async (response) => { const data = await response.json() as { courseSlugs?: string[]; error?: string }; if (!response.ok) throw new Error(data.error || "تعذر تحميل المفضلة"); setSlugs(data.courseSlugs || []); }).catch((caught) => setError(caught instanceof Error ? caught.message : "تعذر تحميل المفضلة")).finally(() => setLoading(false)); }, []);
  const saved = useMemo(() => courses.filter((course) => slugs.includes(course.slug)), [courses, slugs]);
  async function remove(slug: string) { setBusy(slug); try { const response = await fetch("/api/favorites", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ courseSlug: slug, active: false }) }); const data = await response.json() as { courseSlugs?: string[]; error?: string }; if (!response.ok) throw new Error(data.error || "تعذر الحذف"); setSlugs(data.courseSlugs || []); } catch (caught) { setError(caught instanceof Error ? caught.message : "تعذر الحذف"); } finally { setBusy(""); } }
