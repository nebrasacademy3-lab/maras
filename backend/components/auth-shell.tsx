...
export function LoginForm() {
  ...
  return <form className="auth-form" onSubmit={submit}>
    ...
    <label className="form-label">البريد الإلكتروني أو رقم الجوال<div className="input-with-icon"><Mail size={18} /><input name="identifier" required autoComplete="username" placeholder="name@example.com أو 05xxxxxxxx" dir="ltr" /></div></label>
...
export function RegisterForm({ institutions }: { institutions: Institution[] }) {
  ...
  return <form className="auth-form register-form" onSubmit={submit}>
    ...
    {step === 1 && <>
      ...
      <label className="form-label">الاسم الكامل<div className="input-with-icon"><UserRound size={18} /><input required minLength={5} autoComplete="name" value={data.fullName} onChange={(event) => update("fullName", event.target.value)} placeholder="الاسم الثلاثي" /></div></label>
      <div className="two-fields"><label className="form-label">رقم الجوال السعودي<div className="input-with-icon"><Phone size={18} /><input required pattern="(?:\\+?966|0)?5[0-9]{8}" autoComplete="tel" value={data.phone} onChange={(event) => update("phone", event.target.value)} placeholder="05xxxxxxxx" dir="ltr" /></div></label><label className="form-label">البريد الإلكتروني<div className="input-with-icon"><Mail size={18} /><input required type="email" autoComplete="email" value={data.email} onChange={(event) => update("email", event.target.value)} placeholder="name@example.com" dir="ltr" /></div></label></div>
