...
function Support({rows}:{rows:DashboardTicket[]}) { return <><DashboardTitle title="الدعم الفني" description="التذاكر والردود مرتبطة بحسابك ولا يراها غيرك وفريق الدعم." action={<Link href="/support" className="button button-primary">فتح تذكرة جديدة</Link>}/>{rows.length?<section className="dashboard-panel support-ticket-history">{rows.map((ticket)=><Link className="support-ticket-card" href={`/support?ticket=${ticket.id}`} key={ticket.id}><header><span><strong>{ticket.title}</strong><small dir="ltr">#{ticket.ticketNumber}</small></span><em>{statusLabel(ticket.status)}</em></header><p>{ticket.category} · آخر تحديث {new Date(ticket.createdAt).toLocaleDateString("ar-SA")}</p><div className="support-ticket-card-footer"><span>{ticket.replies.length ? `${ticket.replies.length} ردود في المحادثة` : "بانتظار رد فريق الدعم"}</span><b>فتح المحادثة <ArrowLeft size={14}/></b></div></Link>)}</section>:<section className="dashboard-panel support-contact"><LifeBuoy size={33}/><h2>لا توجد تذاكر بعد</h2><p>اذكر رقم الطلب أو اسم المادة لتسريع المعالجة.</p><Link href="/support" className="button button-soft">تواصل مع الدعم</Link></section>}</>; }
