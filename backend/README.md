...
إذا طابق البريد أو الجوال حساب موظف موجود، ترفض الواجهة الطلب افتراضيًا. تحديثه يتطلب إرسال `allowExisting: true` مع عبارة التأكيد العربية التي يعيدها رد التعارض، إضافة إلى التأكيد الصريح في لوحة الإدارة. ترفض العملية تعارض البريد والجوال بين حسابين، ولا تغيّر بريد الحساب الموجود من هذا المسار، وتبقى حماية المدير الحالي وآخر مدير فعّال مطبقة داخل المعاملة.

تعتمد لوحتا الويب وExpo على `/api/admin/console` وعقود التغيير نفسها لإدارة الحسابات والنطاقات والكتالوج والوحدات والدروس والطلبات والدعم والتجارة والإعلانات والإشعارات. تتحقق الواجهة من العلاقات المتاحة لتسهيل الاختيار، لكن الخادم يكرر التحقق من الصلاحية والنطاق ولا يعتمد على إخفاء عناصر الواجهة.
...
يشغّل `npm test` بناء الإنتاج ثم عقود الوظائف وتشغيل خادم الإنتاج ومسار صفحة الدخول ومسار الصحة عند غياب قاعدة البيانات. شغّل `npx drizzle-kit check` كذلك قبل النشر، وراجع النتائج المؤرخة في `../VALIDATION.md` بدل الاعتماد على رقم ثابت داخل هذا الملف. اختبار PostgreSQL وTap الفعليين يبقى مطلوبًا داخل بيئة Railway لأن الحزمة لا تحتوي أسرار إنتاج أو قاعدة بيانات المستخدم.
...
| `/api/assistant` | مساعد مراس المتجدد: يقرأ بيانات PostgreSQL والإعدادات الحية، ويستخدم مزودًا متوافقًا مع OpenAI اختياريًا مع بديل محلي، ولا يعيد إلا مسارات إجراءات داخلية محكومة يفسرها الويب وExpo بأمان. |
