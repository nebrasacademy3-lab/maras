...
const courseAccess = pgTable("course_access", {
  userId: integer("user_id"), userEmail: text("user_email"), courseSlug: text("course_slug"), suspendedAt: text("suspended_at"),
...
test("private support context is scoped to the current account ID, never its reusable email", () => {
  assert.match(context, /inArray\(supportReplies\.ticketId, ticketRows\.map/);
  assert.match(context, /eq\(supportTickets\.userId, user\.id\)/);
  assert.doesNotMatch(context, /eq\(supportTickets\.userEmail, user\.email\)/);
...
test("live retrieval stays ahead of bounded, active account context", () => {
  ...
  assert.match(context, /activeUserAccessWhere\(user\.id, now\)/);
  const instant = "2026-09-12T12:00:00.000Z";
  const query = new PgDialect().sqlToQuery(access.activeUserAccessWhere(123456789, instant));
  const predicate = query.sql.replace(/\s+/g, " ");
  assert.ok(query.params.includes(123456789));
  assert.ok(!predicate.includes("123456789"), "identity is bound as a parameter");
  assert.match(predicate, /"user_id" = \$\d+/);
  assert.doesNotMatch(predicate, /user_email/);
  ...
  assert.match(predicate, /access_grant\.user_id="course_access"\."user_id"/);
