...
test("MP4 HEAD cancels the unused private storage stream but preserves its metadata", async () => {
  ...
  assert.equal(response.status, 200); assert.equal(response.headers.get("content-length"), "1024");
  assert.equal((await response.arrayBuffer()).byteLength, 0); assert.equal(f.cancellations(), 1);
});
