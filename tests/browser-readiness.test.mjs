...
test('browser readiness waits for real finite public responses and load, not an inline palette or networkidle',async()=>{const f=pageFixture();await navigatePublicPage(f.page,'https://qa.example/about');assert.equal(f.calls[0].options.waitUntil,'load');assert.ok(f.calls[0].options.timeout>0);assert.equal(f.calls.filter(c=>c.finished).length,4);assert.equal(f.listeners.size,0);});
...
test('a never-finishing startup response fails under a bounded deadline',async()=>{
 const f=pageFixture(), original=f.page.waitForResponse;
 f.page.waitForResponse=predicate=>original(predicate).then(response=>({...response,finished:()=>new Promise(()=>{})}));
 await assert.rejects(navigatePublicPage(f.page,'https://qa.example/about',{timeout:40}),/timed out/);
 assert.equal(f.listeners.size,0);
});
test('a failed response body is not accepted merely because its headers were 200',async()=>{
 const f=pageFixture(), original=f.page.waitForResponse;
 f.page.waitForResponse=predicate=>original(predicate).then(response=>({...response,finished:async()=>new Error('connection closed')}));
 await assert.rejects(navigatePublicPage(f.page,'https://qa.example/about'),/body failed/);
 assert.equal(f.listeners.size,0);
});
test('a hidden document without animation frames cannot hang QA indefinitely',async()=>{
 const f=pageFixture();f.page.evaluate=()=>new Promise(()=>{});
 await assert.rejects(navigatePublicPage(f.page,'https://qa.example/about',{timeout:40}),/timed out/);
 assert.equal(f.listeners.size,0);
});
