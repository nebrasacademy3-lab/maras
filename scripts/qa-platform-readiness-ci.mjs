/** Isolated release acceptance: real PostgreSQL/routes/PDF, synthetic accounts and providers only. */
...
for (const script of ["qa-instructor-assignments.ts", "qa-instructor-contracts.ts", "qa-account-deletion.ts"]) await run([...argv, `scripts/${script}`]);
console.log("INSTRUCTOR_PRIVACY_ACCEPTANCE_OK: synthetic contracts, private uploads, migrations and account deletion; no live transactions or native-device claims.");
