...
const repeat = Math.max(1, Math.min(10, Number(process.argv.find(value => value.startsWith("--repeat="))?.split("=")[1]) || 1));
...
async function runCase(engine,scene,mode,viewport,theme) {
  ...
  try {
    ...
    const stageMotion=await page.locator('.secure-player-stage').evaluate(stage=>({transition:getComputedStyle(stage).transitionProperty,animation:getComputedStyle(stage).animationName}));
    assert.equal(stageMotion.transition,'none','Player geometry must not inherit motion transitions');assert.equal(stageMotion.animation,'none');item.stageMotion=stageMotion;
    ...
    if(process.argv.includes('--debug')){item.settling=[];for(const delay of [250,1000]){await page.waitForTimeout(delay);item.settling.push({delay,geometry:await geometry(page),style:await page.locator('.secure-player-stage').evaluate(stage=>{const style=getComputedStyle(stage),shell=stage.parentElement;return{className:shell.className,inline:shell.style.cssText,width:style.width,height:style.height,transform:style.transform,translate:style.translate,top:style.top,left:style.left,animation:style.animation,transition:style.transition,documentDirection:getComputedStyle(document.documentElement).direction}})});}await page.screenshot({path:path.join(folder,`settled-${engine.name}-${scene}-${mode}-${viewport.width}-${theme}.png`)});}
  }
