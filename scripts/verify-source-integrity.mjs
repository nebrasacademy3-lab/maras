...
const files = ["lib/staff-policy.ts", "mobile/src/lib/staff-policy.ts", "lib/device-access-policy.ts", "mobile/src/lib/device-access-policy.ts", "tests/auth-device-enrollment-behavior.test.mjs", "tests/platform-security-runtime.test.mjs", "lib/public-origin.ts", "lib/seo.ts"];
...
for (const name of ["staff-policy.ts", "staff-contracts.ts", "information-contract.ts", "device-access-policy.ts"]) assert.equal(readFileSync(`lib/${name}`, "utf8"), readFileSync(`mobile/src/lib/${name}`, "utf8"), `Native/server contract mismatch: ${name}`);
