...
for (const name of ["admin-navigation.ts", "staff-policy.ts", "staff-contracts.ts", "information-contract.ts", "device-access-policy.ts", "operations-contract.ts", "study-progress.ts"]) assert.equal(readFileSync(`lib/${name}`, "utf8"), readFileSync(`mobile/src/lib/${name}`, "utf8"), `Native/server contract mismatch: ${name}`);
