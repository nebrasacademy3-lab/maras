...
try {
  ...
  for (const role of ["admin", "student-a", "student-b"]) {
    ...
    const user = (await client.query("INSERT INTO users (email,full_name,password_hash,role,is_platform_owner,email_verified_at,phone_verified_at,university_slug,specialty,academic_level,profile_completed_at,onboarding_completed_at) VALUES ($1,$2,$3,$4,($4 = 'admin'),$5,$5,'qa-university','علوم الاختبار','1',$5,$5) ON CONFLICT (email) DO UPDATE SET password_hash=EXCLUDED.password_hash RETURNING id", [email, role==="admin" ? "مدير الاختبار" : "طالب الاختبار "+role, hash, role==="admin" ? "admin" : "student", now])).rows[0];
