...
try {
  ...
  await db.query("INSERT INTO orders(user_id,order_number,customer_email,customer_name,course_slug,subtotal,total,status,paid_at) SELECT $3,'QA-ADMIN-ORDER-'||lpad(n::text,4,'0'),$1,'طالب اختبار الإدارة','qa-admin-roster',10,10,'paid',$2 FROM generate_series(1,525) n ON CONFLICT(order_number) DO NOTHING",[student.email, now, student.id]);
