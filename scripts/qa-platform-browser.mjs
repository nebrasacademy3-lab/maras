...
try {
  for (const name of names) {
    ...
    try {
      ...
      await editor.waitForFunction(async (email) => {
        const response = await fetch("/api/admin/staff?q=" + encodeURIComponent(email));
        if (!response.ok) return false;
        const payload = await response.json();
        return payload.staff?.some((member) => member.email === email);
      }, uniqueEmail);
