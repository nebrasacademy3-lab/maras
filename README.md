...
هذا المجلد يحتوي مصدر منصة مراس العلم وملفات نشرها. يعمل الخادم على **Node.js 22 + Next.js + Drizzle ORM + PostgreSQL**، ويقدم واجهة الويب وواجهات API التي يستخدمها تطبيق Expo.
...
npm ci
cp .env.example .env
# اضبط DATABASE_URL في بيئة العملية على قاعدة PostgreSQL التجريبية؛ ملف .env وحده لا يحمّله drizzle.config.ts
npm run db:migrate
npm run dev
```
