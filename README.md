...
هذه نسخة مصدر مطوّرة من منصة مراس العلم. **ابدأ بقراءة [دليل هذه النسخة](START_HERE_AR.md) و[نتائج التحقق الحالية](docs/validation-2026-09-06.md) قبل النشر؛ بناء الإنتاج والتجربة على الأجهزة لم يُتحقّق منهما في هذه الجولة.** يعمل الخادم على **Node.js 22 + Next.js + Drizzle ORM + PostgreSQL**، ويقدم واجهة الويب وواجهات API التي يستخدمها تطبيق Expo.
...
أنشئ خدمة Railway من جذر هذا المجلد الذي يحتوي `Dockerfile` و`package.json`. سيكتشف Railway ملف `Dockerfile`. أضف PostgreSQL من مشروع Railway أو استخدم قاعدة PostgreSQL متوافقة، ثم انسخ `DATABASE_URL` إلى Variables. أضف Volume واربطه بـ `/data`، لأن التخزين المحلي للفيديوهات والمرفقات والشعارات يعتمد على هذا المسار.
...
يسمح منطق جاهزية الكتالوج بالشراء عند وجود درس جاهز واحد على الأقل؛ لا يشترط اكتمال فيديوهات المادة كلها. تبقى شروط النشر والحساب والدفع مطبقة، ويُعرض الدرس التجريبي عندما يكون متاحًا. المادة التي لا تحتوي أي درس جاهز لا تُفتح للشراء في هذه النسخة.
...
يختبر `npm test` البناء وعقود الوظائف وتشغيل خادم الإنتاج ومسار صفحة الدخول ومسار الصحة عند غياب قاعدة البيانات. نتائج الاختبارات الحالية وحدودها موثقة في `docs/validation-2026-09-06.md`؛ لا تُعد نتائج إصدارات أقدم دليلًا على بناء هذه النسخة. اختبار PostgreSQL وTap الفعليين يبقى مطلوبًا داخل بيئة Railway لأن الحزمة لا تحتوي أسرار إنتاج أو قاعدة بيانات المستخدم.
