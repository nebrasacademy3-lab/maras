...
export default async function SupportPage() {
  ...
  return <main><SiteHeader appMode userRole={current.role} userName={current.fullName} /><section className="page-hero"><div className="container"><div className="breadcrumbs"><Link href={current.role === "instructor" ? "/instructor" : "/dashboard"}>لوحتي</Link><ChevronLeft size={13} /><span>الدعم</span></div><h1>كيف نقدر نساعدك؟</h1><p>اختر نوع المشكلة وأرسل التفاصيل. تظهر حالة التذكرة والردود داخل حسابك.</p></div></section><section className="content-page"><div className="container support-page-grid"><div className="support-form-card"><div><h2>فتح تذكرة جديدة</h2><p>كلما كانت التفاصيل أوضح، قدرنا نحل المشكلة أسرع.</p></div><SupportForm /></div><aside className="support-side"><article><Headphones size={25} /><h3>فريق مراس معك</h3><p>يظهر وقت الاستجابة المتوقع داخل تذكرتك وفق ضغط الطلبات وساعات العمل.</p><span><Clock3 size={14} /> {settings.support_hours}</span></article>{whatsapp && <article><MessageCircle size={23} /><div><strong>واتساب مراس</strong><small>للاستفسارات العامة</small></div><a href={whatsapp} target="_blank" rel="noreferrer">ابدأ المحادثة</a></article>}{settings.support_email && <article><Mail size={23} /><div><strong>البريد الإلكتروني</strong><small>{settings.support_email}</small></div><a href={`mailto:${settings.support_email}`}>إرسال</a></article>}<p><ShieldCheck size={15} /> لا ترسل بيانات البطاقة أو كلمة المرور داخل التذكرة.</p></aside></div></section><SiteFooter /></main>;
}
