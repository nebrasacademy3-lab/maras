...
export async function POST(request: Request) {
  ...
  if (status === "completed") {
    ...
    if (applied.newlyFullyRefunded) await createAndSendNotification({ values: { targetUserId: order.userId, userEmail: null, audience: "student", title: "اكتمل الاسترداد", body: `اكتمل استرداد الطلب ${approvedRefund.orderNumber} وتم إيقاف الوصول المرتبط به.`, actionUrl: "/dashboard?view=orders", actionLabel: "عرض الطلب" }, target: { userId: order.userId }, data: { route: "/dashboard?view=orders" } });
  }
  if (status === "provider_pending") await createAndSendNotification({ values: { targetUserId: order.userId, userEmail: null, audience: "student", title: "بدأت معالجة الاسترداد", body: `تم اعتماد استرداد الطلب ${approvedRefund.orderNumber} وأرسل إلى Tap للمعالجة.`, actionUrl: "/dashboard?view=orders", actionLabel: "عرض الطلب" }, target: { userId: order.userId }, data: { route: "/dashboard?view=orders" } });
