...
import { aiFiles, courseResources, courseAccess, courseBundles, courseRequestFiles, courseWaitlist, notificationsDb, orders, paymentSettlementLines, refundRequests, supportReplyFiles } from "@/db/schema";
...
export async function GET(request: Request) {
  ...
  const [waitlist, bundles, requestScans, supportScans, resourceScans, aiScans, pendingOrders, expiringAccess, pendingPush, pendingRefunds, unmatchedSettlements] = await Promise.all([
    ...
    db.select({ total: count() }).from(courseResources).where(eq(courseResources.scanStatus, "pending")),
    db.select({ total: count() }).from(aiFiles).where(eq(aiFiles.scanStatus, "pending")),
  ...
  return Response.json({
    ...
    queues: {
      filesPendingScan: Number(requestScans[0]?.total || 0) + Number(supportScans[0]?.total || 0) + Number(resourceScans[0]?.total || 0) + Number(aiScans[0]?.total || 0),
