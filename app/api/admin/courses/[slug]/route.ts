...
export async function GET(request: Request, { params }: Props) {
  ...
  const totals = await db.execute(sql`SELECT
    (SELECT count(*) FROM course_access WHERE course_slug = ${slug} AND ${scopedStudentSql(scopeId, sql`course_access.user_email`)}) AS subscriptions,
    (SELECT count(*) FROM course_access WHERE course_slug = ${slug} AND ${scopedStudentSql(scopeId, sql`course_access.user_email`)} AND ${active}) AS active,
    (SELECT count(*) FROM course_waitlist WHERE course_slug = ${slug} AND ${scopedStudentSql(scopeId, sql`course_waitlist.user_email`)} AND status = 'active') AS waiting,
    (SELECT count(*) FROM course_waitlist WHERE course_slug = ${slug} AND ${scopedStudentSql(scopeId, sql`course_waitlist.user_email`)} AND notified_at IS NOT NULL) AS notified`);
  ...
  if (kind === "waitlist") {
    ...
    const where = and(scopedStudentSql(scopeId, courseWaitlist.userEmail), eq(courseWaitlist.courseSlug, slug), status === "all" ? undefined : eq(courseWaitlist.status, status), search ? or(ilike(courseWaitlist.userEmail, pattern), ilike(users.fullName, pattern), ilike(users.phone, pattern)) : undefined);
