...
export async function POST(request: Request) {
  ...
  const [user] = await getDb().select().from(users).where(or(eq(users.email, identifier), eq(users.phone, loginPhoneCandidate(identifier)))).limit(1);
  ...
  return Response.json({ ok: true, user: account, next: accountNext(account) }, { headers });
}
