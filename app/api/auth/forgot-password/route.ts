...
export async function POST(request:Request){
  if(!sameOriginRequest(request))return jsonError("تعذر التحقق من مصدر الطلب",403);let payload:Record<string,unknown>;try{payload=await readBoundedJsonObject(request);}catch{return jsonError("بيانات غير صالحة");}
  const email=cleanText(payload.email??payload.identifier,180).toLowerCase();if(!validEmail(email))return jsonError("أدخل بريدًا صالحًا");const limiter=`${clientIp(request)}:${email}`;if(!await checkRateLimit("forgot-password",limiter,5,60*60))return jsonError("محاولات كثيرة. حاول لاحقًا.",429);
  ...
  const deliveryConfigured=Boolean(process.env.RESEND_API_KEY?.trim()&&process.env.EMAIL_FROM?.trim());
  return Response.json({ok:true,delivery:deliveryConfigured?"email":"disabled",message:deliveryConfigured?"إذا كان البريد مسجلًا فستصلك رسالة الاستعادة.":"خدمة البريد غير مفعّلة حاليًا؛ تواصل مع الدعم لاستعادة الحساب."});
}
