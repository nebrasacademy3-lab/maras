...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await getDb().select({ job: aiFileJobs, file: aiFiles }).from(aiFileJobs).innerJoin(aiFiles, eq(aiFileJobs.fileId, aiFiles.id)).where(and(eq(aiFileJobs.id, id), eq(aiFileJobs.userId, user.id), eq(aiFiles.userId, user.id))).limit(1);
  if (!row) return jsonError("الطلب غير موجود", 404);
  try {
    if (row.file.sourceResourceId !== null) await resolveAiSource(row.file, user, isNativeAppRequest(request) ? "app" : "web");
