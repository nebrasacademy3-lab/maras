...
import { scanColumns, queuedFileScan } from "@/lib/file-security";
...
export async function POST(request: Request) {
  ...
  const courseUrl = cleanText(fields.courseUrl, 500) || null;
  if (courseUrl) { try { if (new URL(courseUrl).protocol !== "https:") throw new Error(); } catch { await discardFiles(); return jsonError("رابط المادة يجب أن يكون رابط HTTPS صحيحًا"); } }
  ...
  try {
    row = await db.transaction(async (tx) => {
      ...
      if (files.length) await tx.insert(courseRequestFiles).values(files.map((file) => ({ requestId: created.id, userId: user.id, objectKey: file.objectKey, storageProvider: file.storageProvider, originalName: file.originalName, contentType: file.contentType, sizeBytes: file.sizeBytes, ...scanColumns(fileScans.get(file.objectKey)!) })));
      return created;
    });
  } catch {
    await discardFiles();
  ...
  return Response.json({ ok: true, request: { ...row, courseUrl, attachmentsCount: files.length } }, { status: 201, headers: { "cache-control": "no-store" } });
}
