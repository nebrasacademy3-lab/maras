...
async function authorizedLesson(userId: number, lessonId: string) {
  const db = getDb();
  ...
  const [access] = await db.select({ id: courseAccess.id }).from(courseAccess).where(activeCourseAccessWhere(userId, lesson.courseSlug)).limit(1);
...
export async function GET(request: Request) {
  ...
  const notes = await getDb().select().from(lessonNotes).where(and(eq(lessonNotes.userId, user.id), eq(lessonNotes.lessonId, lessonId))).orderBy(asc(lessonNotes.timestampSeconds), asc(lessonNotes.createdAt)).limit(MAX_NOTES_PER_LESSON);
...
export async function POST(request: Request) {
  ...
  const lesson = await authorizedLesson(user.id, lessonId);
  if (!lesson) return jsonError("لا توجد صلاحية نشطة لهذا الدرس", 403);
  const now = new Date().toISOString();
  const note = await getDb().transaction(async (tx) => {
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${`lesson-note:${user.id}:${lessonId}`}))`);
    const [total] = await tx.select({ value: count() }).from(lessonNotes).where(and(eq(lessonNotes.userId, user.id), eq(lessonNotes.lessonId, lessonId)));
...
export async function PATCH(request: Request) {
  ...
  const [existing] = await getDb().select().from(lessonNotes).where(and(eq(lessonNotes.id, id), eq(lessonNotes.userId, user.id))).limit(1);
  if (!existing) return jsonError("الملاحظة غير موجودة", 404);
  const lesson = await authorizedLesson(user.id, existing.lessonId);
  if (!lesson) return jsonError("لا توجد صلاحية نشطة لهذا الدرس", 403);
  const now = new Date().toISOString();
  const [note] = await getDb().update(lessonNotes).set({ body, timestampSeconds: noteTime(payload.timestampSeconds ?? existing.timestampSeconds, lesson.durationSeconds), updatedAt: now }).where(and(eq(lessonNotes.id, id), eq(lessonNotes.userId, user.id))).returning();
...
export async function DELETE(request: Request) {
  ...
  const [removed] = await getDb().delete(lessonNotes).where(and(eq(lessonNotes.id, id), eq(lessonNotes.userId, user.id))).returning({ id: lessonNotes.id });
...
export async function PUT(request: Request) {
  ...
  const lessonId = cleanText(payload.lessonId, 120);
  const body = cleanText(payload.body, 4000);
  const lesson = lessonId ? await authorizedLesson(user.id, lessonId) : null;
  if (!lesson) return jsonError("لا توجد صلاحية نشطة لهذا الدرس", 403);
  const [existing] = await getDb().select().from(lessonNotes).where(and(eq(lessonNotes.userId, user.id), eq(lessonNotes.lessonId, lessonId))).orderBy(asc(lessonNotes.createdAt)).limit(1);
  const now = new Date().toISOString();
  if (existing) {
    const [note] = await getDb().update(lessonNotes).set({ body, updatedAt: now }).where(eq(lessonNotes.id, existing.id)).returning();
    return Response.json({ ok: true, note }, { headers: mobileNoStoreHeaders });
  }
  const [note] = await getDb().insert(lessonNotes).values({ userId: user.id, userEmail: user.email, lessonId, body, timestampSeconds: 0, createdAt: now, updatedAt: now }).returning();
  return Response.json({ ok: true, note }, { status: 201, headers: mobileNoStoreHeaders });
}
