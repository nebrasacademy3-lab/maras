...
export default async function Home() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.startSection} aria-labelledby="start-title" data-home-reveal><div className="container"><header className={styles.startHeading}><span>خطوات قليلة. بداية واثقة.</span><h2 id="start-title">طريقك إلى المادة المناسبة.</h2><p>اختر البداية التي تناسبك، وستجد التفاصيل قبل اتخاذ قرارك.</p></header><div className={styles.startGrid}><Link href="/universities"><small>01 / اكتشف</small><GraduationCap size={27} /><h3>ابدأ من جامعتك</h3><p>تصفح الجامعات والتخصصات، ثم افتح المقرر الذي تبحث عنه.</p><span>اعثر على مسارك <ArrowLeft size={17} /></span></Link><Link href="/courses"><small>02 / جرّب</small><CirclePlay size={27} /><h3>تعرّف على أسلوب الشرح</h3><p>اطّلع على خطة المادة وشاهد الدروس التجريبية المتاحة قبل الاشتراك.</p><span>استكشف المقررات <ArrowLeft size={17} /></span></Link><Link href="/study-tools"><small>03 / أتقن</small><NotebookPen size={27} /><h3>اجعل المراجعة بطريقتك</h3><p>رتّب ملاحظاتك واستفد من أدوات التلخيص والترجمة والتدريب.</p><span>افتح أدوات المذاكرة <ArrowLeft size={17} /></span></Link></div></div></section>
